...
class BoondService:
    ...
    def find_best_boond_project_match(self, project_name: str, boond_projects: List[Dict[str, Any]], affaire_id: str = "", context_hint: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        context = self._get_metronome_match_context(project_name, affaire_id=affaire_id)
        hint = context_hint or {}
        for key in ["display_name", "affaire", "numero", "client"]:
            if clean_text(hint.get(key)):
                context[key] = clean_text(hint.get(key))
        target_texts = [
            project_name,
        ...
        for p in boond_projects:
            ...
            plabel = clean_text(p.get("project_label")) or pref
            ref_tokens = set(self.normalize_match_text(plabel).split())
            discriminant_hits = discriminant_tokens & ref_tokens

            score, reasons = self.score_project_match(target_texts, target_keywords, plabel)
            if score < 35:
                continue
    ...
    def get_project_imputation_summary(self, project_name: str, refresh: bool = False, affaire_id: str = "", context_hint: Optional[Dict[str, Any]] = None, boond_project_id: str = "") -> Dict[str, Any]:
        ...
        for item in catalog:
            attrs = item.get("attributes", {}) or {}
            pref = clean_text(attrs.get("reference"))
            ptitle = clean_text(attrs.get("title"))
            pname = clean_text(attrs.get("name"))
            label_parts = []
            for part in [pref, ptitle, pname]:
                if part and part not in label_parts:
                    label_parts.append(part)
            boond_projects_for_match.append({
                "project_id": clean_text(item.get("id")),
                "project_reference": pref or ptitle or pname,
                "project_label": " | ".join(label_parts),
            })

        forced_project_id = clean_text(boond_project_id)
        options_scored: List[Dict[str, Any]] = []
        for p in boond_projects_for_match:
            pref = clean_text(p.get("project_reference"))
            plabel = clean_text(p.get("project_label")) or pref
            score, _reasons = self.score_project_match(
                [project_name, clean_text((context_hint or {}).get("display_name")), clean_text((context_hint or {}).get("affaire")), clean_text((context_hint or {}).get("numero")), clean_text((context_hint or {}).get("client"))],
                self._keywords_from_values([project_name, clean_text((context_hint or {}).get("display_name")), clean_text((context_hint or {}).get("affaire")), clean_text((context_hint or {}).get("numero")), clean_text((context_hint or {}).get("client"))]),
                plabel,
            )
        ...
        if forced_project_id:
            ...
            for p in boond_projects_for_match:
                ...
                plabel_norm = self.normalize_match_text(plabel)
                # Tolère un forçage par ID technique ou par référence/titre lisible.
                if pid == forced_project_id or forced_norm == pref_norm or forced_norm == plabel_norm or (forced_norm and (forced_norm in pref_norm or forced_norm in plabel_norm)):
                    matched = {
                        "project_id": pid,
                        "project_reference": pref,
                        "match_score": 100,
                        ...
                        "match_warning": None,
                        "metronome_context": self._get_metronome_match_context(project_name, affaire_id=affaire_id),
                    }
                    self._last_match_debug = {
                        "input_project_name": project_name,
                        "forced_project_id": forced_project_id,
                        "metronome_context": matched.get("metronome_context"),
                        "top_candidates": [matched],
                    }
                    break

        if not matched:
            matched = self.find_best_boond_project_match(project_name, boond_projects_for_match, affaire_id=affaire_id, context_hint=context_hint)
        ...
        if not matched:
            return {
                ...
                "selection": {"forced_project_id": forced_project_id or None},
...
class PointageService:
    ...
    @staticmethod
    def _mission_key(mission_key: str) -> str:
        mk = slugify(mission_key)
        return mk or "__default__"
    ...
    def parse_planning(self, raw: bytes) -> List[Dict[str, Any]]:
        ...
        c_perimeter = self._get_col_by_position(first, 9)
        if not c_perimeter or c_perimeter == c_outline:
            c_perimeter = self._find_col(first, ["perimetre", "périmètre"])
        if c_perimeter == c_outline:
            c_perimeter = ""
        ...
        for idx, r in enumerate(rows):
            ...
            tasks.append({
                ...
                "perimeter": clean_text(r.get(c_perimeter)) if c_perimeter else "",
    ...
    def _ensure_mission_slot(self, proj: Dict[str, Any], mission_key: str) -> Tuple[str, Dict[str, Any]]:
        normalized = self._mission_key(mission_key)
        missions = proj.setdefault("missions", {})
        if normalized not in missions:
            if normalized == "__default__" and ("planning_tasks" in proj or "pointage" in proj or "workState" in proj):
                missions[normalized] = {
                    "planning_tasks": list(proj.get("planning_tasks", [])),
                    "pointage": dict(proj.get("pointage", {"__cetMembers": "", "__cstRate": 80})),
                    "workState": dict(proj.get("workState", {"expanded_levels": []})),
                    "updated_at": clean_text(proj.get("updated_at")) or now_iso(),
                }
            else:
                missions[normalized] = {
                    "planning_tasks": [],
        ...
        return normalized, missions[normalized]
    ...
    def import_planning(self, project_key: str, raw: bytes, mission_key: str = "") -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            normalized_mission_key, slot = self._ensure_mission_slot(proj, mission_key)
            existing_pointage = slot.get("pointage", {})
            remapped: Dict[str, Any] = {k: v for k, v in existing_pointage.items() if k.startswith("__")}
            by_task = {t.get("task_id"): t for t in slot.get("planning_tasks", [])}
            by_stable = {t.get("stable_key"): t for t in slot.get("planning_tasks", [])}
            for nt in tasks:
            ...
            slot["planning_tasks"] = tasks
            slot["pointage"] = remapped
            slot["updated_at"] = now_iso()
            proj["updated_at"] = now_iso()
            self._save(data)
        return self.get_project_data(project_key, normalized_mission_key)
    ...
    def save_pointage(self, project_key: str, pointage_patch: Dict[str, Any], work_state: Optional[Dict[str, Any]] = None, mission_key: str = "") -> Dict[str, Any]:
        with self._lock:
            ...
            proj = data["projects"][project_key]
            normalized_mission_key, slot = self._ensure_mission_slot(proj, mission_key)
            p = slot.setdefault("pointage", {"__cetMembers": "", "__cstRate": 80})
    ...
    def get_project_data(self, project_key: str, mission_key: str = "") -> Dict[str, Any]:
        data = self._load()
        proj = data.get("projects", {}).get(project_key, {"missions": {}, "planning_tasks": [], "pointage": {"__cetMembers": "", "__cstRate": 80}, "workState": {"expanded_levels": []}})
        missions = dict(proj.get("missions", {})) if isinstance(proj.get("missions"), dict) else {}
        if "__default__" not in missions:
            missions["__default__"] = {
                "planning_tasks": list(proj.get("planning_tasks", [])),
                "pointage": dict(proj.get("pointage", {"__cetMembers": "", "__cstRate": 80})),
                "workState": dict(proj.get("workState", {"expanded_levels": []})),
                "updated_at": clean_text(proj.get("updated_at")) or "",
            }

        selected_mission_key = self._mission_key(mission_key)
        if selected_mission_key != "__default__" and selected_mission_key not in missions:
            missions[selected_mission_key] = {
                "planning_tasks": [],
                "pointage": {"__cetMembers": "", "__cstRate": 80},
                "workState": {"expanded_levels": []},
                "updated_at": "",
        ...
        def slot_payload(slot: Dict[str, Any]) -> Dict[str, Any]:
            normalized_planning_tasks = self._normalize_task_dates(list(slot.get("planning_tasks", [])))
            tasks_local = self.compute_tasks(normalized_planning_tasks, slot.get("pointage", {}))
            return {
                "tasks": tasks_local,
                "pointage": slot.get("pointage", {}),
                "workState": slot.get("workState", {}),
                "updated_at": slot.get("updated_at", ""),
            }

        if mission_key:
            selected_slot = missions.get(selected_mission_key, {})
            payload = slot_payload(selected_slot)
            tasks = payload["tasks"]
            pointage = payload["pointage"]
            work_state = payload["workState"]
            updated_at = payload["updated_at"]
        else:
            tasks = []
            pointage = {"__cetMembers": "", "__cstRate": 80}
            work_state = {"expanded_levels": []}
            updated_at = proj.get("updated_at", "")
            for mk, slot in missions.items():
                payload = slot_payload(slot)
                tasks.extend(payload["tasks"])
                if mk == "__default__":
                    pointage = payload["pointage"]
                    work_state = payload["workState"]
                if clean_text(payload["updated_at"]):
                    updated_at = payload["updated_at"]
        ...
        return {
            ...
            "mission_key": selected_mission_key if mission_key else "",
...
def pointage_finance_summary(affaire_id: str, commande_ht: float, mission_key: str = "") -> Dict[str, float]:
    try:
        project_key = PointageService._project_key(affaire_id, "")
        data = pointage_service.get_project_data(project_key)
        tasks = data.get("tasks", []) if isinstance(data, dict) else []
        missions_meta = data.get("missions", []) if isinstance(data, dict) else []
    except Exception:
        tasks = []
        missions_meta = []
    ...
    task_count = len(actionable)

    selected_mission = clean_text(mission_key)
    if selected_mission:
        try:
            selected_data = pointage_service.get_project_data(project_key, mission_key=selected_mission)
            selected_tasks = selected_data.get("tasks", []) if isinstance(selected_data, dict) else []
        except Exception:
            selected_tasks = []
        actionable = [t for t in selected_tasks if not bool(t.get("is_summary"))]
        planned = sum(clean_number(t.get("plannedCostCst")) for t in actionable)
        actual = sum(clean_number(t.get("actualCostCst")) for t in actionable)
        task_count = len(actionable)

    mission_progress_ratios: List[float] = []
    for mission in missions_meta:
        if not isinstance(mission, dict):
            continue
        mk = clean_text(mission.get("mission_key"))
        if mk in ("", "__default__"):
            continue
        try:
            md = pointage_service.get_project_data(project_key, mission_key=mk)
            mtasks = md.get("tasks", []) if isinstance(md, dict) else []
        except Exception:
            mtasks = []
        m_actionable = [t for t in mtasks if not bool(t.get("is_summary"))]
        m_count = len(m_actionable)
        if m_count <= 0:
            continue
        m_ratio = sum(max(0.0, min(100.0, clean_number(t.get("progress")))) for t in m_actionable) / (m_count * 100.0)
        mission_progress_ratios.append(m_ratio)

    if selected_mission:
        planning_progress_ratio = (
            sum(max(0.0, min(100.0, clean_number(t.get("progress")))) for t in actionable) / (task_count * 100.0)
            if task_count > 0 else 0.0
        )
    elif mission_progress_ratios:
        planning_progress_ratio = sum(mission_progress_ratios) / len(mission_progress_ratios)
    else:
        planning_progress_ratio = (
            sum(max(0.0, min(100.0, clean_number(t.get("progress")))) for t in actionable) / (task_count * 100.0)
            if task_count > 0 else 0.0
    ...
    def schedule_ratio(task: Dict[str, Any]) -> float:
        start = MetronomeService._parse_date_only(clean_text(task.get("start")))
        end = MetronomeService._parse_date_only(clean_text(task.get("end"))) or start
        ...
        return max(0.0, min(1.0, elapsed_days / duration_days))

    planned_to_date_cost = sum(clean_number(t.get("plannedCostCst")) * schedule_ratio(t) for t in actionable)
    planned_to_date_ratio = (planned_to_date_cost / planned) if planned > 0 else 0.0
    ...
    cost_progress_ratio = (actual / planned) if planned > 0 else 0.0
    commande_value = clean_number(commande_ht)
    progress_amount = commande_value * planning_progress_ratio if commande_value > 0 else 0.0
    planned_progress_amount = commande_value * planned_to_date_ratio if commande_value > 0 else 0.0
    return {
        "pointage_task_count": task_count,
        "pointage_cost_planned_cst": round(planned, 2),
        "pointage_cost_actual_cst": round(actual, 2),
        "pointage_cost_planned_to_date_cst": round(planned_to_date_cost, 2),
        "production_cumulee_estimee": round(cumulative, 2),
        "production_cumulee_mensuelle": production_cumulative_by_month,
        "pointage_progress_ratio": round(planning_progress_ratio, 4),
        "pointage_planned_to_date_ratio": round(planned_to_date_ratio, 4),
        "pointage_remaining_ratio": round(max(0.0, 1.0 - planning_progress_ratio), 4),
        "pointage_cost_progress_ratio": round(cost_progress_ratio, 4),
        "pointage_progress_amount": round(progress_amount, 2),
        "pointage_planned_to_date_amount": round(planned_progress_amount, 2),
    }
...
def api_boond_imputation_by_project(project_name: str = Query(...), refresh: bool = Query(default=False), affaire_id: str = Query(default=""), affaire: str = Query(default=""), numero: str = Query(default=""), client: str = Query(default=""), display_name: str = Query(default=""), boond_project_id: str = Query(default="")):
    pname = clean_text(project_name)
    if not pname:
        raise HTTPException(status_code=400, detail="project_name obligatoire")
    try:
        return boond_service.get_project_imputation_summary(project_name=pname, refresh=bool(refresh), affaire_id=clean_text(affaire_id), context_hint={"affaire": clean_text(affaire), "numero": clean_text(numero), "client": clean_text(client), "display_name": clean_text(display_name)}, boond_project_id=clean_text(boond_project_id))
    except RuntimeError as exc:
        return JSONResponse(
            status_code=200,
            content={
...
def api_boond_debug_project_cost_path(project_name: str, affaire_id: str = "", affaire: str = "", numero: str = "", client: str = "", display_name: str = ""):
    ...
    matched = boond_service.find_best_boond_project_match(project_name, boond_projects_for_match, affaire_id=clean_text(affaire_id), context_hint={"affaire": clean_text(affaire), "numero": clean_text(numero), "client": clean_text(client), "display_name": clean_text(display_name)})
    if not matched:
        return {
            "input_project_name": project_name,
            "matched_project": None,
  ...
  <div class='wrap'>
    ...
    <section class='grid'>
      <article class='card'>
        ...
        <a id='dashboardLink' class='btn disabled' href='javascript:void(0)' aria-disabled='true'>Ouvrir</a>
      </article>
      <article class='card'>
        <h3>Finances</h3><p>Cockpit financier détaillé de l'affaire.</p>
        <a id='financeLink' class='btn disabled' href='javascript:void(0)' aria-disabled='true'>Ouvrir</a>
      </article>
...
<style>
:root{--bg:#eef2f7;--panel:#fff;--panel2:#f7f9fc;--line:#dfe5ef;--ink:#122033;--muted:#6e7a90;--accent:#ef8d00;--blue:#ef8d00;--green:#1d9a5b;--amber:#c48716;--red:#c84c4c;--shadow:0 14px 40px rgba(19,31,53,.08)}
*{box-sizing:border-box}body{margin:0;font-family:Inter,Segoe UI,Arial,sans-serif;background:linear-gradient(180deg,#f5f7fb 0%,#eef2f7 100%);color:var(--ink)}
...
.notice{padding:14px 16px;border-radius:16px;background:#fff7e7;color:#8c6211;border:1px solid #f0dcab}.error{padding:14px 16px;border-radius:16px;background:#fff0f0;color:#992f2f;border:1px solid #f1c6c6}.empty{padding:28px;border:1px dashed var(--line);border-radius:18px;color:var(--muted);text-align:center;background:#fafbfd}.small{font-size:13px;color:var(--muted)}.footer-row{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-top:12px}.calc-card-btn{width:100%;border:none;cursor:pointer;text-align:left;color:inherit;font:inherit}.calc-card-btn:focus-visible{outline:3px solid rgba(239,141,0,.35);outline-offset:2px}.calc-badge{display:inline-flex;align-items:center;gap:6px;margin-top:10px;padding:6px 10px;border-radius:999px;background:#fff7e8;color:#9a5a00;font-size:12px;font-weight:800}.calc-list{display:grid;gap:12px;margin-top:16px}.calc-step{border:1px solid #d8e0ee;border-radius:14px;padding:14px;background:#fff}.calc-step .calc-label{font-size:12px;color:#5b6880;text-transform:uppercase;font-weight:800}.calc-step .calc-value{margin-top:8px;font-size:24px;font-weight:900}.calc-step .calc-desc{margin-top:6px;font-size:13px;color:#6e7a90}.calc-formula{margin-top:16px;padding:14px;border-radius:14px;background:#f7f9fd;border:1px dashed #cfd8e8}.calc-formula code{display:block;white-space:pre-wrap;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:13px;color:#24354f}.pm-modal{position:fixed;inset:0;background:rgba(11,20,33,.45);display:flex;align-items:center;justify-content:center;padding:20px;z-index:50}.pm-modal.hidden{display:none}.pm-modal-box{background:#fff;border-radius:18px;max-width:min(960px,96vw);width:100%;max-height:84vh;overflow:auto;padding:18px;border:1px solid #d8e0ee;box-shadow:0 14px 40px rgba(19,31,53,.18)}
@media (max-width:1200px){.kpi-grid{grid-template-columns:repeat(3,1fr)}.prod-grid{grid-template-columns:repeat(2,1fr)}.layout{grid-template-columns:1fr}.meta-grid{grid-template-columns:repeat(2,1fr)}}@media (max-width:720px){.topbar{grid-template-columns:1fr}.menu-row,.project-row{flex-direction:column;align-items:stretch}.kpi-grid{grid-template-columns:1fr}.prod-grid{grid-template-columns:1fr}.meta-grid{grid-template-columns:1fr}.hero h2{font-size:30px}.select,.search{min-width:100%}.pm-modal{padding:12px}}
</style>
</head>
<body>
<div class='container'>
  ...
  <div class='kpis'><div class='kpi-grid'>
    ...
    <div class='kpi' id='kpiAvanceCard'><div class='label'>✅ Avancement financier</div><div class='value' id='kpiAvance'>0 %</div><div class='sub' id='kAvanceSub'>Facturé / commande</div></div>
    <div class='kpi' id='kpiPlanningCard'><div class='label'>🗂️ Avancement planning</div><div class='value' id='kPlanningAvance'>0 %</div><div class='sub' id='kPlanningAvanceSub'>Pointage moyen des tâches</div></div>
  </div></div>

  <div class='prod-pilot'><h3 style='margin:0 0 12px'>Pilotage production & rentabilité</h3><div class='prod-grid'><div class='prod-card'><div class='t'>Avancement financier</div><div id='prodFinPct' class='v'>0 %</div><div class='s'>Facturation cumulée / commande</div></div><div class='prod-card'><div class='t'>Avancement production pointé</div><div id='prodPointagePct' class='v'>0 %</div><div class='s'>Issue du pointage du planning</div></div><div class='prod-card'><div class='t'>Écart financier vs production</div><div id='prodGapPct' class='v'>0 pt</div><div id='prodGapEur' class='s'>0 €</div></div><div id='prodBoondRentabilityCard' class='prod-card future'><div class='t'>Rentabilité imputation BOOND</div><div id='prodBoondRentability' class='v'>-</div><div id='prodBoondRentabilitySub' class='s'>Facturé vs coût imputations ressources</div></div></div></div>

  <div class='section'>
    <h3>Niveau 2 — Imputation des temps (BOOND)</h3>
    <div id='boondImputationBox' class='empty'>Sélectionnez une affaire pour charger le coût cumulé BOOND.</div>
  </div>

  <div id='pointagePlanningFinanceSection' class='section' style='display:none;margin-top:18px;margin-bottom:28px'>
    <h3>Niveau 3 — Pointage du planning</h3>
    <div class='chart-controls'>
      <label for='pointageFinanceMissionSelect' class='small'>Mission planning :</label>
      <select id='pointageFinanceMissionSelect' class='chart-select'><option value=''>Aucune mission</option></select>
      <label for='pointageFinanceChartSelect' class='small'>Afficher :</label>
      <select id='pointageFinanceChartSelect' class='chart-select'>
        <option value='planned-vs-pointed'>Vente planifiée à date vs vente pointée à date</option>
        <option value='pointed-vs-boond'>Coût pointé à date vs coût de production BOOND</option>
      </select>
    </div>
    <div id='pointageFinanceInfo' class='small' style='margin-bottom:10px'>Section affichée uniquement si un pointage planning existe.</div>
    <div id='pointageFinanceChartTitle' class='small' style='font-weight:800;margin-bottom:8px'>Vente planifiée à date vs vente pointée à date</div>
    <div id='pointageFinanceChart' class='pilot-list'><div class='small'>Aucune donnée</div></div>
  </div>
  ...
  <div class='section' style='margin-top:12px'><h3>Niveau 3 — Détail des missions <span id='missionsMeta' class='small' style='margin-left:8px'>0 mission</span></h3><div id='missionsTableWrap' class='table-wrap'><div class='empty'>Sélectionnez une affaire pour afficher les missions.</div></div></div>

  <div class='section' style='margin-top:12px'><h3>Lecture projet & alertes</h3><div id='insightsBox' class='insights'><div class='empty' style='width:100%'>Sélectionnez une affaire.</div></div><div class='footer-row'><div class='small' id='statusMeta'>Cache en attente.</div></div></div>

  <div class='section' style='margin-top:18px'><h3>Niveau 2 — Mensuel détaillé</h3><div id='monthlyTableWrap' class='table-wrap'><div class='empty'>Sélectionnez une affaire pour afficher le détail mensuel.</div></div></div>

  <div id='calcDetailModal' class='pm-modal hidden'>
    <div class='pm-modal-box'>
      <div style='display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap'>
        <div>
          <div class='small' style='font-weight:800;text-transform:uppercase'>Détail du calcul</div>
          <h3 id='calcDetailTitle' style='margin:6px 0 0'>-</h3>
        </div>
        <button id='calcDetailClose' class='btn' type='button'>Fermer</button>
      </div>
      <div id='calcDetailBody' style='margin-top:14px'></div>
    </div>
  </div>
...
const state={cacheStatus:null,affaires:[],selectedAffaireId:"",selectedAffaire:null,monthlyExpanded:false,boondImputation:null,boondForcedProjectId:"",boondProjectSearch:"",pointageMissionKey:"",financeMissions:[]};
...
function financeMissionOptions(board){return ((board&&board.finance_missions)||[]).filter(m=>m&&String(m.mission_key||'').trim());}
async function loadSelectedAffaire(id){if(!id){state.selectedAffaireId='';state.selectedAffaire=null;state.boondImputation=null;state.financeMissions=[];state.pointageMissionKey='';renderAll();return;}state.selectedAffaireId=id;try{state.board=await api(`/api/project-management/board?affaire_id=${encodeURIComponent(id)}`);}catch(_){state.board=null;}state.financeMissions=financeMissionOptions(state.board||{});if(state.financeMissions.length&&!state.financeMissions.some(m=>String(m.mission_key||'')===String(state.pointageMissionKey||''))){state.pointageMissionKey=String((state.financeMissions[0]&&state.financeMissions[0].mission_key)||'');}if(!state.financeMissions.length){state.pointageMissionKey='';}const d=await api(`/api/finance/affaire/${encodeURIComponent(id)}?mission_key=${encodeURIComponent(state.pointageMissionKey||'')}`);state.selectedAffaire=d.affaire||null;state.boondImputation=null;renderAll();try{const boondName=(state.selectedAffaire&&(state.selectedAffaire.display_name||state.selectedAffaire.affaire))||'';if(boondName){state.boondForcedProjectId=localStorage.getItem(`boondForcedProjectId:${id}`)||'';state.boondImputation=await api(`/api/boond/imputations/by-project?project_name=${encodeURIComponent(boondName)}&affaire_id=${encodeURIComponent(id)}&affaire=${encodeURIComponent((state.selectedAffaire&&state.selectedAffaire.affaire)||'')}&numero=${encodeURIComponent((state.selectedAffaire&&state.selectedAffaire.numero)||'')}&client=${encodeURIComponent((state.selectedAffaire&&state.selectedAffaire.client)||'')}&display_name=${encodeURIComponent((state.selectedAffaire&&state.selectedAffaire.display_name)||'')}&boond_project_id=${encodeURIComponent(state.boondForcedProjectId||'')}`);}else{state.boondImputation={matched_project:null,totals:{}};}}catch(_){state.boondImputation={matched_project:null,totals:{}};}renderAll();localStorage.setItem('selectedAffaireId',id);}
...
document.getElementById('kPlanningAvance').textContent=pct(a.pointage_progress_ratio||0);const avSub=document.getElementById('kAvanceSub');if(avSub){avSub.textContent='Facturé / commande';}const planningSub=document.getElementById('kPlanningAvanceSub');if(planningSub){planningSub.textContent=`${fmt(a.pointage_task_count||0)} tâche(s) prises en compte`;}
...
function pointageFinanceCalculationRows(mode,a,boond){const today=new Date().toLocaleDateString('fr-FR');const boondCost=Number(boond.total_cost||0);const taskCount=Number(a.pointage_task_count||0);const plannedCstToDate=Number(a.pointage_cost_planned_to_date_cst||0);const pointedCst=Number(a.pointage_cost_actual_cst||0);const plannedCstTotal=Number(a.pointage_cost_planned_cst||0);if(mode==='pointed-vs-boond'){return [{key:'pointed-sale',label:'Vente pointée à date',value:pointedCst,color:'#ef8d00',subtitle:'Somme des coûts pointés CST issus du pointage',badge:'Cliquer pour voir le détail du calcul',details:{title:'Calcul — Vente pointée à date',intro:`Calcul basé sur la somme des coûts pointés CST sur ${taskCount} tâche(s) du planning.`,steps:[{label:'Source',value:'Planning pointé',desc:'Valeur récupérée depuis la partie planning de la gestion de projet.'},{label:'Coût pointé CST',value:euro(pointedCst),desc:'Somme des colonnes « Coût pointé CST » issues du pointage.'},{label:'Tâches prises en compte',value:fmt(taskCount),desc:'Nombre de tâches non récapitulatives intégrées au calcul.'}],formula:`Vente pointée à date = somme des coûts pointés CST
Total retenu = ${euro(pointedCst)}`}},{key:'boond-cost',label:'Coût de production BOOND',value:boondCost,color:'#1b6ef3',subtitle:'Productivité projet BOOND',badge:'Cliquer pour voir la source de calcul',details:{title:'Calcul — Coût de production BOOND',intro:'Valeur issue du projet matché dans BOOND, calculée côté productivité projet.',steps:[{label:'Projet BOOND',value:((state.boondImputation||{}).matched_project||{}).project_label||((state.boondImputation||{}).matched_project||{}).project_reference||'-',desc:'Projet BOOND retenu pour le rapprochement.'},{label:'Source BOOND',value:String(boond.cost_source||'projects_productivity'),desc:'Champ de BOOND utilisé pour récupérer le coût global.'},{label:'Coût retenu',value:euro(boondCost),desc:'Coût cumulé renvoyé par l’API BOOND pour ce projet.'}],formula:`Coût de production BOOND = coût cumulé du projet matché
Valeur renvoyée par BOOND : ${euro(boondCost)}`}}];}
return [{key:'planned-sale',label:'Vente planifiée à date',value:plannedCstToDate,color:'#5da2ff',subtitle:`Somme des coûts planifiés CST à la date du ${today}`,badge:'Cliquer pour voir le détail du calcul',details:{title:'Calcul — Vente planifiée à date',intro:`Calcul théorique à date (${today}) sans utiliser le pointage.`,steps:[{label:'Source',value:'Planning prévisionnel',desc:'Valeur récupérée depuis la partie planning de la gestion de projet.'},{label:'Coût planifié CST à date',value:euro(plannedCstToDate),desc:'Somme des colonnes « Coût planifié CST » proratisées à la date du jour.'},{label:'Coût planifié CST total',value:euro(plannedCstTotal),desc:'Somme totale du coût planifié CST sur les tâches prises en compte.'}],formula:`Vente planifiée à date = somme des coûts planifiés CST à la date du jour
Total retenu = ${euro(plannedCstToDate)}`}},{key:'pointed-sale',label:'Vente pointée à date',value:pointedCst,color:'#ef8d00',subtitle:'Somme des coûts pointés CST issus du pointage',badge:'Cliquer pour voir le détail du calcul',details:{title:'Calcul — Vente pointée à date',intro:`Calcul basé sur la somme des coûts pointés CST sur ${taskCount} tâche(s) du planning.`,steps:[{label:'Source',value:'Planning pointé',desc:'Valeur récupérée depuis la partie planning de la gestion de projet.'},{label:'Coût pointé CST',value:euro(pointedCst),desc:'Somme des colonnes « Coût pointé CST » issues du pointage.'},{label:'Tâches prises en compte',value:fmt(taskCount),desc:'Nombre de tâches non récapitulatives intégrées au calcul.'}],formula:`Vente pointée à date = somme des coûts pointés CST
Total retenu = ${euro(pointedCst)}`}}];}
function openCalcDetailModal(details){const modal=document.getElementById('calcDetailModal');const title=document.getElementById('calcDetailTitle');const body=document.getElementById('calcDetailBody');if(!modal||!title||!body||!details)return;title.textContent=details.title||'Détail du calcul';body.innerHTML=`<div class='small'>${esc(details.intro||'')}</div><div class='calc-list'>${(details.steps||[]).map(step=>`<div class='calc-step'><div class='calc-label'>${esc(step.label||'')}</div><div class='calc-value'>${esc(step.value||'-')}</div><div class='calc-desc'>${esc(step.desc||'')}</div></div>`).join('')}</div><div class='calc-formula'><div class='small' style='margin-bottom:8px;font-weight:800;color:#24354f'>Formule utilisée</div><code>${esc(details.formula||'')}</code></div>`;modal.classList.remove('hidden');}
function closeCalcDetailModal(){const modal=document.getElementById('calcDetailModal');if(modal)modal.classList.add('hidden');}
function renderPointageFinanceChart(){const sec=document.getElementById('pointagePlanningFinanceSection');const root=document.getElementById('pointageFinanceChart');const title=document.getElementById('pointageFinanceChartTitle');const missionSelect=document.getElementById('pointageFinanceMissionSelect');const missions=state.financeMissions||[];if(missionSelect){missionSelect.innerHTML=missions.length?missions.map(m=>`<option value='${esc(String(m.mission_key||''))}' ${String(m.mission_key||'')===String(state.pointageMissionKey||'')?'selected':''}>${esc(String(m.label||m.mission_key||'Mission'))}</option>`).join(''):"<option value=''>Aucune mission</option>";}const a=state.selectedAffaire||{};const boond=(state.boondImputation||{}).totals||{};const taskCount=Number(a.pointage_task_count||0);if(!sec||!root)return;if(taskCount<=0){sec.style.display='none';root.innerHTML="<div class='small'>Aucune donnée de pointage planning.</div>";return;}sec.style.display='block';const mode=((document.getElementById('pointageFinanceChartSelect')||{}).value)||'planned-vs-pointed';const rows=pointageFinanceCalculationRows(mode,a,boond);if(title)title.textContent=mode==='pointed-vs-boond'?'Vente pointée à date vs coût de production BOOND':'Vente planifiée à date vs vente pointée à date';const max=Math.max(1,...rows.map(r=>Number(r.value||0)));root.innerHTML=`<div style='display:grid;grid-template-columns:repeat(${rows.length},minmax(260px,1fr));gap:18px'>${rows.map((r,idx)=>{const width=Math.max(4,(Number(r.value||0)/max)*100);return `<button type='button' class='pilot-row calc-card-btn' data-calc-index='${idx}' style='display:block;padding:18px'><div style='display:flex;justify-content:space-between;gap:18px;align-items:flex-start'><div style='min-width:0'><div style='font-weight:800;font-size:16px'>${esc(r.label)}</div><div class='small' style='margin-top:4px'>${esc(r.subtitle)}</div><div class='calc-badge'>🧮 ${esc(r.badge||'Cliquer pour voir le calcul')}</div></div><div style='font-weight:900;white-space:nowrap'>${euro(r.value||0)}</div></div><div style='margin-top:14px;height:14px;border-radius:999px;background:#edf1f8;overflow:hidden'><div style='height:100%;width:${width}%;background:${r.color}'></div></div></button>`;}).join('')}</div>`;Array.from(root.querySelectorAll('[data-calc-index]')).forEach(btn=>btn.addEventListener('click',()=>{const idx=Number(btn.getAttribute('data-calc-index')||-1);if(idx>=0&&rows[idx])openCalcDetailModal(rows[idx].details);}));}
...
document.getElementById('exportBtn').addEventListener('click',()=>{if(!state.selectedAffaireId)return;window.location.href=`/api/finance/affaire/${encodeURIComponent(state.selectedAffaireId)}/export-csv`;});const pointageFinanceMissionSelect=document.getElementById('pointageFinanceMissionSelect');if(pointageFinanceMissionSelect)pointageFinanceMissionSelect.addEventListener('change',async e=>{state.pointageMissionKey=e.target.value||'';if(state.selectedAffaireId)await loadSelectedAffaire(state.selectedAffaireId);});const pointageFinanceChartSelect=document.getElementById('pointageFinanceChartSelect');if(pointageFinanceChartSelect)pointageFinanceChartSelect.addEventListener('change',renderPointageFinanceChart);const calcDetailClose=document.getElementById('calcDetailClose');if(calcDetailClose)calcDetailClose.addEventListener('click',closeCalcDetailModal);const calcDetailModal=document.getElementById('calcDetailModal');if(calcDetailModal)calcDetailModal.addEventListener('click',e=>{if(e.target===calcDetailModal)closeCalcDetailModal();});}
initFinancePage();
</script>
</body>
</html>
...
.top{padding:14px;display:grid;grid-template-columns:auto 1fr;gap:12px;align-items:start}.logo-link{display:inline-flex;text-decoration:none}.brand-mini{display:inline-flex;align-items:center;justify-content:center;width:78px;height:78px;border:1px solid var(--line);border-radius:16px;background:#fff;box-shadow:0 8px 20px rgba(18,32,51,.08)}.brand-mini img{width:48px;height:48px;object-fit:contain}.top-main{display:flex;flex-direction:column;gap:10px}.menu-row,.project-row{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.search,.select,.board-notes{height:44px;border:1px solid var(--line);border-radius:12px;padding:0 12px;min-width:260px}.search.locked,.select.locked{background:#eef2f8;color:#6e7a90;pointer-events:none}
.btn{height:44px;border-radius:12px;border:none;padding:0 14px;background:var(--accent);color:#fff;text-decoration:none;display:inline-flex;align-items:center;font-weight:800;cursor:pointer}.btn.secondary{background:#122033}.btn.ghost{background:#fff;color:var(--ink);border:1px solid var(--line)}
.kpis{margin-top:12px;padding:12px}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.k{border:1px solid var(--line);border-radius:14px;padding:10px 14px;background:#fbfdff;min-height:96px}.k .v{font-size:28px;font-weight:900;margin-top:4px;line-height:1}
.section{margin-top:12px;padding:14px}.subgrid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:14px}table{width:100%;border-collapse:collapse}th,td{padding:10px 12px;border-bottom:1px solid var(--line);border-right:1px solid #e8edf5;font-size:13px;text-align:left;vertical-align:top}th:last-child,td:last-child{border-right:none}th{background:#f7f9fc;font-size:12px;color:#5b6880;text-transform:uppercase}.small{color:var(--muted);font-size:13px}
.bar{height:10px;background:#edf1f8;border-radius:999px;overflow:hidden}.fill{height:100%;background:#ef8d00}
.loading-wrap{margin-top:8px;padding:8px 10px;border:1px solid #d8e0ee;border-radius:12px;background:#fff}.loading-label{font-size:12px;color:#5b6880;margin-bottom:6px;font-weight:700}.loading-track{height:8px;border-radius:999px;background:#eef2f8;overflow:hidden}.loading-bar{height:100%;width:35%;background:linear-gradient(90deg,#ef8d00,#ffd08a);animation:loadmove 1.2s infinite ease-in-out}@keyframes loadmove{0%{margin-left:-35%}100%{margin-left:100%}}.match-box{margin-top:10px;border:1px solid var(--line);border-radius:10px;padding:8px;background:#fffaf2}.match-box.ok{border-color:#49a66a;background:#f4fcf6}.match-box.warn{border-color:#ef8d00;background:#fff7ec}.match-title{font-weight:700;margin-bottom:4px;font-size:13px}.conf-badge{display:inline-block;margin-left:8px;padding:1px 8px;border-radius:999px;font-size:11px;font-weight:800;background:#eef2f8;color:#3b4f6f}.match-grid{display:none;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.match-box:hover .match-grid,.match-box:focus-within .match-grid{display:grid}.match-item{font-size:12px;color:#30425f}.match-item b{display:block;color:#6e7a90;font-size:11px;margin-bottom:2px}.mono{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;word-break:break-word}.pilot-box{margin-top:12px;border:1px solid #cfd8e8;border-radius:18px;padding:16px;background:#f8fbff}.pilot-title{font-size:34px;font-weight:900;margin:4px 0 10px}.pilot-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.pilot-card{background:#fff;border:1px solid #d8e0ee;border-radius:16px;padding:10px 12px;min-height:92px}.pilot-card .t{font-size:13px;color:#4b5d7a;font-weight:800}.pilot-card .v{font-size:34px;font-weight:900;margin-top:6px;line-height:1}.pilot-list{margin-top:12px;border:1px solid #d8e0ee;border-radius:18px;background:#fff;padding:10px}.pilot-row{display:flex;justify-content:space-between;gap:12px;padding:12px;border:1px solid #e6ecf6;border-radius:12px;margin:8px 0;background:linear-gradient(180deg,#ffffff 0%,#f8fbff 100%)}.pilot-row:last-child{border-bottom:1px solid #e6ecf6}.pilot-row .name{font-weight:800}.pilot-row-btn,.planning-action-btn{appearance:none;-webkit-appearance:none;width:100%;text-align:left;background:none;cursor:pointer;color:inherit;font:inherit}.planning-action-btn{padding:0;border:none}.company-cell{display:flex;align-items:center;gap:10px}.company-logo{width:28px;height:28px;border-radius:50%;border:1px solid #d8e0ee;background:#fff;object-fit:cover;display:inline-flex;align-items:center;justify-content:center;font-size:11px;color:#6e7a90;font-weight:800}.proj-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.progress-card{border:1px solid #d8e0ee;border-radius:14px;padding:12px;background:#fff}.progress-title{font-weight:800;margin-bottom:8px}.progress-row{display:grid;grid-template-columns:180px 1fr 70px;gap:10px;align-items:center;padding:6px 0}.progress-track{height:10px;border-radius:999px;background:#edf1f8;overflow:hidden}.progress-fill{height:100%;background:#1b6ef3}.curve-wrap{border:1px solid #d8e0ee;border-radius:14px;padding:12px;background:#fff}.project-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px}.project-cover{width:220px;height:132px;border-radius:14px;border:1px solid #d8e0ee;object-fit:cover;background:#fff;box-shadow:0 10px 24px rgba(16,32,56,.12)}.project-logo{width:88px;height:88px;border-radius:16px;background:#fff;border:1px solid var(--line);padding:14px;box-shadow:0 8px 20px rgba(18,32,51,.08);object-fit:contain}.project-title{font-size:30px;font-weight:900;margin:4px 0 2px}.project-desc{color:#6e7a90;margin:2px 0 0}.project-actions{display:flex;align-items:center;gap:12px;flex-wrap:wrap}.timeline-box{margin:10px 0;border:1px solid #d8e0ee;border-radius:14px;background:#fff;padding:10px}.timeline-row{display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px}.timeline-track{height:12px;border-radius:999px;background:#edf1f8;overflow:hidden}.timeline-fill{height:100%;background:#1b6ef3}.timeline-fill.over{background:#d64545}.chart-controls{display:flex;gap:8px;align-items:center;margin-bottom:8px;flex-wrap:wrap}.chart-select{height:36px;border:1px solid var(--line);border-radius:10px;padding:0 10px}.react-row{display:flex;justify-content:space-between;gap:12px;padding:8px 0;border-bottom:1px dashed #d8e0ee}.react-row:last-child{border-bottom:none}.pm-modal{position:fixed;inset:0;background:rgba(11,20,33,.45);display:flex;align-items:center;justify-content:center;padding:20px;z-index:50}.pm-modal-box{background:#fff;border-radius:14px;max-width:1100px;width:100%;max-height:82vh;overflow:auto;padding:14px;border:1px solid #d8e0ee}.pm-modal-box.wide{max-width:min(1520px,96vw)}.reminder-modal-table th:last-child,.reminder-modal-table td:last-child{white-space:nowrap;min-width:126px}.reminder-modal-table th:nth-child(2),.reminder-modal-table td:nth-child(2){white-space:nowrap;min-width:120px}.detail-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-top:16px}.detail-card{border:1px solid #d8e0ee;border-radius:14px;padding:14px;background:#f8fbff}.detail-card .label{font-size:12px;color:#5b6880;text-transform:uppercase;font-weight:800}.detail-card .value{margin-top:8px;font-size:18px;font-weight:800}.detail-main{border:1px solid #d8e0ee;border-radius:16px;padding:16px;background:#fff;margin-top:12px}.detail-badge{display:inline-flex;padding:6px 10px;border-radius:999px;background:#fff0e2;color:#9a5a00;font-weight:900;font-size:12px;text-transform:uppercase}.timeline-macro{display:grid;gap:10px}.timeline-macro-scroll{max-height:760px;overflow:auto;display:grid;gap:10px;padding-right:4px}.timeline-help{font-size:12px;color:#5f6f88;margin:2px 0 8px}.timeline-macro-marker.today{background:#1f3f69;box-shadow:0 0 0 2px rgba(31,63,105,.14)}.react-stick-btn{appearance:none;border:none;background:none;padding:0;cursor:pointer;color:inherit;font:inherit}.board-table.lined th,.board-table.lined td{border-top:1px solid #e6ecf6}.board-table.grid th,.board-table.grid td{border-top:1px solid #dfe6f1;border-left:1px solid #e3eaf5}.board-table.fixed{table-layout:fixed}.board-table.fixed th,.board-table.fixed td{min-width:96px}.board-table.grid tr>*:first-child{border-left:none}.cell-pre{color:#ef8d00;font-weight:800}.cell-fac{color:#239b5b;font-weight:800}.prod-modal-chart{width:100%;height:220px;border:1px solid #e6edf7;border-radius:14px;background:#fff;display:block}.prod-modal-table{margin-top:12px}.timeline-macro-row{border:1px solid #d8e0ee;border-radius:22px;padding:14px 18px;background:#fff}.timeline-macro-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start}.timeline-macro-title{font-size:18px;font-weight:900}.timeline-macro-sub{margin-top:4px;font-size:11px;color:#6f7f97;text-transform:uppercase}.timeline-macro-date{font-size:16px;font-weight:900;white-space:nowrap}.timeline-macro-track{position:relative;height:28px;border-radius:999px;background:linear-gradient(90deg,#eef3fb,#f7faff);margin-top:14px;overflow:hidden}.timeline-macro-fill{position:absolute;top:0;height:100%;border-radius:999px;background:linear-gradient(90deg,#f5a623,#ef8d00)}.timeline-macro-marker{position:absolute;top:-2px;width:3px;height:32px;background:#788395}.timeline-macro-labels{display:flex;justify-content:space-between;gap:10px;margin-top:10px;font-size:11px;color:#5b6880;font-weight:700}.lvl-row{background:#f7f9fd;font-weight:800}.task-late{background:#fff1f1}.task-soon{background:#fffaf0}.task-cst td:first-child,.task-cst td:nth-child(2){font-weight:900;color:#9a5a00}.status-badge{display:inline-flex;align-items:center;justify-content:center;padding:4px 10px;border-radius:999px;font-weight:900;font-size:12px;min-width:96px}.status-badge.done{color:#1f7a3f;background:#eaf8ef}.status-badge.late{color:#b42318;background:#ffeded;animation:statuspulse 1s infinite}.status-badge.soon{color:#9a6700;background:#fff4e5;animation:statuspulse 1.4s infinite}@keyframes statuspulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.55;transform:scale(1.03)}}
@media (max-width:980px){.top{grid-template-columns:1fr}.menu-row,.project-row,.project-actions{flex-direction:column;align-items:stretch}.grid{grid-template-columns:repeat(2,1fr)}.subgrid,.detail-grid{grid-template-columns:1fr}.pm-modal-box.wide{max-width:100%}}@media (max-width:640px){.grid{grid-template-columns:1fr}}
</style></head>
<body><div class='wrap'>
  <div class='top'>
    ...
    <div class='top-main'>
      <div class='menu-row'>
        ...
        <a id='dashboardBtn' class='btn' href='/dashboard'>Tableau de bord</a>
        <button id='boardWindowBtn' class='btn secondary' type='button'>BOARD</button>
  ...
  <div class='kpis'>
    <div class='project-head'><img id='projectImage' class='project-cover' alt='Projet' src=''><div style='flex:1'><div id='projectTitle' class='project-title'>-</div><div id='projectDesc' class='project-desc'>-</div></div><div class='project-actions'><button id='boardWindowBtnHero' class='btn secondary' type='button'>BOARD</button><a class='logo-link' href='/' aria-label='Retour accueil'><img class='project-logo' src='/assets/logo-tempo' alt='Logo Tempo'></a></div></div>
  ...
  <div id='sectionCompany' class='section'>
    ...
    <div class='chart-controls'>
      ...
      <select id='gpMetronomeProjectPickerAnalysis' class='chart-select'><option value=''>Choisir un projet</option></select>
    ...
    <div class='small' style='font-weight:800;margin:14px 0 8px'>Réactivité par entreprise suivant réunion MÉTRONOME</div>
    <div id='metronomeReactivityChart' class='pilot-list'><div class='small'>Aucune donnée</div></div>
  </div>

  <div id='sectionPlanningAnalysis' class='section' style='display:none'>
    <h3>Niveau 3 — ANALYSE PLANNING</h3>
    <div class='chart-controls'>
      <label for='planningMissionSelect' class='small'>Mission planning :</label>
      <select id='planningMissionSelect' class='chart-select'><option value=''>Aucune mission</option></select>
      <label for='planningChartSelect' class='small'>Afficher :</label>
      <select id='planningChartSelect' class='chart-select'>
        <option value='delay-trigger'>Déclencheur de retard</option>
        <option value='perimeter-progress'>Avancement par périmètre</option>
        <option value='resource-delay'>Retard moyen par ressource</option>
        <option value='resource-delay-today'>Retard moyen par ressource à date</option>
        <option value='planning-progress'>Avancement planning</option><option value='cst-chronology'>Chronologie des tâches CST</option>
      </select>
    </div>
    <div id='planningChartInfo' class='small' style='margin-bottom:10px'>Calculé à partir du planning importé et du pointage saisi.</div>
    <div id='planningChartTitle' class='small' style='font-weight:800;margin-bottom:8px'>Déclencheur de retard</div>
    <div id='planningChartList' class='pilot-list'><div class='small'>Aucune donnée</div></div>
  </div>

  <div class='section'>
    <h3>Pointage planning</h3>
    <div id='missionsPlanningWrap' class='table-wrap' style='margin-bottom:10px'><div class='small' style='padding:10px'>Aucune mission disponible.</div></div>
    <div class='chart-controls'>
  ...
  <div id='perimeterDetailModal' class='pm-modal' style='display:none'>
    <div class='pm-modal-box'>
      <div style='display:flex;justify-content:space-between;align-items:center;gap:12px'>
        <div>
          <h3 id='perimeterDetailTitle' style='margin:0'>Détail périmètre</h3>
          <div id='perimeterDetailSubtitle' class='small' style='margin-top:4px'>Analyse du déclencheur de retard.</div>
        </div>
        <button id='perimeterDetailClose' class='btn'>Fermer</button>
      </div>
      <div id='perimeterDetailBody'></div>
    </div>
  </div>

  <div id='reminderModal' class='pm-modal' style='display:none'>
    <div class='pm-modal-box wide'>
...
<script>
const state={affaires:[],selectedId:'',selectedLabel:'',board:null,pointage:null,metronomeProjectId:'',planningMissionKey:''};
...
function pointageChildrenByParent(tasks){const map={};for(const t of (tasks||[])){const key=String(t.parent_task_id||'').trim();if(!key)continue;(map[key]=map[key]||[]).push(t);}return map;}
function groupTasksByPerimeter(tasks,{includeSummary=false}={}){const groups={};for(const t of (tasks||[])){if(!includeSummary&&t.is_summary)continue;const perimeter=String(t.perimeter||'').trim();const normalized=normalizeText(perimeter);if(!perimeter||normalized==='general')continue;const key=normalized||perimeter;(groups[key]=groups[key]||{label:perimeter,tasks:[]}).tasks.push(t);}return Object.values(groups);}
function taskSequenceTime(task){const primary=parseDateOnly(task.start||'')||parseDateOnly(task.end||'')||parseDateOnly(task.actualEnd||'');return primary?primary.getTime():Number.MAX_SAFE_INTEGER;}
function nextBlockingTaskForPerimeter(tasks){const actionable=(tasks||[]).filter(t=>!t.is_summary).slice().sort((a,b)=>taskSequenceTime(a)-taskSequenceTime(b)||String(a.name||'').localeCompare(String(b.name||''),'fr'));if(!actionable.length)return null;const progressed=actionable.filter(t=>Number(t.progress||0)>0 || !!t.actualEnd);const lastProgressed=progressed.length?progressed[progressed.length-1]:null;const remaining=actionable.filter(t=>Number(t.progress||0)<100 && !t.actualEnd);let nextTask=null;if(lastProgressed){const pivot=actionable.findIndex(t=>String(t.task_id||'')===String(lastProgressed.task_id||''));if(pivot>=0)nextTask=actionable.slice(pivot+1).find(t=>Number(t.progress||0)<100 && !t.actualEnd) || null;}if(!nextTask)nextTask=remaining[0]||null;if(!nextTask)return null;return {name:String(nextTask.name||'').trim()||'Tâche non renseignée',delay:Number(nextTask.delayDays||0),owner:String(nextTask.owner||'').trim()||'Non attribué'};}
function renderReactivityChart(root,items){if(!root)return;if(!items.length){root.innerHTML="<div class='small'>Pas assez de tâches clôturées avec échéance.</div>";return;}const vals=items.map(x=>Math.abs(Number(x.avg_gap_days||0)));const max=Math.max(1,...vals);root.innerHTML=`<div style='display:flex;align-items:flex-end;gap:14px;height:240px;padding:10px 0'>${items.map(x=>{const v=Number(x.avg_gap_days||0);const h=Math.max(8,(Math.abs(v)/max)*190);const color=normalizeText(x.name)==='tempo'?'#ef8d00':(v>0?'#d64545':'#1b6ef3');return `<div style='flex:1;min-width:60px;text-align:center'><div title='${esc(x.name)}: ${v} j' style='margin:0 auto;width:34px;height:${h}px;background:${color};border-radius:8px 8px 0 0'></div><div style='font-size:11px;margin-top:6px;font-weight:700'>${esc(x.name)}</div><div style='font-size:11px;color:#6e7a90'>${v} j</div></div>`;}).join('')}</div>`;}
function firstLateTaskForPerimeter(tasks){const actionable=(tasks||[]).filter(t=>!t.is_summary).slice().sort((a,b)=>taskSequenceTime(a)-taskSequenceTime(b)||String(a.name||'').localeCompare(String(b.name||''),'fr'));const completed=actionable.filter(t=>t.status==='past'||!!t.actualEnd||t.isLate);return completed.map(t=>({...t,delay:Number(t.delayDays||0)})).filter(t=>t.delay>0).sort((a,b)=>{const ad=parseDateOnly(a.actualEnd||'')||parseDateOnly(a.end||'')||parseDateOnly(a.start||'');const bd=parseDateOnly(b.actualEnd||'')||parseDateOnly(b.end||'')||parseDateOnly(b.start||'');const at=ad?ad.getTime():Number.MAX_SAFE_INTEGER;const bt=bd?bd.getTime():Number.MAX_SAFE_INTEGER;return at-bt||a.delay-b.delay||String(a.name||'').localeCompare(String(b.name||''),'fr');})[0]||null;}
function openPerimeterDetail(detail){const modal=document.getElementById('perimeterDetailModal');const title=document.getElementById('perimeterDetailTitle');const subtitle=document.getElementById('perimeterDetailSubtitle');const body=document.getElementById('perimeterDetailBody');if(title)title.textContent=detail.label||'Périmètre';if(subtitle)subtitle.textContent=`Déclencheur de retard pour ${detail.label||'ce périmètre'}.`;if(body){const delay=Number(detail.delayDays||detail.delay||0);const startLabel=fmtDateFrShort(detail.start||'');const endLabel=fmtDateFrShort(detail.end||'');const period=(startLabel||endLabel)?`${startLabel||'?' } → ${endLabel||'?'}`:'Dates non renseignées';body.innerHTML=`<div class='detail-main'><div class='detail-badge'>${delay>0?`${delay} j de retard`:'Point de vigilance'}</div><div style='margin-top:12px;font-size:20px;font-weight:900'>${esc(detail.taskName||detail.task||'Tâche non renseignée')}</div><div class='small' style='margin-top:10px'>${esc(detail.taskDescription||detail.taskName||detail.task||'')}</div></div><div class='detail-grid'><div class='detail-card'><div class='label'>Responsable</div><div class='value'>${esc(String(detail.owner||detail.lot||'').trim()||'Non attribué')}</div></div><div class='detail-card'><div class='label'>Retard</div><div class='value'>${esc(String(delay))} jour(s)</div></div><div class='detail-card'><div class='label'>Fenêtre planning</div><div class='value'>${esc(period)}</div></div></div>`;}if(modal)modal.style.display='flex';}
function hidePerimeterDetailModal(){const modal=document.getElementById('perimeterDetailModal');if(modal)modal.style.display='none';}

function renderDelayTriggerChart(root,tasks){if(!root)return;const items=groupTasksByPerimeter(tasks).map(group=>{const trigger=firstLateTaskForPerimeter(group.tasks||[]);if(!trigger)return null;return {label:group.label,lot:String(trigger.owner||'Non attribué').trim()||'Non attribué',owner:String(trigger.owner||'Non attribué').trim()||'Non attribué',delay:Number(trigger.delayDays||0),delayDays:Number(trigger.delayDays||0),task:trigger.name||'',taskName:trigger.name||'',taskDescription:trigger.name||'',start:trigger.start||'',end:trigger.end||''};}).filter(Boolean).sort((a,b)=>a.label.localeCompare(b.label,'fr'));if(!items.length){root.innerHTML="<div class='small'>Aucun déclencheur de retard détecté sur les tâches terminées.</div>";return;}root.innerHTML=`<div style='display:flex;flex-direction:column;gap:10px'>${items.map(x=>`<button type='button' class='planning-action-btn' data-perimeter='${escAttr(x.label)}'><div class='pilot-row' style='align-items:center'><div style='flex:1;min-width:0'><div style='font-weight:800'>${esc(x.label)}</div><div class='small'>Première tâche en retard : ${esc(x.task||'-')} · ${esc(String(x.delay))} j</div></div><div style='min-width:140px;text-align:right;font-weight:900;color:#d64545'>${esc(x.lot)}</div></div></button>`).join('')}</div>`;Array.from(root.querySelectorAll('[data-perimeter]')).forEach(btn=>btn.addEventListener('click',()=>{const item=items.find(x=>x.label===btn.getAttribute('data-perimeter'));if(item)openPerimeterDetail(item);}));}
function renderPerimeterProgressChart(root,tasks){if(!root)return;const items=groupTasksByPerimeter(tasks).map(group=>{const actionable=(group.tasks||[]).filter(t=>!t.is_summary);if(!actionable.length)return null;const avg=actionable.reduce((sum,t)=>sum+Number(t.progress||0),0)/actionable.length;const blocker=firstLateTaskForPerimeter(actionable)||nextBlockingTaskForPerimeter(actionable);return {label:group.label,value:Math.max(0,Math.min(100,avg)),blocker:blocker?{taskName:blocker.name||'',task:blocker.name||'',taskDescription:blocker.name||'',delayDays:Number(blocker.delayDays||blocker.delay||0),delay:Number(blocker.delayDays||blocker.delay||0),owner:String(blocker.owner||'').trim()||'Non attribué',lot:String(blocker.owner||'').trim()||'Non attribué',start:blocker.start||'',end:blocker.end||''}:null};}).filter(Boolean).sort((a,b)=>b.value-a.value||a.label.localeCompare(b.label,'fr'));if(!items.length){root.innerHTML="<div class='small'>Aucune donnée de périmètre disponible.</div>";return;}root.innerHTML=items.map(x=>{const tooltip=x.blocker?`Cliquer pour voir la tâche déclencheuse ou suivante de ${x.label}`:(x.value>=100?'Périmètre terminé':'Aucune tâche bloquante identifiée');return `<button type='button' class='planning-action-btn' data-progress-perimeter='${escAttr(x.label)}'><div style='margin:10px 0'><div style='display:flex;justify-content:space-between;gap:12px;margin-bottom:6px'><span style='font-weight:700'>${esc(x.label)}</span><strong>${x.value.toFixed(1)}%</strong></div><div title='${escAttr(tooltip)}' style='height:14px;border-radius:999px;background:#edf1f8;overflow:hidden;cursor:pointer'><div style='height:100%;width:${x.value}%;background:linear-gradient(90deg,#1b6ef3,#5da2ff)'></div></div></div></button>`;}).join('');Array.from(root.querySelectorAll('[data-progress-perimeter]')).forEach(btn=>btn.addEventListener('click',()=>{const item=items.find(x=>x.label===btn.getAttribute('data-progress-perimeter'));if(item&&item.blocker)openPerimeterDetail({...item.blocker,label:item.label});}));}
function renderPlanningProgressComparison(root,tasks){if(!root)return;const actionable=(tasks||[]).filter(t=>!t.is_summary);if(!actionable.length){root.innerHTML="<div class='small'>Aucune donnée d'avancement disponible.</div>";return;}const today=new Date();today.setHours(0,0,0,0);const clamp=v=>Math.max(0,Math.min(1,v));const actualRatio=clamp(actionable.reduce((sum,t)=>sum+Math.max(0,Math.min(100,Number(t.progress||0))),0)/(actionable.length*100));const plannedRatio=clamp(actionable.reduce((sum,t)=>{const start=parseDateOnly(t.start||'');const end=parseDateOnly(t.end||'')||start;if(!start&&!end)return sum;const s=start||end;const e=end||start;if(today<=s)return sum;if(today>=e)return sum+1;const duration=Math.max(1,Math.round((e-s)/(1000*60*60*24)));const elapsed=Math.max(0,Math.round((today-s)/(1000*60*60*24)));return sum+clamp(elapsed/duration);},0)/actionable.length);const rows=[{label:'Pointé vs reste à faire',value:actualRatio,color:'#ef8d00',subtitle:`${(actualRatio*100).toFixed(1)}% pointé · ${((1-actualRatio)*100).toFixed(1)}% restant`},{label:'Avancement attendu à date',value:plannedRatio,color:'#1b6ef3',subtitle:'Calculé uniquement à partir des dates du planning'}];root.innerHTML=`<div style='display:grid;gap:14px'>${rows.map(r=>`<div class='progress-card'><div style='display:flex;justify-content:space-between;gap:12px;align-items:flex-start'><div><div class='progress-title'>${esc(r.label)}</div><div class='small'>${esc(r.subtitle)}</div></div><strong style='font-size:24px'>${(r.value*100).toFixed(1)}%</strong></div><div class='progress-track' style='margin-top:10px;height:16px'><div class='progress-fill' style='width:${r.value*100}%;background:${r.color}'></div></div></div>`).join('')}</div>`;}
function renderMetronomeReactivity(){const root=document.getElementById('metronomeReactivityChart');const board=state.board||{};renderReactivityChart(root,(((board||{}).kpis_pilotage)||{}).reactivity_by_company||[]);}
function renderCstChronology(root,tasks){if(!root)return;const rows=(tasks||[]).filter(t=>!t.is_summary&&normalizeText(t.owner||'')==='cst').map(t=>({label:String(t.perimeter||'Sans périmètre').trim()||'Sans périmètre',task:String(t.name||'').trim()||'Tâche CST',start:parseDateOnly(t.start||''),end:parseDateOnly(t.end||'')||parseDateOnly(t.start||''),delay:Number(t.delayDays||0),owner:String(t.owner||'').trim()||'CST'})).filter(x=>x.start||x.end).sort((a,b)=>(a.start||a.end)-(b.start||b.end)||a.label.localeCompare(b.label,'fr'));if(!rows.length){root.innerHTML="<div class='small'>Aucune tâche CST datée dans le planning importé.</div>";return;}const startMin=new Date(Math.min(...rows.map(r=>(r.start||r.end).getTime())));const endMax=new Date(Math.max(...rows.map(r=>(r.end||r.start).getTime())));const total=Math.max(1,Math.round((endMax-startMin)/(1000*60*60*24)));root.innerHTML=`<div class='timeline-macro'>${rows.map(r=>{const s=r.start||r.end;const e=r.end||r.start;const offset=Math.max(0,Math.round((s-startMin)/(1000*60*60*24)));const span=Math.max(1,Math.round((e-s)/(1000*60*60*24))+1);const left=(offset/total)*100;const width=Math.max(2,(span/total)*100);const today=new Date();today.setHours(0,0,0,0);const todayLeft=Math.max(0,Math.min(100,((today-startMin)/(1000*60*60*24))/total*100));return `<div class='timeline-macro-row'><div class='timeline-macro-head'><div><div class='timeline-macro-title'>${esc(r.label)}</div><div class='timeline-macro-sub'>${esc(r.task)} · ${esc(r.owner)}${r.delay>0?` · ${r.delay} j`:''}</div></div><div class='timeline-macro-date'>${esc(fmtDateFrShort(s.toISOString().slice(0,10)))} → ${esc(fmtDateFrShort(e.toISOString().slice(0,10)))}</div></div><div class='timeline-macro-track'><div class='timeline-macro-marker' style='left:${todayLeft}%'></div><div class='timeline-macro-fill' style='left:${left}%;width:${width}%'></div></div><div class='timeline-macro-labels'><span>${esc(fmtDateFrShort(startMin.toISOString().slice(0,10)))}</span><span>${esc(fmtDateFrShort(endMax.toISOString().slice(0,10)))}</span></div></div>`;}).join('')}</div>`;}
function getMissionCatalog(board,pointage){const byKey={};for(const m of ((board&&board.finance_missions)||[])){if(!m||typeof m!=='object')continue;const mk=String(m.mission_key||'').trim();if(!mk)continue;byKey[mk]={mission_key:mk,label:String(m.label||m.mission_key||'Mission').trim()||'Mission',tasks_count:0};}for(const m of ((pointage&&pointage.missions)||[])){if(!m||typeof m!=='object')continue;const mk=String(m.mission_key||'').trim();if(!mk||!byKey[mk])continue;byKey[mk].tasks_count=Math.max(Number(byKey[mk].tasks_count||0),Number(m.tasks_count||0));if(!String(byKey[mk].label||'').trim())byKey[mk].label=String(m.label||mk).trim()||mk;}const items=Object.values(byKey);items.sort((a,b)=>String(a.label||'').localeCompare(String(b.label||''),'fr'));return items;}
function renderPlanningChart(){const sec=document.getElementById('sectionPlanningAnalysis');const root=document.getElementById('planningChartList');const title=document.getElementById('planningChartTitle');const missions=getMissionCatalog(state.board||{},state.pointage||{});const pointage=state.pointage||{};const tasks=(pointage&&pointage.tasks)||[];const hasPlanning=missions.some(m=>Number(m.tasks_count||0)>0)||tasks.length>0;if(!sec||!root)return;if(!hasPlanning){sec.style.display='none';root.innerHTML="<div class='small'>Aucune donnée</div>";return;}sec.style.display='block';if(!tasks.length){root.innerHTML="<div class='small'>Un planning est chargé sur une autre mission. Sélectionnez-la dans Mission planning pour afficher l'analyse.</div>";return;}const mode=((document.getElementById('planningChartSelect')||{}).value)||'delay-trigger';if(mode==='delay-trigger'){if(title)title.textContent='Déclencheur de retard';renderDelayTriggerChart(root,tasks);return;}if(mode==='perimeter-progress'){if(title)title.textContent='Avancement par périmètre';renderPerimeterProgressChart(root,tasks);return;}if(mode==='resource-delay-today'){if(title)title.textContent='Retard moyen par ressource à date';renderPointageAnalytics(root,tasks,{includeUnpointed:true});return;}if(mode==='planning-progress'){if(title)title.textContent='Avancement planning';renderPlanningProgressComparison(root,tasks);return;}if(mode==='cst-chronology'){if(title)title.textContent='Chronologie macro des tâches CST';renderCstChronology(root,tasks);return;}if(title)title.textContent='Retard moyen par ressource';renderPointageAnalytics(root,tasks,{includeUnpointed:false});}
function renderReactivity(b){renderMetronomeReactivity();renderPlanningChart();}
...
function renderPlanningMissionSelect(board){const select=document.getElementById('planningMissionSelect');if(!select)return;const missions=getMissionCatalog(board,state.pointage);const selected=state.planningMissionKey||'';if(!missions.length){select.innerHTML="<option value=''>Aucune mission</option>";select.value='';return;}select.innerHTML=missions.map(m=>`<option value='${escAttr(m.mission_key||'')}' ${(m.mission_key||'')===selected?'selected':''}>${esc(m.label||'Mission')}</option>`).join('');if(!missions.some(m=>String(m.mission_key||'')===selected)){state.planningMissionKey=String(missions[0].mission_key||'');select.value=state.planningMissionKey;}}
function pointageMissionQuery(missionKey){const mk=typeof missionKey==='string'?missionKey:(state.planningMissionKey||'');return mk?`&mission_key=${encodeURIComponent(mk)}`:'';}
function renderMissionsPlanningTable(){const wrap=document.getElementById('missionsPlanningWrap');if(!wrap)return;const missions=getMissionCatalog(state.board||{},state.pointage||{});if(!missions.length){wrap.innerHTML="<div class='small' style='padding:10px'>Aucune mission finance trouvée. Vérifiez les missions dans l'onglet Finance.</div>";return;}wrap.innerHTML=`<table><thead><tr><th>Mission</th><th>Planning chargé</th><th style='width:320px'>Action</th></tr></thead><tbody>${missions.map(m=>{const key=String(m.mission_key||'');const loaded=Number(m.tasks_count||0)>0;const badge=loaded?`<span class='status-badge done'>${Number(m.tasks_count||0)} tâche(s)</span>`:`<span class='status-badge soon'>Aucun planning</span>`;const active=(key===(state.planningMissionKey||''))?" style='font-weight:800'":'';return `<tr${active}><td>${esc(m.label||'Mission')}</td><td>${badge}</td><td style='display:flex;gap:8px;flex-wrap:wrap'><button type='button' class='btn btn-open-mission' data-mission='${escAttr(key)}' style='height:34px'>Ouvrir pointage</button><button type='button' class='btn btn-import-mission' data-mission='${escAttr(key)}' style='height:34px'>Importer planning CSV</button></td></tr>`;}).join('')}</tbody></table>`;Array.from(wrap.querySelectorAll('.btn-open-mission')).forEach(btn=>btn.addEventListener('click',async()=>{state.planningMissionKey=btn.getAttribute('data-mission')||'';const missionSelect=document.getElementById('planningMissionSelect');if(missionSelect)missionSelect.value=state.planningMissionKey;await loadPointage();}));Array.from(wrap.querySelectorAll('.btn-import-mission')).forEach(btn=>btn.addEventListener('click',()=>{const mission=btn.getAttribute('data-mission')||'';const input=document.getElementById('planningCsvInput');if(!input)return;input.dataset.missionKey=mission;input.click();}));}
function renderMetronomeProjectPickers(board){const options=(board&&board.metronome_projects)||[];const selected=String((board&&board.selected_metronome_project_id)||state.metronomeProjectId||'');const html=options.length?options.map(opt=>{const id=String(opt.project_id||'').trim();const label=String(opt.project_title||opt.project_id||'Projet').trim();return `<option value='${esc(id)}' ${id===selected?'selected':''}>${esc(label)}</option>`;}).join(''):"<option value=''>Aucun projet METRONOME</option>";['gpMetronomeProjectPickerPilot','gpMetronomeProjectPickerAnalysis'].forEach(id=>{const el=document.getElementById(id);if(el){el.innerHTML=html;el.value=selected;}});}
...
async function loadBoard(id){const requested=id||state.selectedId||'';if(!requested){state.selectedId='';state.selectedLabel='';state.board=null;renderBoard();return;}const selected=resolveAffaire(requested);const aid=(selected&&selected.affaire_id)||requested;if(String(aid)!==String(state.selectedId||'')){state.metronomeProjectId='';state.planningMissionKey='';}state.selectedId=aid;const sel=document.getElementById('affaireSelect');if(sel)sel.value=aid;state.selectedLabel=(selected&&selected.display_name)||state.selectedLabel||'';localStorage.setItem('selectedAffaireId',aid);showLoading(true,'Chargement de la vue projet…');try{let b;const projectParam=state.metronomeProjectId?`&metronome_project_id=${encodeURIComponent(state.metronomeProjectId)}`:'';try{b=await api(`/api/project-management/board?affaire_id=${encodeURIComponent(aid)}${projectParam}`);}catch(errById){const msg=String((errById&&errById.message)||'');if(state.selectedLabel&&(msg.includes('Affaire introuvable')||msg.includes('affaire_id ou affaire_name requis')||msg.includes('404'))){const alt=String(state.selectedLabel||'').split(' - ').slice(-1)[0]||state.selectedLabel;b=await api(`/api/project-management/board?affaire_name=${encodeURIComponent(alt)}${projectParam}`);}else{throw errById;}}state.board=b;const missions=(b&&b.finance_missions)||[];if(missions.length&&!missions.some(m=>String(m.mission_key||'')===String(state.planningMissionKey||''))){state.planningMissionKey=String((missions[0]&&missions[0].mission_key)||'');}renderBoard();}catch(err){console.error(err);state.board=null;renderBoard();showPageError(err);}finally{showLoading(false);}}
...
async function loadPointage(){if(!state.selectedId)return;try{const missions=getMissionCatalog(state.board||{},state.pointage||{});if(!missions.length){state.pointage={tasks:[],missions:[]};renderPlanningMissionSelect(state.board||{});renderMissionsPlanningTable();renderPointage();renderPlanningChart();return;}if(!state.planningMissionKey||!missions.some(m=>String(m.mission_key||'')===String(state.planningMissionKey||''))){state.planningMissionKey=String((missions[0]&&missions[0].mission_key)||'');}const d=await pointageApi(`/api/project-management/pointage?affaire_id=${encodeURIComponent(state.selectedId)}${pointageMissionQuery(state.planningMissionKey)}`);state.pointage=d;renderPlanningMissionSelect(state.board||{});renderMissionsPlanningTable();renderPointage();renderPlanningChart();}catch(e){console.warn(e);}}
...
async function init(){
  try{
    ...
    document.getElementById('planningCsvInput').addEventListener('change',async e=>{if(e.target.files&&e.target.files[0]){const missionKey=(e.target.dataset&&typeof e.target.dataset.missionKey==='string')?e.target.dataset.missionKey:'';await importPlanningFile(e.target.files[0],missionKey);e.target.value='';e.target.dataset.missionKey='';}});
    ...
    const affaireSelect=document.getElementById('affaireSelect');if(affaireSelect)affaireSelect.addEventListener('change',async e=>{await loadBoard(e.target.value||'');await loadPointage();});
    ...
    const planningMissionSelect=document.getElementById('planningMissionSelect');if(planningMissionSelect)planningMissionSelect.addEventListener('change',async e=>{state.planningMissionKey=e.target.value||'';await loadPointage();});
...
<html lang='fr'>
<head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1'>
<title>METRONOME - Cockpit affaire</title>
<style>
:root{--line:#d9e1ef;--ink:#18273f;--muted:#617089;--accent:#ef8d00;--ok:#239b5b;--warn:#d08a1a;--bad:#cc4747;--panel:#fff;--bg:#f4f7fb;--shadow:0 14px 36px rgba(15,30,54,.08)}
*{box-sizing:border-box}body{margin:0;font-family:Inter,Segoe UI,Arial,sans-serif;background:var(--bg);color:var(--ink)}
.wrap{max-width:1400px;margin:20px auto;padding:0 16px}.card,.top{background:var(--panel);border:1px solid var(--line);border-radius:22px;box-shadow:var(--shadow)}
.top{padding:14px;display:grid;grid-template-columns:auto 1fr;gap:12px;align-items:start}.logo-link{display:inline-flex;text-decoration:none}.brand-mini{display:inline-flex;align-items:center;justify-content:center;width:78px;height:78px;border:1px solid var(--line);border-radius:16px;background:#fff;padding:10px}.brand-mini img{width:100%;height:100%;object-fit:contain}
.top-main{display:flex;flex-direction:column;gap:10px}.menu-row,.project-row{display:flex;gap:10px;flex-wrap:wrap}.search,.select{height:44px;border:1px solid var(--line);border-radius:12px;padding:0 12px;min-width:260px}
...
.header{margin-top:14px;padding:22px}.header-top{display:flex;justify-content:space-between;gap:16px;align-items:flex-start}.eyeb{font-size:12px;font-weight:800;letter-spacing:.13em;color:var(--accent);text-transform:uppercase}.title{font-size:40px;font-weight:900;margin:6px 0}.subtitle{color:var(--muted)}.badge{display:inline-flex;align-items:center;padding:8px 14px;border-radius:999px;font-size:12px;font-weight:800}.badge.ok{background:#e9f8ef;color:var(--ok)}.badge.warn{background:#fff6e8;color:var(--warn)}.badge.bad{background:#ffeded;color:var(--bad)}
.media{display:flex;gap:12px}.hero-media{display:none}.hero-media.show{display:block}.hero-media img{width:250px;max-width:30vw;height:150px;object-fit:cover;border-radius:16px;border:1px solid var(--line)}.hero-logo{width:84px;height:84px;border-radius:14px;border:1px solid var(--line);padding:12px;background:#fff;object-fit:contain}
.health{margin-top:16px;padding:16px;border:1px solid #d2deef;border-radius:18px;background:linear-gradient(180deg,#fff,#f7faff)}.health h3{margin:0 0 12px;font-size:18px}.health-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:10px}.health-item{background:#fff;border:1px solid #dbe5f3;border-radius:12px;padding:10px}.health-item .l{font-size:11px;color:var(--muted);text-transform:uppercase;font-weight:700}.health-item .v{margin-top:6px;font-size:24px;font-weight:900}.signal{margin-top:12px;padding:12px;border-radius:12px;background:#fff;border:1px solid #dbe5f3;font-weight:700}
.kpis{margin-top:14px;padding:14px}.kpi-grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:12px}.k{border:1px solid var(--line);border-radius:14px;padding:14px;background:#fbfdff}.k .l{color:var(--muted);font-size:12px;text-transform:uppercase;font-weight:700}.k .v{font-size:34px;font-weight:900;margin-top:8px}.k .sub{margin-top:6px;color:var(--muted);font-size:12px;font-weight:700}
.charts{margin-top:14px;padding:16px}.chart-wrap{height:350px;border:1px solid #d5deec;border-radius:16px;padding:12px;background:linear-gradient(180deg,#fbfcff,#f4f7fd)}.cum{margin-top:12px;border:1px solid #d5deec;border-radius:16px;padding:12px;background:#fff}.row{display:grid;grid-template-columns:180px 1fr 120px;gap:10px;align-items:center;margin:10px 0}.track{height:11px;background:#edf2f9;border-radius:999px;overflow:hidden}.fill{height:100%}.pre{background:#b7caea}.fac{background:#ef8d00}.prod{background:#239b5b}.axis-note{font-size:12px;color:#60708a;font-weight:700;margin-top:8px;display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap}
@media (max-width:1100px){.health-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.kpi-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media (max-width:740px){.top{grid-template-columns:1fr}.menu-row,.project-row{flex-direction:column}.kpi-grid,.health-grid{grid-template-columns:1fr}.title{font-size:30px}}
</style></head>
<body><div class='wrap'>
  <div class='top'><a class='logo-link' href='/'><div class='brand-mini'><img src='/assets/logo-tempo' alt='Tempo'></div></a><div class='top-main'><div class='menu-row'><a class='btn' href='/'>Accueil</a><a id='financeBtn' class='btn' href='/finance'>Finances</a><a id='pmBtn' class='btn' href='/gestion-projet'>Gestion de projet</a><button id='exportBtn' class='btn' disabled>Exporter CSV</button></div><div class='project-row'><input id='searchInput' class='search' type='search' placeholder='Rechercher une affaire'><select id='affaireSelect' class='select locked' disabled><option value=''>Projet verrouillé</option></select></div></div></div>
  <section class='card header'><div class='header-top'><div><div class='eyeb'>Niveau 1 — Pilotage</div><div id='title' class='title'>Sélectionnez une affaire</div><div id='subtitle' class='subtitle'>Vision synthétique de pilotage.</div><div id='healthBadge' class='badge warn'>Sous surveillance</div></div><div class='media'><div id='projectImageBox' class='hero-media'><img id='projectImage' alt='Projet'></div><img class='hero-logo' src='/assets/logo-tempo' alt='Logo Tempo'></div></div><div class='health'><h3>Santé projet</h3><div class='health-grid'><div class='health-item'><div class='l'>Production %</div><div id='hProd' class='v'>0%</div></div><div class='health-item'><div class='l'>Facturation %</div><div id='hFac' class='v'>0%</div></div><div class='health-item'><div class='l'>Écart fac/prod</div><div id='hGap' class='v'>0 pt</div></div><div class='health-item'><div class='l'>Reste à facturer</div><div id='hReste' class='v'>0€</div></div><div class='health-item'><div class='l'>Alertes</div><div id='hAlerts' class='v'>0</div></div><div class='health-item'><div class='l'>Rappels ouverts</div><div id='hRappels' class='v'>0</div></div><div class='health-item'><div class='l'>Sujets ouverts</div><div id='hSujets' class='v'>0</div></div></div><div id='healthSignal' class='signal'>Sélectionnez une affaire pour afficher le diagnostic.</div></div></section>
  <section class='card kpis'><div class='eyeb'>Niveau 2 — Analyse</div><div class='kpi-grid'><div class='k'><div class='l'>Commande / budget</div><div id='kCommande' class='v'>0 €</div></div><div class='k'><div class='l'>Facturé</div><div id='kFacture' class='v'>0 €</div></div><div class='k'><div class='l'>Reste à facturer</div><div id='kReste' class='v'>0 €</div></div><div class='k'><div class='l'>Avancement financier</div><div id='kAvance' class='v'>0 %</div><div class='sub' id='kAvanceSub'>Facturé / commande</div></div><div class='k'><div class='l'>Avancement planning</div><div id='kPlanningAvance' class='v'>0 %</div><div class='sub' id='kPlanningAvanceSub'>Pointage moyen des tâches</div></div><div class='k'><div class='l'>Marge projetée (10%)</div><div id='kMarge' class='v'>0 €</div></div></div></section>
  <section class='card charts'><div class='eyeb'>Niveau 3 — Trajectoire</div><h3 style='margin:6px 0 10px'>Prévisionnel, facturation et production cumulés</h3><div class='chart-wrap'><svg id='chart' width='100%' height='320' viewBox='0 0 980 320' preserveAspectRatio='xMidYMid meet'></svg></div><div class='axis-note'><span>Axe X : Mois (Janv. → Déc.)</span><span>Axe Y : Montants cumulés (€)</span></div><div class='cum'><div id='cumRows'></div></div></section>
</div>
<script>
const MONTHS=['janvier','fevrier','mars','avril','mai','juin','juillet','aout','septembre','octobre','novembre','decembre'];
const MONTH_LABELS={janvier:'Janv.',fevrier:'Févr.',mars:'Mars',avril:'Avr.',mai:'Mai',juin:'Juin',juillet:'Juil.',aout:'Août',septembre:'Sept.',octobre:'Oct.',novembre:'Nov.',decembre:'Déc.'};
const state={affaires:[],selectedId:'',selected:null,board:null};
...
[['kCommande',cmd],['kFacture',billed],['kReste',reste],['kMarge',cmd*0.10],['hReste',reste]].forEach(x=>document.getElementById(x[0]).textContent=euro(x[1]));document.getElementById('kAvance').textContent=`${finPct.toFixed(1)} %`;document.getElementById('kPlanningAvance').textContent=`${(Number(a.pointage_progress_ratio||0)*100).toFixed(1)} %`;document.getElementById('kAvanceSub').textContent='Facturé / commande';document.getElementById('kPlanningAvanceSub').textContent=`${Number(a.pointage_task_count||0)} tâche(s) prises en compte`;document.getElementById('hProd').textContent=`${prodPct.toFixed(1)}%`;document.getElementById('hFac').textContent=`${finPct.toFixed(1)}%`;document.getElementById('hGap').textContent=`${gap.toFixed(1)} pt`;document.getElementById('hAlerts').textContent=String(alerts);document.getElementById('hRappels').textContent=String(rappels);document.getElementById('hSujets').textContent=String(sujets);renderChart(prodPct);}
...
def dashboard_board_html() -> str:
    return """<!doctype html>
<html lang='fr'>
<head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1'>
<title>BOARD client</title>
<style>
:root{--line:#dfe5ef;--ink:#122033;--muted:#6e7a90;--accent:#ef8d00;--accent-soft:#fff4e6;--panel:#fff;--panel2:#f8fbff;--shadow:0 12px 34px rgba(18,32,51,.07)}
*{box-sizing:border-box}html{-webkit-print-color-adjust:exact;print-color-adjust:exact}body{margin:0;font-family:Inter,Segoe UI,Arial,sans-serif;background:#f3f6fb;color:var(--ink)}
.wrap{max-width:1480px;margin:18px auto;padding:0 14px}.board-shell,.board-section{background:#fff;border:1px solid var(--line);border-radius:22px;box-shadow:var(--shadow)}.board-shell{padding:18px}.board-header{display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap}.board-brand{display:flex;align-items:center;gap:12px;flex-wrap:wrap}.board-kicker{font-size:16px;font-weight:900;letter-spacing:.08em;color:var(--accent);text-transform:uppercase}.board-date-wrap{display:inline-flex;align-items:center;gap:10px;padding:6px 14px;border:1px solid #d4dceb;border-radius:999px;background:var(--accent-soft)}.board-date-label{font-size:11px;font-weight:900;letter-spacing:.08em;color:#7e5b26;text-transform:uppercase}.board-date{height:32px;padding:0 2px;border:none;background:transparent;color:var(--ink);font:inherit;font-size:20px;font-weight:900}.board-date::-webkit-calendar-picker-indicator{cursor:pointer;opacity:.9}.board-actions{display:flex;gap:8px;flex-wrap:wrap}.action-btn{height:40px;padding:0 16px;border:1px solid #1d2b43;border-radius:999px;background:rgba(255,255,255,.94);color:var(--ink);font-size:14px;font-weight:900;letter-spacing:.02em;cursor:pointer;box-shadow:0 6px 18px rgba(18,32,51,.06)}.action-btn:hover{background:#fff}.project-head{margin-top:16px;display:flex;align-items:flex-start;justify-content:space-between;gap:18px}.project-main{display:flex;gap:18px;align-items:flex-start;min-width:0;flex:1}.project-cover{width:360px;max-width:30vw;height:210px;border-radius:22px;border:1px solid #d8e1ef;object-fit:cover;background:#fff;box-shadow:0 10px 24px rgba(16,32,56,.12)}.project-copy{min-width:0;flex:1}.project-title{margin:0;font-size:56px;line-height:.98;font-weight:900;letter-spacing:-.03em}.project-desc{margin-top:12px;font-size:20px;line-height:1.24;color:#697991}.project-logo{width:80px;height:80px;border-radius:18px;border:1px solid var(--line);padding:12px;background:#fff;object-fit:contain;flex:0 0 auto}.section-label{margin:18px 0 8px;font-size:15px;font-weight:900;color:var(--accent);text-transform:uppercase;letter-spacing:.02em}.pill-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.pill-grid.two{grid-template-columns:repeat(2,minmax(0,1fr))}.pill{border:2px solid #1d2b43;border-radius:20px;padding:11px 15px;background:#fff;min-height:70px}.pill .label{font-size:12px;font-weight:900;text-transform:uppercase}.pill .value{margin-top:6px;font-size:20px;font-weight:900}.composer{margin-top:18px;display:flex;justify-content:space-between;gap:16px;align-items:center;flex-wrap:wrap}.composer-title{font-size:16px;font-weight:900;color:var(--accent);text-transform:uppercase}.toggle-list{display:flex;gap:8px;flex-wrap:wrap}.toggle-chip{display:inline-flex;align-items:center;gap:7px;border:1px solid #1d2b43;border-radius:999px;padding:5px 12px;background:#fff;font-size:13px;font-weight:900}.toggle-chip input{width:16px;height:16px;margin:0}.board-sections{display:grid;gap:12px}.board-section{margin-top:0;padding:12px}.section-head{display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:10px}.section-head h3{margin:0;font-size:24px;font-weight:900}.notes-btn{height:30px;padding:0 12px;border:1px solid #1d2b43;border-radius:999px;background:#fff;font-size:12px;font-weight:900;cursor:pointer}.section-tools{display:flex;gap:6px;flex-wrap:wrap;align-items:center}.mini-toggle{display:inline-flex;align-items:center;gap:6px;border:1px solid #1d2b43;border-radius:999px;padding:4px 10px;background:#fff;font-size:11px;font-weight:800}.mini-toggle input{width:13px;height:13px;margin:0}.panel{border:1px solid #d8e1ef;border-radius:18px;background:var(--panel2);padding:12px}.mini-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.mini-card{background:#fff;border:1px solid #dbe4f2;border-radius:16px;padding:12px;min-height:92px}.mini-card .label{font-size:12px;color:#4b5d7a;font-weight:800}.mini-card .value{margin-top:6px;font-size:31px;font-weight:900;line-height:1}.stack{display:grid;gap:10px}.company-row{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:11px 14px;border:1px solid #e6ecf6;border-radius:16px;background:#fff}.company-row .name{font-size:15px;font-weight:900}.company-row .count{font-size:15px}.company-row-btn{appearance:none;-webkit-appearance:none;width:100%;text-align:left;background:none;border:none;padding:0;cursor:pointer;font:inherit;color:inherit}.company-cell{display:flex;align-items:center;gap:10px}.company-logo{width:30px;height:30px;border-radius:50%;border:1px solid #d8e0ee;background:#fff;object-fit:cover;display:inline-flex;align-items:center;justify-content:center;font-size:11px;color:#6e7a90;font-weight:800}.chart-box{border:1px solid #d8e1ef;border-radius:18px;background:#fff;padding:12px}.chart-title{font-size:14px;font-weight:900;margin:0 0 8px}.bar-row{display:grid;grid-template-columns:minmax(200px,280px) 1fr 56px;gap:12px;align-items:center;margin:8px 0}.bar-row .label{font-size:14px;font-weight:900}.bar-track{height:12px;border-radius:999px;background:#edf2f8;overflow:hidden}.bar-fill{height:100%;background:linear-gradient(90deg,#ef8d00,#ffbc62)}.reactivity-bars{display:flex;align-items:flex-end;gap:14px;height:220px;padding:6px 6px 0}.react-col{flex:1;min-width:68px;text-align:center}.react-stick{margin:0 auto;width:38px;border-radius:12px 12px 0 0;background:#d64545}.react-stick.tempo{background:#ef8d00}.react-name{font-size:11px;font-weight:900;margin-top:8px}.react-val{font-size:11px;color:#6e7a90}.planning-card{border:1px solid #d8e1ef;border-radius:16px;background:#fff;padding:12px}.planning-card .head{display:flex;justify-content:space-between;gap:10px;align-items:flex-start}.planning-card .title{font-size:16px;font-weight:900;margin:0}.planning-card .sub{margin-top:3px;font-size:13px;color:#6f7f97}.planning-card .side{min-width:130px;text-align:right;font-size:14px;font-weight:900}.planning-stack{display:grid;gap:10px}.progress-block{display:grid;gap:10px}.progress-row{background:#fff;border:1px solid #d8e1ef;border-radius:16px;padding:12px}.progress-row .meta{display:flex;justify-content:space-between;gap:10px;align-items:flex-start}.progress-row .name{font-size:15px;font-weight:900}.progress-track{height:12px;border-radius:999px;background:#edf2f8;overflow:hidden;margin-top:8px}.progress-fill{height:100%;background:linear-gradient(90deg,#1b6ef3,#5da2ff)}.timeline-macro{display:grid;gap:10px}.timeline-macro-scroll{max-height:760px;overflow:auto;display:grid;gap:10px;padding-right:4px}.timeline-help{font-size:12px;color:#5f6f88;margin:2px 0 8px}.timeline-macro-marker.today{background:#1f3f69;box-shadow:0 0 0 2px rgba(31,63,105,.14)}.react-stick-btn{appearance:none;border:none;background:none;padding:0;cursor:pointer;color:inherit;font:inherit}.board-table.lined th,.board-table.lined td{border-top:1px solid #e6ecf6}.board-table.grid th,.board-table.grid td{border-top:1px solid #dfe6f1;border-left:1px solid #e3eaf5}.board-table.fixed{table-layout:fixed}.board-table.fixed th,.board-table.fixed td{min-width:96px}.board-table.grid tr>*:first-child{border-left:none}.cell-pre{color:#ef8d00;font-weight:800}.cell-fac{color:#239b5b;font-weight:800}.prod-modal-chart{width:100%;height:220px;border:1px solid #e6edf7;border-radius:14px;background:#fff;display:block}.prod-modal-table{margin-top:12px}.timeline-macro-row{border:1px solid #d8e0ee;border-radius:18px;padding:14px;background:#fff}.timeline-macro-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.timeline-macro-title{font-size:16px;font-weight:900}.timeline-macro-sub{margin-top:4px;font-size:13px;color:#6f7f97;text-transform:none}.timeline-macro-date{font-size:16px;font-weight:900;white-space:nowrap}.timeline-macro-track{position:relative;height:12px;border-radius:999px;background:linear-gradient(90deg,#eef3fb,#f7faff);margin-top:12px;overflow:hidden}.timeline-macro-fill{position:absolute;top:0;height:100%;border-radius:999px;background:linear-gradient(90deg,#f5a623,#ef8d00)}.timeline-macro-marker{position:absolute;top:-3px;width:3px;height:18px;background:#788395}.timeline-macro-labels{display:flex;justify-content:space-between;gap:10px;margin-top:8px;font-size:12px;color:#5b6880;font-weight:700}.finance-svg{width:100%;height:290px;display:block;border:1px solid #edf2f8;border-radius:18px;background:#fff}.legend{display:flex;gap:14px;flex-wrap:wrap;align-items:center;font-size:12px;font-weight:800;margin-bottom:8px}.legend span::before{content:'';display:inline-block;width:16px;height:4px;border-radius:999px;margin-right:6px;vertical-align:middle}.legend .pre::before{background:#9db4de}.legend .fac::before{background:#ef8d00}.legend .prod::before{background:#239b5b}.finance-prod-hitbox{cursor:pointer}.finance-prod-hitbox:hover{fill:rgba(35,155,91,.12)}.finance-prod-note{margin-top:10px;padding:12px 14px;border:1px solid #dbe7df;border-radius:14px;background:#f5fbf7;font-size:13px;color:#345846}.finance-prod-steps{display:grid;gap:10px;margin-top:12px}.finance-prod-step{border:1px solid #dbe4f2;border-radius:14px;padding:12px;background:#fff}.finance-prod-step .label{font-size:11px;font-weight:900;text-transform:uppercase;color:#617089}.finance-prod-step .value{margin-top:4px;font-size:20px;font-weight:900}.summary-row{display:grid;grid-template-columns:220px 1fr 120px;gap:12px;align-items:center;margin-top:8px}.summary-track{height:12px;border-radius:999px;background:#eaf0f8;overflow:hidden}.summary-fill.pre{background:#9db4de;height:100%}.summary-fill.fac{background:#ef8d00;height:100%}.summary-fill.prod{background:#239b5b;height:100%}.board-table{width:100%;border-collapse:collapse}.board-table th,.board-table td{padding:10px 12px;border-bottom:1px solid #e6ecf6;text-align:left;font-size:13px}.board-table th{font-size:11px;text-transform:uppercase;color:#617089}.board-table td.num{text-align:right}.notes-wrap{display:none;margin-top:10px}.notes-wrap.open{display:block}.notes-area{width:100%;min-height:92px;border:1px dashed #cfd8e8;border-radius:14px;padding:12px;font:inherit;font-size:13px;background:#fff;resize:vertical}.notes-help{margin-top:6px;font-size:11px;color:var(--muted)}.pm-modal{position:fixed;inset:0;background:rgba(11,20,33,.45);display:flex;align-items:center;justify-content:center;padding:20px;z-index:50}.pm-modal-box{background:#fff;border-radius:18px;max-width:min(1480px,96vw);width:100%;max-height:86vh;overflow:auto;padding:18px;border:1px solid #d8e0ee}.reminder-group{border:1px solid #dbe4f2;border-radius:18px;overflow:hidden;background:#fff;margin-top:10px}.reminder-group-title{padding:12px 16px;background:#fbfcff;font-size:14px;font-weight:900}.reminder-table{width:100%;border-collapse:collapse}.reminder-table th,.reminder-table td{padding:12px 14px;border-top:1px solid #e7edf7;font-size:13px;text-align:left;vertical-align:top}.reminder-table th{font-size:11px;color:#60708c;text-transform:uppercase;background:#f5f8fd}.reminder-level{font-weight:900;color:#d64545;white-space:nowrap}.print-only{display:none !important}.hidden{display:none !important}
@media (max-width:1100px){.project-head,.project-main,.composer,.section-head{flex-direction:column;align-items:stretch}.pill-grid,.pill-grid.two,.mini-grid{grid-template-columns:1fr}.project-cover{max-width:100%;width:100%;height:220px}.project-title{font-size:42px}.finance-svg{height:240px}.summary-row,.bar-row{grid-template-columns:1fr}.board-date-wrap{width:100%;justify-content:space-between}}@page{size:A3 landscape;margin:8mm}@media print{html,body{width:100%;height:100%;background:#fff;font-size:8.6pt;overflow:hidden}.wrap{max-width:none;margin:0;padding:0}.board-shell,.board-section,.chart-box,.panel,.planning-card,.progress-row,.timeline-macro-row,.mini-card,.pill{box-shadow:none !important;background:#fff !important}.board-shell{padding:0;border:none;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:4mm;align-items:start}.board-header,.project-head,.section-label,.pill-grid,.composer,.board-sections{grid-column:1 / -1}.board-header{padding-bottom:1mm;border-bottom:1px solid #d8e1ef}.project-head{margin-top:0;gap:4mm;align-items:center}.project-main{gap:4mm}.project-cover{width:66mm;height:40mm;max-width:none;box-shadow:none}.project-title{font-size:22pt}.project-desc{margin-top:3mm;font-size:9pt;line-height:1.2;max-height:24mm;overflow:hidden}.project-logo{display:none !important}.section-label{margin:0 0 1mm;font-size:10pt}.pill-grid{gap:3mm}.pill-grid.two{grid-template-columns:repeat(2,minmax(0,1fr))}.pill{min-height:auto;padding:2.2mm 3mm;border-radius:12px}.pill .label{font-size:7pt}.pill .value{margin-top:1mm;font-size:11pt}.composer,.board-actions,.section-tools,.notes-btn,.toggle-list,.pm-modal,.screen-only{display:none !important}.board-sections{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:4mm}.board-section{margin-top:0;padding:3mm;border:1px solid #d8e1ef;border-radius:14px;break-inside:avoid-page;min-height:0}.board-section:last-child{grid-column:1 / -1}.section-head{margin-bottom:2mm}.section-head h3{font-size:11pt}.panel,.chart-box,.planning-card,.progress-row,.timeline-macro-row,.mini-card{padding:2.5mm;border-radius:12px}.chart-box{break-inside:avoid-page}.chart-box[style]{margin-top:3mm !important}.mini-grid{gap:2mm}.mini-card{min-height:auto}.mini-card .label,.chart-title,.notes-help,.timeline-macro-sub,.react-val,.board-table th,.reminder-table th{font-size:6.7pt}.mini-card .value{margin-top:1mm;font-size:14pt}.stack,.planning-stack,.progress-block,.timeline-macro{gap:2mm}.company-row{padding:2mm 2.5mm;border-radius:10px}.company-row .name,.company-row .count,.bar-row .label,.planning-card .title,.progress-row .name,.timeline-macro-title{font-size:7.6pt}.company-logo{width:18px;height:18px;font-size:7pt}.bar-row{grid-template-columns:minmax(0,118px) 1fr 42px;gap:2mm;margin:1.5mm 0}.bar-track,.progress-track,.summary-track{height:6px;border:1px solid #cbd6e8;background:#eef3fb !important}.bar-fill,.progress-fill,.summary-fill.pre,.summary-fill.fac,.summary-fill.prod,.react-stick,.timeline-macro-fill{background-image:none !important}.bar-fill,.summary-fill.fac{background:#ef8d00 !important}.progress-fill{background:#3f7ee8 !important}.summary-fill.pre{background:#9db4de !important}.summary-fill.prod{background:#239b5b !important}.print-only{display:block !important}.finance-svg,.reactivity-bars,.timeline-macro-track{display:none !important}.summary-row{grid-template-columns:100px 1fr 70px;gap:2mm;margin-top:1.5mm;font-size:7pt}.timeline-macro-head{gap:4mm}.timeline-macro-date{font-size:7pt}.timeline-macro-labels{margin-top:1mm;font-size:6.5pt}.board-date-wrap{border:1px solid #d4dceb;background:#fff7ed;padding:1.5mm 3mm}.board-date-label{font-size:6.5pt}.board-date{font-size:10pt;height:auto}.board-table,.reminder-table{table-layout:fixed}.board-table th,.board-table td,.reminder-table th,.reminder-table td{padding:1.3mm 1.6mm;font-size:6.4pt;line-height:1.15;word-break:break-word}.notes-wrap{display:block !important;margin-top:2mm}.notes-area{min-height:14mm;padding:2mm;border:1px solid #d8e1ef;font-size:6.8pt;resize:none}.reminder-group{break-inside:avoid-page}.legend{margin-bottom:2mm;font-size:6.7pt;gap:3mm}::-webkit-scrollbar{display:none}}
</style></head>
<body><div class='wrap'><div class='board-shell'>
  <div class='board-header'>
    <div class='board-brand'><div class='board-kicker'>BOARD CLIENT</div><label class='board-date-wrap'><span class='board-date-label'>Date du board</span><input id='boardDate' class='board-date' type='date'></label></div>
    <div class='board-actions'>
      <button id='exportBtn' class='action-btn'>EXPORT</button>
      <button id='importBtn' class='action-btn'>IMPORT</button>
      <button id='printBtn' class='action-btn'>IMPRIMER</button>
      <button id='closeBtn' class='action-btn'>FERMER</button>
  ...
  <div class='project-head'>
    <div class='project-main'>
      <img id='projectImage' class='project-cover' alt='Projet'>
      <div class='project-copy'><h1 id='projectTitle' class='project-title'>VALHUBERT</h1><div id='projectDesc' class='project-desc'>Réhabilitation d'un immeuble de bureaux à destination d'Essilor Luxottica</div></div>
    </div>
    <img class='project-logo' src='/assets/logo-tempo' alt='Tempo'>
  </div>

  <div class='section-label'>Commande</div>
  <div class='pill-grid'>
    <div class='pill'><div class='label'>Commande HT</div><div id='pillBudget' class='value'>0 €</div></div>
    <div class='pill'><div class='label'>Reste à facturer</div><div id='pillRemain' class='value'>0 €</div></div>
    <div class='pill'><div class='label'>% Facturé</div><div id='pillBilledPct' class='value'>0 %</div></div>
  </div>

  <div class='section-label'>Planning</div>
  <div class='pill-grid two'>
    <div class='pill'><div class='label'>Avancement % planning attendu</div><div id='pillPlannedPct' class='value'>0 %</div></div>
    <div class='pill'><div class='label'>Avancement % planning pointé</div><div id='pillPointedPct' class='value'>0 %</div></div>
  </div>

  <div class='composer'>
    <div class='composer-title'>Gestion de projet</div>
    <div class='toggle-list'>
      <label class='toggle-chip'><input type='checkbox' data-section='metronome' checked>METRO</label>
      <label class='toggle-chip'><input type='checkbox' data-section='analysis' checked>ANALYSE</label>
      <label class='toggle-chip'><input type='checkbox' data-section='planning' checked>PLANNING</label>
      <label class='toggle-chip'><input type='checkbox' data-section='finance' checked>FINANCE</label>
      <label class='toggle-chip'><input type='checkbox' data-section='notes' checked>NOTES</label>
    </div>
  </div>

  <div class='board-sections'>
  <section id='section-metronome' class='board-section'>
    <div class='section-head'><h3>GESTION DE PROJET | METRONOME</h3><div class='section-tools'><label for='metronomeProjectPickerPilot' class='small'>Projet METRONOME</label><select id='metronomeProjectPickerPilot' class='action-btn' style='height:32px;padding:0 12px'><option value=''>Choisir un projet</option></select><button class='notes-btn' data-notes-target='notes-metronome'>+ Notes</button></div></div>
    <div class='panel'>
      <div class='mini-grid'>
    ...
    <div id='notes-metronome' class='notes-wrap'><textarea class='notes-area' data-notes='metronome' placeholder='Notes de réunion pour la section METRONOME…'></textarea><div class='notes-help'>Ces notes sont exportables et imprimables.</div></div>
  </section>

  <section id='section-analysis' class='board-section'>
    <div class='section-head'><h3>GESTION DE PROJET | ANALYSE METRONOME</h3><div class='section-tools'><label for='metronomeProjectPicker' class='small'>Projet METRONOME</label><select id='metronomeProjectPicker' class='action-btn' style='height:32px;padding:0 12px'><option value=''>Choisir un projet</option></select><label class='mini-toggle'><input type='checkbox' data-chart='analysis-company' checked>Entreprises</label><label class='mini-toggle'><input type='checkbox' data-chart='analysis-reactivity' checked>Réactivité</label><button class='notes-btn' data-notes-target='notes-analysis'>+ Notes</button></div></div>
    ...
    <div id='chart-analysis-reactivity' class='chart-box' style='margin-top:14px'>
      <div class='chart-title'>Réactivité par entreprise suivant réunion METRONOME</div>
      <div id='analysisReactivity' class='reactivity-bars'></div><div id='analysisReactivityPrint' class='print-only'></div>
    </div>
    <div id='notes-analysis' class='notes-wrap'><textarea class='notes-area' data-notes='analysis' placeholder='Notes de réunion pour la section Analyse METRONOME…'></textarea><div class='notes-help'>Ces notes sont exportables et imprimables.</div></div>
  </section>

  <section id='section-planning' class='board-section'>
    <div class='section-head'><h3>GESTION DE PROJET | PLANNING</h3><div class='section-tools'><label for='planningMissionPickerBoard' class='small'>Mission planning</label><select id='planningMissionPickerBoard' class='action-btn' style='height:32px;padding:0 12px'><option value=''>Aucune mission</option></select><label class='mini-toggle'><input type='checkbox' data-chart='planning-delay' checked>Déclencheur</label><label class='mini-toggle'><input type='checkbox' data-chart='planning-progress' checked>Périmètres</label><label class='mini-toggle'><input type='checkbox' data-chart='planning-resources' checked>Ressources</label><label class='mini-toggle'><input type='checkbox' data-chart='planning-resources-today' checked>Ressources à date</label><label class='mini-toggle'><input type='checkbox' data-chart='planning-expected' checked>Avancement</label><label class='mini-toggle'><input type='checkbox' data-chart='planning-cst' checked>CST</label><button class='notes-btn' data-notes-target='notes-planning'>+ Notes</button></div></div>
    <div id='chart-planning-delay' class='chart-box'><div class='chart-title'>Déclencheur de retard</div><div id='planningDelay' class='planning-stack'></div></div>
    <div id='chart-planning-progress' class='chart-box' style='margin-top:14px'><div class='chart-title'>Avancement par périmètre</div><div id='planningProgress' class='progress-block'></div></div>
    <div id='chart-planning-resources' class='chart-box' style='margin-top:14px'><div class='chart-title'>Retard moyen par ressource, suivant planning pointé</div><div id='planningResources' class='reactivity-bars'></div><div id='planningResourcesPrint' class='print-only'></div></div>
    <div id='chart-planning-resources-today' class='chart-box' style='margin-top:14px'><div class='chart-title'>Retard moyen par ressource à date</div><div id='planningResourcesToday' class='reactivity-bars'></div><div id='planningResourcesTodayPrint' class='print-only'></div></div>
    <div id='chart-planning-expected' class='chart-box' style='margin-top:14px'><div class='chart-title'>Avancement planning</div><div id='planningExpected' class='progress-block'></div></div>
    <div id='chart-planning-cst' class='chart-box' style='margin-top:14px'><div style='display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap'><div class='chart-title' style='margin:0'>Chronologie des périmètres</div><label class='mini-toggle screen-only'><input id='planningTimelineActualToggle' type='checkbox'>Afficher le pointé</label></div><div id='planningCst' class='timeline-macro' style='margin-top:10px'></div><div id='planningCstPrint' class='print-only'></div></div>
    <div id='notes-planning' class='notes-wrap'><textarea class='notes-area' data-notes='planning' placeholder='Notes de réunion pour la section Planning…'></textarea><div class='notes-help'>Ces notes sont exportables et imprimables.</div></div>
  </section>

  <section id='section-finance' class='board-section'>
    <div class='section-head'><h3>GESTION FINANCIÈRE</h3><div class='section-tools'><label class='mini-toggle'><input type='checkbox' data-chart='finance-curve' checked>Courbe</label><label class='mini-toggle'><input type='checkbox' data-chart='finance-missions' checked>Missions</label><button class='notes-btn' data-notes-target='notes-finance'>+ Notes</button></div></div>
    <div id='chart-finance-curve' class='chart-box'><div class='legend'><span class='pre'>Prévisionnel</span><span class='fac'>Facturation</span><span class='prod'>Production</span></div><div class='section-tools screen-only' style='margin-bottom:8px'><label class='mini-toggle'><input id='financeModeMonthly' name='financeMode' type='radio' checked>Mensuel facturé</label><label class='mini-toggle'><input id='financeModeCumulative' name='financeMode' type='radio'>Cumulé facturé</label><label class='mini-toggle'><input id='financeShowProduction' type='checkbox' checked>Courbe production</label></div><div id='financeMissionFilters' class='toggle-list screen-only' style='margin-bottom:10px'></div><div style='max-height:260px;overflow:auto;border:1px solid #edf2f8;border-radius:18px;padding:6px;background:#fff'><svg id='financeChart' class='finance-svg' viewBox='0 0 980 320' preserveAspectRatio='xMidYMid meet' style='height:220px'></svg></div><div id='financeMissionMonthlyDetail' style='margin-top:10px;overflow-x:auto;overflow-y:visible'></div><div id='financeSummary'></div><div id='financePrintSummary' class='print-only'></div></div>
    <div id='financeProductionModal' class='pm-modal hidden'><div class='pm-modal-box'><div style='display:flex;justify-content:space-between;align-items:center;gap:12px'><h3 id='financeProductionTitle' style='margin:0'>Courbe production</h3><button id='financeProductionClose' class='action-btn' style='height:38px;background:#239b5b;color:#fff;border-color:#239b5b'>Fermer</button></div><div id='financeProductionBody' style='margin-top:14px'></div></div></div>
    <div id='planningDelayModal' class='pm-modal hidden'><div class='pm-modal-box'><div style='display:flex;justify-content:space-between;align-items:center;gap:12px'><h3 id='planningDelayModalTitle' style='margin:0'>Retard moyen par ressource</h3><button id='planningDelayModalClose' class='action-btn' style='height:38px;background:#d64545;color:#fff;border-color:#d64545'>Fermer</button></div><div id='planningDelayModalBody' style='margin-top:14px'></div></div></div>
    <div id='chart-finance-missions' class='chart-box' style='margin-top:14px'><div class='chart-title'>Niveau 3 — Détail des missions</div><div style='overflow:auto'><table class='board-table grid'><thead><tr><th>Tag</th><th>Mission</th><th>N°</th><th>Commande</th><th>Facturation totale</th><th>Reste</th><th>Prévisionnel</th><th>Facturé</th></tr></thead><tbody id='financeMissions'></tbody></table></div></div>
    <div id='notes-finance' class='notes-wrap'><textarea class='notes-area' data-notes='finance' placeholder='Notes de réunion pour la section Finance…'></textarea><div class='notes-help'>Ces notes sont exportables et imprimables.</div></div>
  </section>

  <section id='section-notes' class='board-section'>
    <div class='section-head'><h3>NOTES GÉNÉRALES</h3></div>
    <textarea class='notes-area' data-notes='general' placeholder='Messages clés, arbitrages, demandes client, décisions…'></textarea>
  </section>
  </div>
</div></div>
<script>
const MONTHS=['janvier','fevrier','mars','avril','mai','juin','juillet','aout','septembre','octobre','novembre','decembre'];
const MONTH_LABELS={janvier:'Janv.',fevrier:'Févr.',mars:'Mars',avril:'Avr.',mai:'Mai',juin:'Juin',juillet:'Juil.',aout:'Août',septembre:'Sept.',octobre:'Oct.',novembre:'Nov.',decembre:'Déc.'};
const FINANCE_PRE_COLORS=['#cfdcf6','#b9cbed','#a4bae4','#8faada','#7a99d1','#6488c7'];
const FINANCE_FAC_COLORS=['#ffd29a','#ffbf73','#ffab4d','#ff9826','#f58a00','#db7b00'];
const qs=new URLSearchParams(window.location.search);const affaireId=qs.get('affaire_id')||localStorage.getItem('selectedAffaireId')||'';
const euro=v=>new Intl.NumberFormat('fr-FR',{style:'currency',currency:'EUR',maximumFractionDigits:0}).format(Number(v||0));
...
function initials(v){const parts=String(v||'').trim().split(/\s+/).slice(0,2);return parts.map(x=>x[0]||'').join('').toUpperCase()||'?';}
function companyLogoHtml(item){const name=item.name||item.label||'';const logo=item.logo||'';if(logo)return `<img class='company-logo' src='${esc(logo)}' alt='${esc(name)}'>`;return `<span class='company-logo'>${esc(initials(name))}</span>`;}
function projectStorageKey(suffix){return `${suffix}::${affaireId}`;}
function restoreLayout(){let data={};try{data=JSON.parse(localStorage.getItem(projectStorageKey('board-layout'))||'{}')||{};}catch(_){data={};}document.querySelectorAll('[data-section],[data-chart]').forEach(inp=>{const key=inp.getAttribute('data-section')||inp.getAttribute('data-chart');inp.checked=data[key]!==false;inp.addEventListener('change',saveLayout);});applyLayout();}
function saveLayout(){const data={};document.querySelectorAll('[data-section],[data-chart]').forEach(inp=>{const key=inp.getAttribute('data-section')||inp.getAttribute('data-chart');data[key]=inp.checked;});localStorage.setItem(projectStorageKey('board-layout'),JSON.stringify(data));applyLayout();}
function applyLayout(){document.querySelectorAll('[data-section]').forEach(inp=>{const key=inp.getAttribute('data-section');const sec=document.getElementById(`section-${key}`);if(sec)sec.classList.toggle('hidden',!inp.checked);});document.querySelectorAll('[data-chart]').forEach(inp=>{const key=inp.getAttribute('data-chart');const chart=document.getElementById(`chart-${key}`);if(chart)chart.classList.toggle('hidden',!inp.checked);});}
function restoreNotes(){let data={};try{data=JSON.parse(localStorage.getItem(projectStorageKey('board-notes'))||'{}')||{};}catch(_){data={};}document.querySelectorAll('[data-notes]').forEach(el=>{const key=el.getAttribute('data-notes');el.value=data[key]||'';const wrap=el.closest('.notes-wrap');if(wrap&&el.value)wrap.classList.add('open');el.addEventListener('input',saveNotes);});const dateInput=document.getElementById('boardDate');if(dateInput){dateInput.value=data.board_date||new Date().toISOString().slice(0,10);dateInput.addEventListener('change',saveNotes);}}
function saveNotes(){const data={};document.querySelectorAll('[data-notes]').forEach(el=>data[el.getAttribute('data-notes')]=el.value||'');const dateInput=document.getElementById('boardDate');if(dateInput)data.board_date=dateInput.value||'';localStorage.setItem(projectStorageKey('board-notes'),JSON.stringify(data));}
function toggleNotes(id){const wrap=document.getElementById(id);if(wrap)wrap.classList.toggle('open');}
function exportBoard(){const payload={affaire_id:affaireId,exported_at:new Date().toISOString(),layout:JSON.parse(localStorage.getItem(projectStorageKey('board-layout'))||'{}'),notes:JSON.parse(localStorage.getItem(projectStorageKey('board-notes'))||'{}')};const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`board-${affaireId||'affaire'}.json`;a.click();URL.revokeObjectURL(a.href);}
async function importBoard(file){if(!file)return;const data=JSON.parse(await file.text());if(data.layout)localStorage.setItem(projectStorageKey('board-layout'),JSON.stringify(data.layout));if(data.notes)localStorage.setItem(projectStorageKey('board-notes'),JSON.stringify(data.notes));restoreLayout();restoreNotes();}
function setText(id,v){const el=document.getElementById(id);if(el)el.textContent=v;}
function renderCompanyRows(rows){return rows.length?rows.map(r=>{const name=r.name||r.company||'-';return `<button class='company-row-btn' data-company='${esc(name)}'><div class='company-row'><span class='company-cell'>${companyLogoHtml(r)}<span class='name'>${esc(name)}</span></span><span class='count'><strong>${Number(r.count||0)}</strong> rappel(s)</span></div></button>`;}).join(''):`<div class='company-row'><span class='name'>Aucune donnée</span><span class='count'>0</span></div>`;}
function renderBarList(rootId,items,color='#ef8d00'){const root=document.getElementById(rootId);if(!root)return;if(!items.length){root.innerHTML=`<div class='company-row'><span class='name'>Aucune donnée</span><span class='count'>0</span></div>`;return;}const max=Math.max(1,...items.map(x=>Number(x.count||x.value||0)));root.innerHTML=items.map(x=>{const val=Number(x.count||x.value||0);return `<div class='bar-row'><div class='label'>${esc(x.name||x.label||'-')}</div><div class='bar-track'><div class='bar-fill' style='width:${(val/max)*100}%;background:${color}'></div></div><div style='text-align:right;font-size:16px;font-weight:900'>${val.toFixed(val%1?1:0)}</div></div>`;}).join('');}
function renderVerticalBars(rootId,items,{tempoName='tempo',suffix=' j'}={}){const root=document.getElementById(rootId);if(!root)return;if(!items.length){root.innerHTML=`<div class='company-row'><span class='name'>Aucune donnée</span><span class='count'>0</span></div>`;return;}const max=Math.max(1,...items.map(x=>Math.abs(Number(x.avg_gap_days??x.avg??0))));const showDelayDetails=rootId==='planningResources'||rootId==='planningResourcesToday';root.innerHTML=items.map((x,idx)=>{const val=Number(x.avg_gap_days??x.avg??0);const h=Math.max(8,(Math.abs(val)/max)*168);const cls=normalizeText(x.name||'')===tempoName?'tempo':'';const title=`${x.name||'-'}: ${val.toFixed(1)}${suffix}${showDelayDetails?` · ${Number(x.count||0)} tâche(s) prises en compte`:''}`;const bar=`<div class='react-stick ${cls}' style='height:${h}px'></div>`;const barHtml=showDelayDetails?`<button type='button' class='react-stick-btn' data-delay-idx='${idx}' title='${escAttr(title)}'>${bar}</button>`:`<div title='${escAttr(title)}'>${bar}</div>`;return `<div class='react-col'>${barHtml}<div class='react-name'>${esc(x.name||'-')}</div><div class='react-val'>${val.toFixed(1)}${suffix}</div></div>`;}).join('');if(showDelayDetails){Array.from(root.querySelectorAll('[data-delay-idx]')).forEach(btn=>btn.addEventListener('click',()=>{const idx=Number(btn.getAttribute('data-delay-idx'));const item=items[idx];if(item)showPlanningDelayModal({rootId,item,suffix});}));}}
function renderPrintMetricList(rootId,items,{suffix=''}={}){const root=document.getElementById(rootId);if(!root)return;if(!items.length){root.innerHTML=`<div class='company-row'><span class='name'>Aucune donnée</span><span class='count'>0</span></div>`;return;}const max=Math.max(1,...items.map(x=>Math.abs(Number(x.avg_gap_days??x.avg??x.count??x.value??0))));root.innerHTML=items.map(x=>{const label=x.name||x.label||'-';const val=Number(x.avg_gap_days??x.avg??x.count??x.value??0);return `<div class='summary-row'><div>${esc(label)}</div><div class='summary-track'><div class='summary-fill fac' style='width:${Math.max(3,(Math.abs(val)/max)*100)}%'></div></div><div style='text-align:right;font-weight:900'>${val.toFixed(val%1?1:0)}${suffix}</div></div>`;}).join('');}
function extractPlanningTasks(pointage,board){const candidates=[pointage&&pointage.tasks,pointage&&pointage.planning_tasks,board&&board.pointage&&board.pointage.tasks,board&&board.pointage_tasks,board&&board.planning_tasks,board&&board.tasks];for(const c of candidates){if(Array.isArray(c)&&c.length)return c;}return [];}

function renderPlanningCards(rootId,items){const root=document.getElementById(rootId);if(!root)return;if(!items.length){root.innerHTML=`<div class='company-row'><span class='name'>Aucune donnée</span><span class='count'>0</span></div>`;return;}root.innerHTML=items.map(x=>`<div class='planning-card'><div class='head'><div><div class='title'>${esc(x.label||'-')}</div><div class='sub'>${esc(x.subtitle||'')}</div></div><div class='side'>${esc(x.side||'')}</div></div></div>`).join('');}
function renderProgressRows(rootId,items){const root=document.getElementById(rootId);if(!root)return;if(!items.length){root.innerHTML=`<div class='company-row'><span class='name'>Aucune donnée</span><span class='count'>0</span></div>`;return;}root.innerHTML=items.map(x=>`<div class='progress-row'><div class='meta'><div><div class='name'>${esc(x.label)}</div><div class='sub' style='margin-top:4px;color:#6f7f97'>${esc(x.subtitle||'')}</div></div><div style='font-size:20px;font-weight:900'>${Number(x.value||0).toFixed(1)}%</div></div><div class='progress-track'><div class='progress-fill' style='width:${Math.max(0,Math.min(100,Number(x.value||0)))}%;background:${x.color||'linear-gradient(90deg,#1b6ef3,#5da2ff)'}'></div></div></div>`).join('');}
function computePlanningDatasets(tasks){const actionable=(tasks||[]).filter(t=>!t.is_summary);const byPerimeter={};for(const t of actionable){const key=String(t.perimeter||'Général').trim()||'Général';(byPerimeter[key]=byPerimeter[key]||[]).push(t);}const delayTrigger=Object.entries(byPerimeter).map(([label,list])=>{const late=list.filter(t=>Number(t.delayDays||0)>0).sort((a,b)=>Number(b.delayDays||0)-Number(a.delayDays||0));const it=late[0];return it?{label,subtitle:`Première tâche en retard : ${it.name||'-'} · ${Number(it.delayDays||0)} j`,side:String(it.owner||'Non attribué').trim()||'Non attribué'}:null;}).filter(Boolean).sort((a,b)=>a.label.localeCompare(b.label,'fr'));const perimeterProgress=Object.entries(byPerimeter).map(([label,list])=>{const avg=list.length?list.reduce((s,t)=>s+Number(t.progress||0),0)/list.length:0;const ordered=list.slice().sort((a,b)=>{const ad=parseDateOnly(a.start||'')||parseDateOnly(a.end||'');const bd=parseDateOnly(b.start||'')||parseDateOnly(b.end||'');return (ad?ad.getTime():Number.MAX_SAFE_INTEGER)-(bd?bd.getTime():Number.MAX_SAFE_INTEGER);});const progressed=ordered.filter(t=>Number(t.progress||0)>0||!!t.actualEnd);const lastProgressed=progressed.length?progressed[progressed.length-1]:null;let blocker=null;if(lastProgressed){const pivot=ordered.findIndex(t=>String(t.task_id||'')===String(lastProgressed.task_id||''));if(pivot>=0)blocker=ordered.slice(pivot+1).find(t=>Number(t.progress||0)<100&&!t.actualEnd)||null;}if(!blocker)blocker=ordered.find(t=>Number(t.progress||0)<100&&!t.actualEnd)||null;const subtitle=avg<100&&blocker?`Tâche bloquante : ${blocker.name||'-'} · ${String(blocker.owner||'Non attribué').trim()||'Non attribué'}`:`${list.length} tâche(s)`;return {label,value:avg,subtitle,color:'linear-gradient(90deg,#1b6ef3,#5da2ff)'};}).sort((a,b)=>b.value-a.value).slice(0,8);const stats=(includeUnpointed)=>{const rec={};const push=(name,delay)=>{const k=String(name||'').trim();if(!k||k==='Non attribué'||k.includes('+'))return;const r=rec[k]||(rec[k]={sum:0,count:0});r.sum+=Number(delay||0);r.count+=1;};for(const t of actionable){if(t.is_cet&&Array.isArray(t.cetMembers)&&t.cetMembers.length){for(const m of t.cetMembers){const cm=((t.cet||{})[m]||{});const pointed=Number(cm.progress||0)>0||!!cm.actualEnd;const ok=includeUnpointed?(cm.status==='late'||cm.status==='soon'||cm.status==='past'||pointed):((cm.status==='past'||!!cm.actualEnd)&&pointed);if(ok)push(m,cm.delayDays||0);}continue;}const pointed=Number(t.progress||0)>0||!!t.actualEnd;const ok=includeUnpointed?(t.status==='late'||t.status==='soon'||t.status==='past'||pointed):((t.status==='past'||!!t.actualEnd)&&pointed);if(ok)push(t.owner,t.delayDays||0);}return Object.entries(rec).map(([name,v])=>({name,avg:v.count?v.sum/v.count:0})).sort((a,b)=>b.avg-a.avg).slice(0,6);};const today=new Date();today.setHours(0,0,0,0);const clamp=v=>Math.max(0,Math.min(1,v));const actualRatio=actionable.length?clamp(actionable.reduce((sum,t)=>sum+Math.max(0,Math.min(100,Number(t.progress||0))),0)/(actionable.length*100)):0;const plannedRatio=actionable.length?clamp(actionable.reduce((sum,t)=>{const start=parseDateOnly(t.start||'');const end=parseDateOnly(t.end||'')||start;if(!start&&!end)return sum;const s=start||end;const e=end||start;if(today<=s)return sum;if(today>=e)return sum+1;const duration=Math.max(1,Math.round((e-s)/(1000*60*60*24)));const elapsed=Math.max(0,Math.round((today-s)/(1000*60*60*24)));return sum+clamp(elapsed/duration);},0)/actionable.length):0;const perimeterTimeline=Object.entries(byPerimeter).map(([label,list])=>{const starts=list.map(t=>parseDateOnly(t.start||'')).filter(Boolean);const ends=list.map(t=>parseDateOnly(t.end||'')).filter(Boolean);const plannedStart=starts.length?new Date(Math.min(...starts.map(d=>d.getTime()))):null;const plannedEnd=ends.length?new Date(Math.max(...ends.map(d=>d.getTime()))):(plannedStart?new Date(plannedStart):null);const pointedTasks=list.filter(t=>Number(t.progress||0)>0||!!t.actualEnd);const actualStarts=pointedTasks.map(t=>parseDateOnly(t.start||'')).filter(Boolean);const actualEnds=pointedTasks.map(t=>parseDateOnly(t.actualEnd||'')||parseDateOnly(t.end||'')).filter(Boolean);const actualStart=actualStarts.length?new Date(Math.min(...actualStarts.map(d=>d.getTime()))):null;const actualEnd=actualEnds.length?new Date(Math.max(...actualEnds.map(d=>d.getTime()))):null;return {label,plannedStart,plannedEnd,actualStart,actualEnd,taskCount:list.length,progress:list.length?list.reduce((s,t)=>s+Number(t.progress||0),0)/list.length:0};}).filter(x=>x.plannedStart||x.plannedEnd).sort((a,b)=>(a.plannedStart||a.plannedEnd)-(b.plannedStart||b.plannedEnd));return {delayTrigger,perimeterProgress,resources:stats(false),resourcesToday:stats(true),expected:[{label:'Avancement % planning attendu',value:plannedRatio*100,subtitle:'L’avancement théorique à date si aucun retard n’existe.'},{label:'Avancement % planning pointé',value:actualRatio*100,subtitle:'Le planning pointé à date.',color:'linear-gradient(90deg,#ef8d00,#ffbc62)'}],perimeterTimeline};}
function renderPerimeterTimeline(rows,{showActual=false}={}){const root=document.getElementById('planningCst');const printRoot=document.getElementById('planningCstPrint');if(!root)return;if(!rows.length){root.innerHTML=`<div class='company-row'><span class='name'>Aucun périmètre daté</span><span class='count'>0</span></div>`;if(printRoot)printRoot.innerHTML=root.innerHTML;return;}const starts=rows.map(r=>(r.plannedStart||r.plannedEnd).getTime());const ends=rows.map(r=>(r.plannedEnd||r.plannedStart).getTime());const startMin=new Date(Math.min(...starts));const endMax=new Date(Math.max(...ends));const totalMs=Math.max(24*60*60*1000,endMax.getTime()-startMin.getTime());const today=new Date();today.setHours(0,0,0,0);const todayLeft=Math.max(0,Math.min(100,((today.getTime()-startMin.getTime())/totalMs)*100));root.innerHTML=`<div class='timeline-macro-scroll'>${rows.map(r=>{const ps=r.plannedStart||r.plannedEnd;const pe=r.plannedEnd||r.plannedStart;const pLeft=Math.max(0,Math.min(100,((ps.getTime()-startMin.getTime())/totalMs)*100));const pWidth=Math.max(2,((Math.max(pe.getTime(),ps.getTime())-ps.getTime()+24*60*60*1000)/totalMs)*100);let actualBar='';let actualMeta='Aucun pointage daté';if(showActual&&r.actualStart&&r.actualEnd){const as=r.actualStart,ae=r.actualEnd;const aLeft=Math.max(0,Math.min(100,((as.getTime()-startMin.getTime())/totalMs)*100));const aWidth=Math.max(2,((Math.max(ae.getTime(),as.getTime())-as.getTime()+24*60*60*1000)/totalMs)*100);actualBar=`<div class='timeline-macro-fill' style='left:${aLeft}%;width:${aWidth}%;height:8px;top:2px;background:linear-gradient(90deg,#1b6ef3,#5da2ff)'><title>Pointé : ${esc(fmtDate(as.toISOString().slice(0,10)))} → ${esc(fmtDate(ae.toISOString().slice(0,10)))}</title></div>`;actualMeta=`Pointé : ${fmtDate(as.toISOString().slice(0,10))} → ${fmtDate(ae.toISOString().slice(0,10))}`;}return `<div class='timeline-macro-row'><div class='timeline-macro-head'><div><div class='timeline-macro-title'>${esc(r.label)}</div><div class='timeline-macro-sub'>${esc(actualMeta)} · ${Number(r.progress||0).toFixed(1)}% · ${Number(r.taskCount||0)} tâche(s)</div></div><div class='timeline-macro-date'>${esc(fmtDate(ps.toISOString().slice(0,10)))} → ${esc(fmtDate(pe.toISOString().slice(0,10)))}</div></div><div class='timeline-macro-track'><div class='timeline-macro-marker today' style='left:${todayLeft}%'><title>Aujourd'hui : ${esc(fmtDate(today.toISOString().slice(0,10)))}</title></div><div class='timeline-macro-fill' style='left:${pLeft}%;width:${pWidth}%'><title>Planning : ${esc(fmtDate(ps.toISOString().slice(0,10)))} → ${esc(fmtDate(pe.toISOString().slice(0,10)))}</title></div>${actualBar}</div><div class='timeline-macro-labels'><span>${esc(fmtDate(startMin.toISOString().slice(0,10)))}</span><span>${esc(fmtDate(endMax.toISOString().slice(0,10)))}</span></div></div>`;}).join('')}</div>`;if(printRoot){printRoot.innerHTML=`<table class='board-table'><thead><tr><th>Périmètre</th><th>Planning</th><th>Pointé</th><th>Avancement</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.label)}</td><td>${esc(fmtDate(r.plannedStart&&r.plannedStart.toISOString().slice(0,10)))} → ${esc(fmtDate(r.plannedEnd&&r.plannedEnd.toISOString().slice(0,10)))}</td><td>${r.actualStart&&r.actualEnd?`${esc(fmtDate(r.actualStart.toISOString().slice(0,10)))} → ${esc(fmtDate(r.actualEnd.toISOString().slice(0,10)))}`:'-'}</td><td>${Number(r.progress||0).toFixed(1)}%</td></tr>`).join('')}</tbody></table>`;}}
...
function populateMetronomeProjectPicker(board){const options=(board&&board.metronome_projects)||[];const selected=String((board&&board.selected_metronome_project_id)||'');const html=options.length?options.map(opt=>{const id=String(opt.project_id||'').trim();const title=String(opt.project_title||opt.project_id||'Projet').trim();return `<option value='${escAttr(id)}' ${id===selected?'selected':''}>${esc(title)}</option>`;}).join(''):"<option value=''>Aucun projet METRONOME</option>";['metronomeProjectPicker','metronomeProjectPickerPilot'].forEach(id=>{const picker=document.getElementById(id);if(picker)picker.innerHTML=html;});}
...
async function loadMetronomeBoard(projectId=''){const params=new URLSearchParams({affaire_id:affaireId});if(projectId)params.set('metronome_project_id',projectId);const board=await api(`/api/project-management/board?${params.toString()}`);window.__boardData=board;populateMetronomeProjectPicker(board);renderMetronomeBoard(board);return board;}
...
@app.get("/dashboard-board", response_class=HTMLResponse)
def dashboard_board_page():
    return dashboard_board_html()
...
def api_affaire_detail(affaire_id: str, mission_key: str = Query(default="")):
    cache = service.get_finance_cache()
    item = cache.get("items", {}).get(affaire_id)
    if not item:
        raise HTTPException(status_code=404, detail=f"Affaire introuvable : {affaire_id}")
    affaire = dict(item)
    affaire.update(pointage_finance_summary(affaire_id, clean_number(affaire.get("commande_ht")), mission_key=mission_key))
...
def api_project_management_board(
    ...
    if affaire_id and not name:
        ...
        best["finance_missions"] = [
            {
                "mission_key": slugify(clean_text(m.get("label")) or clean_text(m.get("tag")) or clean_text(m.get("numero")) or f"mission-{idx+1}"),
                "label": clean_text(m.get("label")) or clean_text(m.get("tag")) or clean_text(m.get("numero")) or f"Mission {idx+1}",
            }
            for idx, m in enumerate(item.get("missions") or [])
