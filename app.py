...
function renderFinance(affaire){const billed=Number(affaire.facturation_totale||((affaire.anteriorite||0)+(affaire.facture_2026||affaire.facturation_cumulee_2026||0)));const cmd=Number(affaire.commande_ht||0);const remain=Number(affaire.reste_a_facturer||0);const missions=(affaire.missions||[]);const filtersRoot=document.getElementById('financeMissionFilters');const detailRoot=document.getElementById('financeMissionMonthlyDetail');const selectedKey=`finance-missions::${affaireId}`;let selected={};try{selected=JSON.parse(localStorage.getItem(selectedKey)||'{}')||{};}catch(_){selected={};}const missionSeries=missions.map((mission,idx)=>{let run=Number(mission.anteriorite||0);const monthly=MONTHS.map(month=>Number((((mission.mensuel||{})[month]||{}).facture)||0));const cumulative=monthly.map(value=>{run+=value;return run;});return {key:String(mission.tag||mission.label||idx),label:String(mission.tag||mission.label||`Mission ${idx+1}`),facColor:FINANCE_FAC_COLORS[idx%FINANCE_FAC_COLORS.length],monthly,cumulative};});const activeMissions=missionSeries.filter(m=>selected[m.key]!==false);if(filtersRoot){filtersRoot.innerHTML=missionSeries.length?missionSeries.map(m=>`<label class='toggle-chip'><input type='checkbox' data-mission-key='${escAttr(m.key)}' ${selected[m.key]!==false?'checked':''}>${esc(m.label)}</label>`).join(''):"<div class='small'>Aucune mission détaillée.</div>";Array.from(filtersRoot.querySelectorAll('[data-mission-key]')).forEach(inp=>inp.addEventListener('change',()=>{selected[inp.getAttribute('data-mission-key')||'']=inp.checked;localStorage.setItem(selectedKey,JSON.stringify(selected));renderFinance(affaire);}));}const monthlyProduction=MONTHS.map((month,idx)=>{const curr=Number(((affaire.production_cumulee_mensuelle||{})[month])||0);const prev=idx>0?Number(((affaire.production_cumulee_mensuelle||{})[MONTHS[idx-1]])||0):0;return Math.max(0,curr-prev);});const isMonthly=!!((document.getElementById('financeModeMonthly')||{}).checked);const showProd=!!((document.getElementById('financeShowProduction')||{}).checked);const barValues=MONTHS.map((month,idx)=>activeMissions.reduce((sum,m)=>sum+Number((isMonthly?m.monthly[idx]:m.cumulative[idx])||0),0));const lineValues=showProd?MONTHS.map((month,idx)=>Number(isMonthly?monthlyProduction[idx]:(((affaire.production_cumulee_mensuelle||{})[month])||0))):[];const max=Math.max(1,...barValues,...lineValues);const left=54,top=18,width=892,height=176,step=width/Math.max(1,MONTHS.length),barW=Math.max(18,Math.min(40,step*0.5));let out='';for(let i=0;i<=4;i++){const y=top+(height/4)*i;const val=Math.round(max*(1-i/4));out+=`<line x1='${left}' y1='${y}' x2='${left+width}' y2='${y}' stroke='#e6edf7'/><text x='${left-8}' y='${y+4}' text-anchor='end' fill='#5f6f88' font-size='11'>${val.toLocaleString('fr-FR')} €</text>`;}MONTHS.forEach((month,idx)=>{const x=left+idx*step+step*0.5;let base=0;activeMissions.forEach(m=>{const val=Number((isMonthly?m.monthly[idx]:m.cumulative[idx])||0);if(val<=0)return;const y=top+height-((base+val)/max)*height;const h=(val/max)*height;out+=`<rect x='${x-barW/2}' y='${y}' width='${barW}' height='${Math.max(1,h)}' rx='4' fill='${m.facColor}'><title>${esc(m.label)} — ${isMonthly?'Facturé':'Cumulé'} ${MONTH_LABELS[month]}: ${euro(val)}</title></rect>`;base+=val;});if(idx%2===0)out+=`<text x='${x}' y='${top+height+18}' fill='#5f6f88' font-size='11' text-anchor='middle'>${MONTH_LABELS[month]}</text>`;});if(showProd&&lineValues.length){const prodPts=lineValues.map((d,i)=>`${left+i*step+step*0.5},${top+height-(d/max)*height}`).join(' ');out+=`<polyline points='${prodPts}' fill='none' stroke='#239b5b' stroke-width='3' stroke-dasharray='6 5'><title>${isMonthly?'Production mensuelle':'Production cumulée'}</title></polyline>`;}document.getElementById('financeChart').innerHTML=out;const cumulativeByMonth=MONTHS.map((month,idx)=>activeMissions.reduce((sum,m)=>sum+Number(m.cumulative[idx]||0),0));if(detailRoot){detailRoot.innerHTML=`<table class='board-table'><thead><tr><th>Mission</th>${MONTHS.map(month=>`<th>${MONTH_LABELS[month]}</th>`).join('')}<th>Total cumulé</th></tr></thead><tbody>${activeMissions.map(m=>`<tr><td>${esc(m.label)}</td>${MONTHS.map((month,idx)=>`<td class='num'>${euro(m.monthly[idx]||0)}</td>`).join('')}<td class='num'>${euro(m.cumulative[m.cumulative.length-1]||0)}</td></tr>`).join('')||`<tr><td colspan='14'>Aucune mission sélectionnée.</td></tr>`}<tr><td><strong>Cumul par mois</strong></td>${cumulativeByMonth.map(v=>`<td class='num'><strong>${euro(v)}</strong></td>`).join('')}<td class='num'><strong>${euro(cumulativeByMonth[cumulativeByMonth.length-1]||0)}</strong></td></tr></tbody></table>`;}const maxFinal=Math.max(cmd,billed,remain,1);document.getElementById('financeSummary').innerHTML=[['Facturation cumulée',billed,'fac'],['Reste à facturer',remain,'prod']].map(r=>`<div class='summary-row'><div>${r[0]}</div><div class='summary-track'><div class='summary-fill ${r[2]}' style='width:${Math.min(100,(Number(r[1]||0)/maxFinal)*100)}%'></div></div><div style='text-align:right;font-weight:900'>${euro(r[1])}</div></div>`).join('');const printSummary=document.getElementById('financePrintSummary');if(printSummary){printSummary.innerHTML=`<table class='board-table'><thead><tr><th>Indicateur</th><th>Montant</th></tr></thead><tbody><tr><td>Commande HT</td><td class='num'>${euro(cmd)}</td></tr><tr><td>Facturation cumulée</td><td class='num'>${euro(billed)}</td></tr><tr><td>Reste à facturer</td><td class='num'>${euro(remain)}</td></tr></tbody></table>`;}document.getElementById('financeMissions').innerHTML=missions.length?missions.map(m=>`<tr><td>${esc(m.tag||'')}</td><td>${esc(m.label||'')}</td><td>${esc(m.numero||'')}</td><td class='num'>${euro(m.commande_ht)}</td><td class='num'>${euro(m.facturation_totale||((m.anteriorite||0)+(m.facture_2026||m.facturation_cumulee_2026||0)))}</td><td class='num'>${euro(m.reste_a_facturer)}</td><td class='num'>${euro(m.total_previsionnel)}</td><td class='num'>${euro(m.total_facture)}</td></tr>`).join(''):`<tr><td colspan='8'>Aucune mission détaillée.</td></tr>`;}
function reminderLevelLabel(item){const raw=Number(item.reminder_level||item.level||item.rappel||0);if(raw>0)return `Rappel ${raw}`;const deadline=parseDateOnly(item.deadline||'');if(!deadline)return 'Rappel';const today=new Date();today.setHours(0,0,0,0);const diff=Math.max(0,Math.ceil((today-deadline)/(1000*60*60*24)));return diff>0?`Rappel ${Math.max(1,Math.ceil(diff/7))}`:'Rappel';}
