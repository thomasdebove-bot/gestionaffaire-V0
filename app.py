...
function boondOptionsHtml(b){const opts=(b&&b.project_options)||[];const current=String(state.boondForcedProjectId||(((b||{}).matched_project||{}).project_id||''));return [`<option value=''>-- Choisir un projet BOOND --</option>`].concat(opts.map(o=>`<option value='${esc(o.project_id)}' ${String(o.project_id)===current?'selected':''}>${esc(o.project_reference||o.project_id)} [${esc(String(o.project_id))}] (score ${esc(String(o.score||0))})</option>`)).join('');}
