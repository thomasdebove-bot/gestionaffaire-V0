...
<body>
  <div class='wrap'>
    ...
    <section class='grid'>
      ...
      <article class='card'><h3>Imputation des temps</h3><p>Coûts et jours imputés BOOND pour le projet courant.</p><a id='boondImputationLink' class='btn disabled' href='javascript:void(0)' aria-disabled='true'>Ouvrir</a></article>
    </section>
  </div>
<script>
...
function updateUi(){document.getElementById('projectBadge').textContent=state.selectedLabel?`Affaire : ${state.selectedLabel}`:'Aucune affaire sélectionnée';document.getElementById('state').textContent=state.selectedLabel?`Vous naviguez sur l'affaire ${state.selectedLabel}.`:'Sélectionnez une affaire pour activer les modules.';setModuleLink('financeLink','finance');setModuleLink('dashboardLink','dashboard');setModuleLink('pmLink','gestion-projet');const boond=document.getElementById('boondImputationLink');if(state.selectedLabel){boond.className='btn primary';boond.href=`/boond-imputations?project_name=${encodeURIComponent(state.selectedLabel)}`;boond.removeAttribute('aria-disabled');}else{boond.className='btn disabled';boond.href='javascript:void(0)';boond.setAttribute('aria-disabled','true');}if(state.selectedId){localStorage.setItem('selectedAffaireId',state.selectedId);} }
...
<div class='container'>
  <div class='topbar'>
    ...
    <div class='controls'>
      ...
      <a id='boondBtn' class='btn dark' href='/boond-imputations'>Imputation des temps</a>
...
<div class='wrap'>
  <div class='top'>
    ...
    <a id='boondBtn' class='btn' href='/boond-imputations'>Imputation des temps</a>
