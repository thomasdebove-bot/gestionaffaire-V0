...
class MetronomeService:
    ...
    def _parse_date_value(value: str) -> Optional[datetime]:
        ...
        if not txt:
            return None
        for fmt in (
            "%Y-%m-%d", "%d/%m/%Y", "%Y/%m/%d", "%d/%m/%Y %H:%M", "%d/%m/%Y %H:%M:%S", "%Y-%m-%d %H:%M", "%Y-%m-%d %H:%M:%S", "%d/%m/%y %H:%M:%S"
        ):
            try:
...
class PointageService:
    ...
    def parse_planning(self, raw: bytes) -> List[Dict[str, Any]]:
        ...
        c_start = self._find_col(first, ["start", "start date", "debut", "début", "date debut", "date début"])
        c_end = self._find_col(first, ["finish", "finish date", "fin", "date fin", "end", "end date"])
        ...
        c_pct = self._find_col(first, ["%", "% acheve", "% achevé", "percent", "percentage complete", "percent complete", "acheve", "achevé", "avancement", "pourcentage_acheve", "pourcentage acheve"])
...
function renderFinanceChart(){const root=document.getElementById('monthlyChart');const a=state.selectedAffaire;if(!a){root.innerHTML=`<text x="490" y="160" text-anchor="middle" fill="#6e7a90" font-size="18">Sélectionnez une affaire</text>`;return;}const missions=(a.missions||[]);const palette=['#ef8d00','#1b6ef3','#16a085','#8e44ad','#d64545','#f39c12','#2d98da','#6ab04c','#eb4d4b','#4834d4'];const rows=MONTHS.map((m,idx)=>{const byMission=missions.map((ms,i)=>({name:ms.label||ms.tag||`Mission ${i+1}`,value:Number((((ms.mensuel||{})[m]||{}).facture)||0),color:palette[i%palette.length]})).filter(x=>x.value>0);const fac=byMission.reduce((acc,x)=>acc+x.value,0);const pre=Number((((a.mensuel||{})[m]||{}).previsionnel)||0);return {label:MONTH_LABELS[m],month:m,pre,fac,byMission,idx};});const maxVal=Math.max(1,...rows.flatMap(x=>[x.pre,x.fac]));const left=56,top=16,width=880,height=250,step=width/rows.length,barW=step*0.56;let grid='',bars='',labels='';for(let i=0;i<=4;i++){const y=top+(height/4)*i,val=Math.round(maxVal*(1-i/4));grid+=`<line x1="${left}" y1="${y}" x2="${left+width}" y2="${y}" stroke="#dfe5ef" stroke-width="1"/><text x="${left-10}" y="${y+4}" text-anchor="end" fill="#8090a8" font-size="12">${fmt(val)}</text>`;}rows.forEach((it,i)=>{const x=left+i*step+(step-barW)/2;const preH=(it.pre/maxVal)*height;const preY=top+height-preH;bars+=`<rect x="${x}" y="${preY}" width="${barW}" height="${Math.max(preH,0.5)}" rx="8" fill="#eef3ff"><title>${it.label} prévisionnel: ${euro(it.pre)}</title></rect>`;let stackBottom=top+height;it.byMission.forEach(seg=>{const h=(seg.value/maxVal)*height;const y=stackBottom-h;bars+=`<rect x="${x+2}" y="${y}" width="${Math.max(1,barW-4)}" height="${Math.max(h,1)}" rx="4" fill="${seg.color}"><title>${it.label} · ${seg.name}: ${euro(seg.value)}</title></rect>`;stackBottom=y;});if(!it.byMission.length&&it.fac>0){const h=(it.fac/maxVal)*height;const y=top+height-h;bars+=`<rect x="${x+2}" y="${y}" width="${Math.max(1,barW-4)}" height="${Math.max(h,1)}" rx="4" fill="#ef8d00"><title>${it.label} facturé: ${euro(it.fac)}</title></rect>`;}labels+=`<text x="${x+barW/2}" y="${top+height+22}" text-anchor="middle" fill="#66748b" font-size="12">${it.label}</text>`;});root.innerHTML=`${grid}<line x1="${left}" y1="${top+height}" x2="${left+width}" y2="${top+height}" stroke="#b8c3d4" stroke-width="1.2"/>${bars}${labels}`;}
...
<div class='wrap'>
  ...
  <div class='chart'><h3>Prévisionnel vs facturation (mois écoulés)</h3><div class='chart-wrap'><svg id='chart' width='100%' height='320' viewBox='0 0 980 320' preserveAspectRatio='none'></svg></div></div>
</div>
<script>
const MONTHS=['janvier','fevrier','mars','avril','mai','juin','juillet','aout','septembre','octobre','novembre','decembre'];
const MONTH_LABELS={janvier:'Janv.',fevrier:'Févr.',mars:'Mars',avril:'Avr.',mai:'Mai',juin:'Juin',juillet:'Juil.',aout:'Août',septembre:'Sept.',octobre:'Oct.',novembre:'Nov.',decembre:'Déc.'};
