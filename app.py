...
async function loadAffaires(q=''){const d=await api(`/api/finance/affaires${q?`?q=${encodeURIComponent(q)}`:''}`);state.affaires=d.affaires||[];const sel=document.getElementById('affaireSelect');sel.innerHTML="<option value=''>Sélectionner une affaire</option>"+state.affaires.map(a=>`<option value='${a.affaire_id}'>${a.display_name||a.affaire_id}</option>`).join('');}
async function loadSelected(id){state.selectedId=id;state.selected=(state.affaires||[]).find(a=>a.affaire_id===id)||null;if(!id||!state.selected){render();return;}state.selected=await api(`/api/finance/affaire/${encodeURIComponent(id)}`);try{state.board=await api(`/api/project-management/board?affaire_id=${encodeURIComponent(id)}`);}catch(_){state.board=null;}try{state.boond=await api(`/api/boond/imputations/by-project?project_name=${encodeURIComponent(state.selected.affaire||state.selected.display_name||'')}&affaire_id=${encodeURIComponent(id)}&affaire=${encodeURIComponent(state.selected.affaire||'')}&numero=${encodeURIComponent(state.selected.numero||'')}&client=${encodeURIComponent(state.selected.client||'')}&display_name=${encodeURIComponent(state.selected.display_name||'')}`);}catch(_){state.boond=null;}render();}
