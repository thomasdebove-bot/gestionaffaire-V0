...
function pointageFinanceCalculationRows(mode,a,boond){const today=new Date().toLocaleDateString('fr-FR');const commande=Number(a.commande_ht||0);const plannedRatio=Number(a.pointage_planned_to_date_ratio||0);const pointedRatio=Number(a.pointage_progress_ratio||0);const boondCost=Number(boond.total_cost||0);const taskCount=Number(a.pointage_task_count||0);if(mode==='pointed-vs-boond'){return [{key:'pointed-sale',label:'Vente pointée à date',value:Number(a.pointage_progress_amount||0),color:'#ef8d00',subtitle:'Commande x avancement réellement pointé',badge:'Cliquer pour voir le détail du calcul',details:{title:'Calcul — Vente pointée à date',intro:`Calcul basé sur ${taskCount} tâche(s) pointée(s) du planning.`,steps:[{label:'Commande HT',value:euro(commande),desc:'Montant contractuel de référence utilisé pour valoriser l’avancement.'},{label:'Avancement pointé',value:pct(pointedRatio),desc:'Moyenne des pourcentages réellement pointés sur les tâches prises en compte.'},{label:'Vente pointée à date',value:euro(a.pointage_progress_amount||0),desc:'Valorisation financière de l’avancement réellement pointé.'}],formula:`Vente pointée à date = Commande HT × avancement pointé
${euro(commande)} × ${pct(pointedRatio)} = ${euro(a.pointage_progress_amount||0)}`}},{key:'boond-cost',label:'Coût de production BOOND',value:boondCost,color:'#1b6ef3',subtitle:'Productivité projet BOOND',badge:'Cliquer pour voir la source de calcul',details:{title:'Calcul — Coût de production BOOND',intro:'Valeur issue du projet matché dans BOOND, calculée côté productivité projet.',steps:[{label:'Projet BOOND',value:((state.boondImputation||{}).matched_project||{}).project_label||((state.boondImputation||{}).matched_project||{}).project_reference||'-',desc:'Projet BOOND retenu pour le rapprochement.'},{label:'Source BOOND',value:String(boond.cost_source||'projects_productivity'),desc:'Champ de BOOND utilisé pour récupérer le coût global.'},{label:'Coût retenu',value:euro(boondCost),desc:'Coût cumulé renvoyé par l’API BOOND pour ce projet.'}],formula:`Coût de production BOOND = coût cumulé du projet matché
Valeur renvoyée par BOOND : ${euro(boondCost)}`}}];}
return [{key:'planned-sale',label:'Vente planifiée à date',value:Number(a.pointage_planned_to_date_amount||0),color:'#5da2ff',subtitle:`Où le planning devrait en être au ${today}`,badge:'Cliquer pour voir le détail du calcul',details:{title:'Calcul — Vente planifiée à date',intro:`Calcul théorique à date (${today}) à partir du planning importé.`,steps:[{label:'Commande HT',value:euro(commande),desc:'Montant contractuel de référence.'},{label:'Avancement planifié à date',value:pct(plannedRatio),desc:'Part théorique du planning qui devrait être réalisée à la date du jour.'},{label:'Vente planifiée à date',value:euro(a.pointage_planned_to_date_amount||0),desc:'Valorisation financière du planning théorique à date.'}],formula:`Vente planifiée à date = Commande HT × avancement planifié à date
${euro(commande)} × ${pct(plannedRatio)} = ${euro(a.pointage_planned_to_date_amount||0)}`}},{key:'pointed-sale',label:'Vente pointée à date',value:Number(a.pointage_progress_amount||0),color:'#ef8d00',subtitle:'Commande x avancement réellement pointé',badge:'Cliquer pour voir le détail du calcul',details:{title:'Calcul — Vente pointée à date',intro:`Calcul basé sur ${taskCount} tâche(s) pointée(s) du planning.`,steps:[{label:'Commande HT',value:euro(commande),desc:'Montant contractuel de référence utilisé pour valoriser l’avancement.'},{label:'Avancement pointé',value:pct(pointedRatio),desc:'Moyenne des pourcentages réellement pointés sur les tâches prises en compte.'},{label:'Vente pointée à date',value:euro(a.pointage_progress_amount||0),desc:'Valorisation financière de l’avancement réellement pointé.'}],formula:`Vente pointée à date = Commande HT × avancement pointé
${euro(commande)} × ${pct(pointedRatio)} = ${euro(a.pointage_progress_amount||0)}`}}];}
