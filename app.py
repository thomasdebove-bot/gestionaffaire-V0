...
.topbar{display:flex;gap:16px;align-items:center;padding:14px 18px;position:sticky;top:0;z-index:20;background:rgba(255,255,255,.96);backdrop-filter:blur(6px)}.brand{min-width:220px;display:flex;gap:10px;align-items:flex-start}.brand img{width:34px;height:34px;object-fit:contain;border-radius:8px;background:#fff7ef;border:1px solid #f8ddb4;padding:4px}.brand h1{margin:0;font-size:36px;line-height:1}.brand p{margin:6px 0 0;color:var(--muted);font-size:14px}
...
<div class='container'>
  <div class='topbar'>
    <div class='brand'><img src='/assets/logo-tempo' alt='Logo Tempo'><div><div class='eyebrow'>Gestion affaire</div><h1>Finance</h1><p>Cockpit financier par affaire</p></div></div>
    <div class='controls'>
...
function renderBoondImputation(){const root=document.getElementById('boondImputationBox');if(!root)return;const b=state.boondImputation;const a=state.selectedAffaire;if(!a){root.className='empty';root.textContent='Sélectionnez une affaire pour charger le coût cumulé BOOND.';return;}if(!b){root.className='empty';root.textContent='Chargement des données BOOND…';return;}if(!b.matched_project){root.className='empty';root.innerHTML=`<div>Aucun projet BOOND matché pour cette affaire.</div><div style='margin-top:8px' class='small'>Choisissez provisoirement un projet BOOND dans la liste :</div><div style='margin-top:8px;display:flex;gap:8px;align-items:center'><input id='boondProjectSearch' placeholder='Filtrer (ex: laffitte)' value='${esc(state.boondProjectSearch||'')}' style='padding:6px 8px;min-width:220px'><select id='boondProjectSelect' style='padding:6px 8px;min-width:520px'>${boondOptionsHtml(b)}</select><button id='boondProjectOverrideApply' class='btn' type='button'>Forcer</button><button id='boondProjectOverrideClear' class='btn' type='button'>Retirer</button></div>`;const search=document.getElementById('boondProjectSearch');if(search)search.addEventListener('input',()=>{state.boondProjectSearch=(search.value||'').trim();renderBoondImputation();});const apply=document.getElementById('boondProjectOverrideApply');if(apply)apply.addEventListener('click',async()=>{const sel=document.getElementById('boondProjectSelect');const v=(sel&&sel.value||'').trim();state.boondForcedProjectId=v;if(state.selectedAffaireId){if(v)localStorage.setItem(`boondForcedProjectId:${state.selectedAffaireId}`,v);else localStorage.removeItem(`boondForcedProjectId:${state.selectedAffaireId}`);await loadSelectedAffaire(state.selectedAffaireId);}});const clear=document.getElementById('boondProjectOverrideClear');if(clear)clear.addEventListener('click',async()=>{state.boondForcedProjectId='';if(state.selectedAffaireId){localStorage.removeItem(`boondForcedProjectId:${state.selectedAffaireId}`);await loadSelectedAffaire(state.selectedAffaireId);}});return;}const t=b.totals||{};const cost=(t.total_cost==null)?'-':euro(t.total_cost);const billed=Number((a&&a.facturation_totale)||0);const imputed=Number(t.total_cost||0);const calcMargin=(imputed>0||billed>0)?(billed-imputed):null;const calcRent=(imputed>0&&billed>0)?(((billed-imputed)/Math.max(imputed,1))*100):null;const margin=(calcMargin==null)?'-':euro(calcMargin);const prof=(calcRent==null)?'-':`${fmt(calcRent)}%`;const resources=(b.resources_imputation||[]);const selectedInfo=`<div style='padding:10px 12px' class='small'>Projet BOOND utilisé: <b>${esc((b.matched_project||{}).project_label||(b.matched_project||{}).project_reference||'-')}</b> (id: <code>${esc((b.matched_project||{}).project_id||'-')}</code>) · mode: ${esc((b.matched_project||{}).matched_on||'auto')}</div>`;const override=`<div style='padding:0 12px 10px;display:flex;gap:8px;align-items:center'><input id='boondProjectSearch' placeholder='Filtrer (ex: laffitte)' value='${esc(state.boondProjectSearch||'')}' style='padding:6px 8px;min-width:220px'><select id='boondProjectSelect' style='padding:6px 8px;min-width:520px'>${boondOptionsHtml(b)}</select><button id='boondProjectOverrideApply' class='btn' type='button'>Forcer</button><button id='boondProjectOverrideClear' class='btn' type='button'>Retirer</button></div>`;const detail=resources.length?`<table style='margin-top:10px'><thead><tr><th>Ressource</th><th class='num'>Jours</th><th class='num'>Coût imputé</th></tr></thead><tbody>${resources.map(r=>`<tr><td>${esc(r.resource_name||r.resource_id||'-')}</td><td class='num'>${fmt(r.days)}</td><td class='num'>${r.cost==null?'-':euro(r.cost)}</td></tr>`).join('')}</tbody></table>`:`<div style='padding:8px 12px' class='small'>Détail ressource indisponible.</div>`;root.className='table-wrap';root.innerHTML=`<table><thead><tr><th>Source</th><th class='num'>Coût imputé cumulé</th><th class='num'>Marge (facturé - imputé)</th><th class='num'>Rentabilité (facturé/imputé)</th></tr></thead><tbody><tr><td>${esc(t.cost_source||'projects_productivity')}</td><td class='num'><strong>${cost}</strong></td><td class='num'>${margin}</td><td class='num'>${prof}</td></tr></tbody></table>${selectedInfo}<div style='padding:10px 12px' class='small'>Coût global issu de la productivité projet BOOND.</div>${override}${detail}`;const search=document.getElementById('boondProjectSearch');if(search)search.addEventListener('input',()=>{state.boondProjectSearch=(search.value||'').trim();renderBoondImputation();});const apply=document.getElementById('boondProjectOverrideApply');if(apply)apply.addEventListener('click',async()=>{const sel=document.getElementById('boondProjectSelect');const v=(sel&&sel.value||'').trim();state.boondForcedProjectId=v;if(state.selectedAffaireId){if(v)localStorage.setItem(`boondForcedProjectId:${state.selectedAffaireId}`,v);else localStorage.removeItem(`boondForcedProjectId:${state.selectedAffaireId}`);await loadSelectedAffaire(state.selectedAffaireId);}});const clear=document.getElementById('boondProjectOverrideClear');if(clear)clear.addEventListener('click',async()=>{state.boondForcedProjectId='';if(state.selectedAffaireId){localStorage.removeItem(`boondForcedProjectId:${state.selectedAffaireId}`);await loadSelectedAffaire(state.selectedAffaireId);}});} 
...
.top{padding:14px;display:flex;gap:10px;align-items:center;flex-wrap:wrap}.brand-mini{display:inline-flex;align-items:center;gap:8px;padding:6px 10px;border:1px solid var(--line);border-radius:12px;background:#fff}.brand-mini img{width:24px;height:24px;object-fit:contain}.brand-mini b{font-size:12px;letter-spacing:.08em}.search,.select{height:44px;border:1px solid var(--line);border-radius:12px;padding:0 12px;min-width:280px}.search.locked,.select.locked{background:#eef2f8;color:#6e7a90;pointer-events:none}
...
<body><div class='wrap'>
  <div class='top'>
    <div class='brand-mini'><img src='/assets/logo-tempo' alt='Tempo'><b>TEMPO</b></div><a class='btn' href='/'>Accueil</a>
    <input id='searchInput' class='search locked' type='search' placeholder='Projet verrouillé (changer depuis Accueil)' readonly>
    <select id='affaireSelect' class='select locked' disabled><option value=''>Projet verrouillé</option></select>
...
<div class='wrap'>
  <div class='top'>
    <div class='brand-mini'><img src='/assets/logo-tempo' alt='Tempo'><b>TEMPO</b></div>
