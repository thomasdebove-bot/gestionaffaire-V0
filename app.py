...
<body>
  <div class='wrap'>
    <section class='hero'>
      ...
      <div class='sub'>Choisissez une affaire depuis la liste ci-dessous, puis accédez directement aux modules disponibles.</div>
      <div class='selector'>
        <select id='projectSelect' class='search' aria-label='Sélection du projet'>
...
function syncSelectedProject(selected){state.selectedId=(selected&&selected.id)||'';state.selectedLabel=(selected&&selected.label)||'';if(state.selectedId){localStorage.setItem('selectedAffaireId',state.selectedId);localStorage.setItem('selectedAffaireLabel',state.selectedLabel||'');}else{localStorage.removeItem('selectedAffaireId');localStorage.removeItem('selectedAffaireLabel');}}
function updateUi(){document.getElementById('projectBadge').textContent=state.selectedLabel?`Affaire : ${state.selectedLabel}`:'Aucune affaire sélectionnée';document.getElementById('state').textContent=state.selectedLabel?`Vous naviguez sur l'affaire ${state.selectedLabel}.`:'Sélectionnez une affaire pour activer les modules.';setModuleLink('financeLink','finance');setModuleLink('dashboardLink','dashboard');setModuleLink('pmLink','gestion-projet');}
...
const select=document.getElementById('projectSelect');select.innerHTML=`<option value=''>Sélectionner une affaire…</option>`+state.projects.map(p=>`<option value="${esc(p.id)}">${esc(p.label)}</option>`).join('');const savedId=localStorage.getItem('selectedAffaireId')||'';const selected=state.projects.find(x=>x.id===savedId)||null;if(selected){syncSelectedProject(selected);select.value=selected.id;}else{syncSelectedProject(null);select.value='';}updateUi();}
document.getElementById('projectSelect').addEventListener('change',ev=>{const selected=state.projects.find(p=>p.id===String(ev.target.value||''))||null;syncSelectedProject(selected);updateUi();});
loadProjects();
</script>
</body>
</html>"""
...
<body><div class='wrap'>
  <div class='top'>
    ...
    <div class='top-main'>
      <div class='menu-row'>
        ...
        <a id='dashboardBtn' class='btn' href='/dashboard'>Tableau de bord</a>
        <a id='changeProjectLink' class='btn' href='/'>Changer de projet</a>
      </div>
      <div class='project-row'>
        <input id='searchInput' class='search locked' type='search' placeholder='Projet verrouillé (changer depuis Accueil)' readonly>
        <select id='affaireSelect' class='select locked' disabled><option value=''>Projet verrouillé</option></select>
  ...
  <div id='boardEmptyState' class='empty-state' style='display:none'>
    <h2>Aucun projet sélectionné</h2>
    <p>Sélectionnez un projet depuis l’accueil pour afficher le board.</p>
  ...
  <div class='section'>
    <h3>Sujets board</h3>
    <div id='boardTable' class='table-wrap'><div class='small' style='padding:12px'>Aucun sujet ouvert.</div></div>
  </div>
...
function resolveAffaireById(input){const raw=String(input||'').trim();if(!raw)return null;return (state.affaires||[]).find(a=>String(a.affaire_id||'')===raw)||null;}
function syncProjectUi(){const search=document.getElementById('searchInput');if(search)search.value=state.selectedLabel||'';const select=document.getElementById('affaireSelect');if(select){if(state.selectedId){select.innerHTML=`<option value='${escAttr(state.selectedId)}' selected>${esc(state.selectedLabel||state.selectedId)}</option>`;select.value=state.selectedId;}else{select.innerHTML="<option value=''>Projet sélectionné depuis l’accueil</option>";select.value='';}}document.getElementById('financeBtn').href=state.selectedId?`/finance?affaire_id=${encodeURIComponent(state.selectedId)}`:'/finance';document.getElementById('dashboardBtn').href=state.selectedId?`/dashboard?affaire_id=${encodeURIComponent(state.selectedId)}`:'/dashboard';const changeLink=document.getElementById('changeProjectLink');if(changeLink)changeLink.href='/';}
function setEmptyStateVisible(visible){const empty=document.getElementById('boardEmptyState');const content=document.getElementById('boardContent');if(empty)empty.style.display=visible?'block':'none';if(content)content.style.display=visible?'none':'block';if(visible)showLoading(false);}
function esc(v){return String(v??'').replace(/[&<>]/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[ch]||ch));}
...
async function loadBoard(id){const requested=String(id||state.selectedId||'').trim();if(!requested){state.selectedId='';state.selectedLabel='';state.board=null;state.pointage=null;syncProjectUi();setEmptyStateVisible(true);renderBoard();renderPointage();return;}const selected=resolveAffaireById(requested);state.selectedId=requested;state.selectedLabel=(selected&&selected.display_name)||localStorage.getItem('selectedAffaireLabel')||requested;localStorage.setItem('selectedAffaireId',requested);localStorage.setItem('selectedAffaireLabel',state.selectedLabel||'');syncProjectUi();setEmptyStateVisible(false);showLoading(true,'Chargement de la vue projet…');try{const b=await api(`/api/project-management/board?affaire_id=${encodeURIComponent(requested)}`);state.board=b;renderBoard();}catch(err){console.error(err);state.board=null;renderBoard();showPageError(err);}finally{showLoading(false);}}
...
async function loadAffaires(q=''){const data=await api(`/api/finance/affaires?search=${encodeURIComponent(q||'')}`);state.affaires=data.items||[];const selected=resolveAffaireById(state.selectedId);if(selected){state.selectedLabel=selected.display_name||state.selectedLabel||'';}syncProjectUi();}
...
async function loadPointage(){if(!state.selectedId){state.pointage=null;renderPointage();renderPlanningChart();return;}try{const d=await pointageApi(`/api/project-management/pointage?affaire_id=${encodeURIComponent(state.selectedId)}`);state.pointage=d;renderPointage();renderPlanningChart();}catch(e){console.warn(e);}}
...
async function init(){
  try{
    ...
    const urlId=String(params.get('affaire_id')||'').trim();
    const storedId=String(localStorage.getItem('selectedAffaireId')||'').trim();
    state.selectedId=urlId||storedId;
    if(urlId){localStorage.setItem('selectedAffaireId',urlId);} 
    state.selectedLabel=localStorage.getItem('selectedAffaireLabel')||'';
    await loadAffaires('');
    if(state.selectedId){
      const selected=resolveAffaireById(state.selectedId);
      if(selected){state.selectedLabel=selected.display_name||state.selectedLabel||'';localStorage.setItem('selectedAffaireLabel',state.selectedLabel||'');}
