...
async function loadAffaires(){const d=await api('/api/finance/affaires');state.affaires=d.items||[];const sel=document.getElementById('affaireSelect');if(!state.selectedId){sel.innerHTML="<option value=''>Projet verrouillé</option>";document.getElementById('searchInput').value='';return;}const found=resolveBoardAffaire(state.selectedId);if(found){state.selectedId=found.affaire_id||state.selectedId;sel.innerHTML=`<option value='${esc(found.affaire_id)}'>${esc(found.display_name)}</option>`;sel.value=found.affaire_id;document.getElementById('searchInput').value=found.display_name||'';}else{sel.innerHTML="<option value=''>Projet verrouillé</option>";document.getElementById('searchInput').value='';}}
