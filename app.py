...
function getMissionCatalog(board,pointage){const byKey={};const add=(key,label,tasksCount)=>{const mk=String(key||'').trim();const normalized=mk==='__default__'?'':mk;const lk=normalized||'__default__';if(!byKey[lk])byKey[lk]={mission_key:normalized,label:String(label||'').trim()||'Projet principal',tasks_count:0};byKey[lk].tasks_count=Math.max(Number(byKey[lk].tasks_count||0),Number(tasksCount||0));};add('', 'Projet principal', 0);for(const m of ((board&&board.finance_missions)||[])){if(!m||typeof m!=='object')continue;add(m.mission_key||'',m.label||'Mission',0);}for(const m of ((pointage&&pointage.missions)||[])){if(!m||typeof m!=='object')continue;add(m.mission_key||'',m.label||'Mission',m.tasks_count||0);}const items=Object.values(byKey);items.sort((a,b)=>{if(!a.mission_key&&b.mission_key)return -1;if(a.mission_key&&!b.mission_key)return 1;return String(a.label||'').localeCompare(String(b.label||''),'fr');});return items;}
function renderPlanningChart(){const sec=document.getElementById('sectionPlanningAnalysis');const root=document.getElementById('planningChartList');const title=document.getElementById('planningChartTitle');const pointage=state.pointage||{};const tasks=(pointage&&pointage.tasks)||[];const missions=(pointage&&pointage.missions)||[];const hasPlanning=missions.some(m=>Number(m.tasks_count||0)>0)||tasks.length>0;if(!sec||!root)return;if(!hasPlanning){sec.style.display='none';root.innerHTML="<div class='small'>Aucune donnée</div>";return;}sec.style.display='block';if(!tasks.length){root.innerHTML="<div class='small'>Un planning est chargé sur une autre mission. Sélectionnez-la dans Mission planning pour afficher l'analyse.</div>";return;}const mode=((document.getElementById('planningChartSelect')||{}).value)||'delay-trigger';if(mode==='delay-trigger'){if(title)title.textContent='Déclencheur de retard';renderDelayTriggerChart(root,tasks);return;}if(mode==='perimeter-progress'){if(title)title.textContent='Avancement par périmètre';renderPerimeterProgressChart(root,tasks);return;}if(mode==='resource-delay-today'){if(title)title.textContent='Retard moyen par ressource à date';renderPointageAnalytics(root,tasks,{includeUnpointed:true});return;}if(mode==='planning-progress'){if(title)title.textContent='Avancement planning';renderPlanningProgressComparison(root,tasks);return;}if(mode==='cst-chronology'){if(title)title.textContent='Chronologie macro des tâches CST';renderCstChronology(root,tasks);return;}if(title)title.textContent='Retard moyen par ressource';renderPointageAnalytics(root,tasks,{includeUnpointed:false});}
...
function renderPlanningMissionSelect(board){const select=document.getElementById('planningMissionSelect');if(!select)return;const missions=getMissionCatalog(board,state.pointage);const selected=state.planningMissionKey||'';select.innerHTML=missions.map(m=>`<option value='${escAttr(m.mission_key||'')}' ${(m.mission_key||'')===selected?'selected':''}>${esc(m.label||'Mission')}</option>`).join('');}
...
function renderMissionsPlanningTable(){const wrap=document.getElementById('missionsPlanningWrap');if(!wrap)return;const missions=getMissionCatalog(state.board||{},state.pointage||{});if(!missions.length){wrap.innerHTML="<div class='small' style='padding:10px'>Aucune mission disponible.</div>";return;}wrap.innerHTML=`<table><thead><tr><th>Mission</th><th>Planning chargé</th><th style='width:210px'>Action</th></tr></thead><tbody>${missions.map(m=>{const key=String(m.mission_key||'');const loaded=Number(m.tasks_count||0)>0;const badge=loaded?`<span class='status-badge done'>${Number(m.tasks_count||0)} tâche(s)</span>`:`<span class='status-badge soon'>Aucun planning</span>`;const active=(key===(state.planningMissionKey||''))?" style='font-weight:800'":'';return `<tr${active}><td>${esc(m.label||'Mission')}</td><td>${badge}</td><td><button type='button' class='btn btn-import-mission' data-mission='${escAttr(key)}' style='height:34px'>Importer planning CSV</button></td></tr>`;}).join('')}</tbody></table>`;Array.from(wrap.querySelectorAll('.btn-import-mission')).forEach(btn=>btn.addEventListener('click',()=>{const mission=btn.getAttribute('data-mission')||'';const input=document.getElementById('planningCsvInput');if(!input)return;input.dataset.missionKey=mission;input.click();}));}
...
async function loadPointage(){if(!state.selectedId)return;try{const d=await pointageApi(`/api/project-management/pointage?affaire_id=${encodeURIComponent(state.selectedId)}${pointageMissionQuery()}`);state.pointage=d;renderPlanningMissionSelect(state.board||{});renderMissionsPlanningTable();renderPointage();renderPlanningChart();}catch(e){console.warn(e);}}
