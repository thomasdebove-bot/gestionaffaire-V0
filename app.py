...
<style>
:root{--line:#dfe5ef;--ink:#122033;--muted:#6e7a90;--accent:#ef8d00;--panel:#fff;--bg:#f3f6fb;--soft:#f8fbff}
*{box-sizing:border-box}body{margin:0;font-family:Inter,Segoe UI,Arial,sans-serif;background:#fff;color:var(--ink)}
.wrap{max-width:560px;margin:0 auto;padding:26px 18px 34px}.board-top{display:flex;justify-content:space-between;align-items:flex-start;gap:12px}.board-brand{display:flex;align-items:center;gap:10px}.board-brand img{width:18px;height:36px;object-fit:contain}.board-kicker{font-size:10px;font-weight:800;letter-spacing:.08em;color:var(--accent);text-transform:uppercase}.action-row{display:flex;gap:6px;flex-wrap:wrap}.action-btn{height:22px;padding:0 8px;border:1px solid #1d1d1d;border-radius:999px;background:#fff;font-size:8px;font-weight:800;cursor:pointer}.title{margin:14px 0 10px;font-size:24px;font-weight:900;letter-spacing:.01em}.hero{display:grid;grid-template-columns:86px 1fr;gap:10px;align-items:start}.hero img{width:86px;height:52px;border-radius:8px;object-fit:cover;border:1px solid var(--line)}.hero-name{font-size:13px;font-weight:900}.hero-desc{font-size:8px;color:var(--muted);margin-top:3px;line-height:1.35}.section-label{margin:10px 0 4px;font-size:10px;font-weight:900;color:var(--accent);text-transform:uppercase}.pill-row{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.pill-row.two{grid-template-columns:repeat(2,minmax(0,1fr))}.pill{border:1px solid #20242d;border-radius:12px;padding:8px 10px;background:#fff;min-height:42px}.pill .label{font-size:8px;font-weight:800;text-transform:uppercase;color:#111}.pill .value{margin-top:4px;font-size:11px;font-weight:900}.layout-controls{display:flex;justify-content:space-between;align-items:center;gap:8px;margin:14px 0 6px}.layout-title{font-size:10px;font-weight:900;color:var(--accent);text-transform:uppercase}.toggle-list{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}.toggle-chip{display:inline-flex;align-items:center;gap:4px;border:1px solid #1d1d1d;border-radius:999px;padding:2px 6px;background:#fff;font-size:8px;font-weight:800}.toggle-chip input{width:10px;height:10px;margin:0}.board-section{margin-top:8px;border:1px solid var(--line);border-radius:10px;background:#fff;padding:6px}.board-section.hidden{display:none}.board-section-head{display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px}.board-section-head h3{margin:0;font-size:9px;font-weight:900;text-transform:uppercase}.notes-badge{height:16px;padding:0 7px;border:1px solid #1d1d1d;border-radius:999px;background:#fff;font-size:7px;font-weight:800}.panel{border:1px solid #e6ecf6;border-radius:8px;background:var(--soft);padding:6px}.mini-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.mini-card{background:#fff;border:1px solid #e3e9f4;border-radius:6px;padding:6px;min-height:42px}.mini-card .label{font-size:6px;font-weight:800;color:#68758b;text-transform:uppercase}.mini-card .value{margin-top:4px;font-size:17px;font-weight:900}.mini-list{margin-top:6px;display:grid;gap:4px}.mini-row{display:flex;justify-content:space-between;gap:8px;padding:6px 8px;border:1px solid #e6ecf6;border-radius:8px;background:#fff;font-size:8px}.mini-row .name{font-weight:900}.bar-chart{display:grid;gap:6px}.bar-row{display:grid;grid-template-columns:minmax(0,1fr) 190px 28px;align-items:center;gap:8px;font-size:8px}.bar-track{height:3px;border-radius:999px;background:#edf1f8;overflow:hidden}.bar-fill{height:100%;background:linear-gradient(90deg,#ef8d00,#ffb649)}.vbars{display:grid;grid-template-columns:repeat(auto-fit,minmax(52px,1fr));gap:8px;align-items:end;height:86px;margin-top:8px}.vbar{display:flex;flex-direction:column;align-items:center;justify-content:flex-end;gap:4px}.vbar-stick{width:10px;border-radius:4px 4px 0 0;background:#df5454;min-height:4px}.vbar-stick.cst{background:#ef8d00}.vbar-label{font-size:6px;text-align:center;font-weight:800}.line-legend{display:flex;gap:10px;align-items:center;flex-wrap:wrap;font-size:7px;font-weight:800;margin-bottom:4px}.dot{display:inline-block;width:8px;height:2px;border-radius:999px}.dot.orange{background:#ef8d00}.dot.blue{background:#9db4de}.dot.green{background:#239b5b}.finance-svg{width:100%;height:112px;display:block;border:1px solid #eef2f8;border-radius:8px;background:#fff}.summary-row{display:grid;grid-template-columns:116px 1fr 58px;gap:6px;align-items:center;font-size:7px;margin-top:6px}.summary-track{height:3px;border-radius:999px;background:#eaf0f8;overflow:hidden}.summary-fill.pre{background:#9db4de}.summary-fill.fac{background:#ef8d00}.summary-fill.prod{background:#239b5b}.board-table{width:100%;border-collapse:collapse;font-size:7px}.board-table th,.board-table td{padding:5px 6px;border-bottom:1px solid #e7edf7;text-align:left}.board-table th{font-size:6px;text-transform:uppercase;color:#71809a}.notes-area{width:100%;min-height:72px;border:1px dashed #cfd8e8;border-radius:8px;padding:8px;font:inherit;font-size:8px;resize:vertical;background:#fff;margin-top:6px}.notes-help{font-size:7px;color:var(--muted);margin-top:4px}.hide-print{display:block}
@media print{body{background:#fff}.wrap{max-width:none;padding:0}.hide-print{display:none !important}.board-section.hidden{display:none !important}.action-row{display:none}.notes-area{border:1px solid #dfe5ef;resize:none;min-height:92px}.board-section{break-inside:avoid-page}.panel{break-inside:avoid-page}}
</style></head>
<body><div class='wrap'>
  <div class='board-top'>
    <div class='board-brand'><img src='/assets/logo-tempo' alt='Tempo'><div class='board-kicker'>BOARD CLIENT DU JJ/XX/AA</div></div>
    <div class='action-row hide-print'><button id='exportPdfBtn' class='action-btn'>EXPORT</button><button id='printBtn' class='action-btn'>IMPRIMER</button><button id='closeBtn' class='action-btn'>FERMER</button></div>
  </div>
  <div id='boardTitle' class='title'>CLIENT - NOM DU PROJET</div>
  <div class='hero'><img id='projectImage' alt='Projet'><div><div id='projectName' class='hero-name'>VALHUBERT</div><div id='projectDesc' class='hero-desc'>Réhabilitation d'un immeuble de bureaux à destination d'Essilor Luxottica.</div></div></div>
  ...
  <div class='layout-controls hide-print'>
    <div class='layout-title'>Gestion de projet | composition du board</div>
  ...
  <section id='section-metronome' class='board-section'>
    <div class='board-section-head'><h3>Gestion de projet | METRONOME</h3><button class='notes-badge hide-print'>+ notes</button></div>
    <div class='panel'>
      <div class='mini-grid'>
        ...
        <div class='mini-card'><div class='label'>Date de référence</div><div id='metroDate' class='value' style='font-size:13px'>-</div></div>
      </div>
      <div class='mini-list' id='metroCompanies'><div class='mini-row'><span class='name'>Chargement</span><span>…</span></div></div>
    </div>
  </section>

  <section id='section-analysis' class='board-section'>
    <div class='board-section-head'><h3>Niveau 2 — Analyse METRONOME</h3><button class='notes-badge hide-print'>+ notes</button></div>
    <div class='panel'>
      <div id='analysisBars' class='bar-chart'></div>
      <div style='margin-top:10px;font-size:7px;font-weight:800;color:#68758b;text-transform:uppercase'>Retard moyen par entreprise suivant réunion METRONOME</div>
      <div id='analysisVbars' class='vbars'></div>
    </div>
  </section>

  <section id='section-planning' class='board-section'>
    <div class='board-section-head'><h3>Gestion de projet | PLANNING</h3><button class='notes-badge hide-print'>+ notes</button></div>
    <div class='panel'>
      <div style='font-size:7px;font-weight:900;margin-bottom:4px'>Niveau 3 — Analyse planning</div>
      <div id='planningBars' class='vbars'></div>
    </div>
  </section>

  <section id='section-finance' class='board-section'>
    <div class='board-section-head'><h3>Gestion financière</h3><button class='notes-badge hide-print'>+ notes</button></div>
    <div class='panel'>
      <div class='line-legend'><span><span class='dot orange'></span> Facturation</span><span><span class='dot blue'></span> Prévisionnel</span><span><span class='dot green'></span> Production</span></div>
      <svg id='financeChart' class='finance-svg' viewBox='0 0 360 112' preserveAspectRatio='none'></svg>
      <div id='financeSummary'></div>
    </div>
  </section>

  <section id='section-notes' class='board-section'>
    <div class='board-section-head'><h3>Notes board</h3><button class='notes-badge hide-print'>éditable</button></div>
    <div class='panel'>
      <textarea id='notes' class='notes-area' placeholder='Messages clés, risques, arbitrages, points à porter à la connaissance du client…'></textarea>
      <div class='notes-help'>Les notes sont mémorisées localement pour cette affaire et imprimées avec le board.</div>
    </div>
  </section>
</div>
<script>
const MONTHS=['janvier','fevrier','mars','avril','mai','juin','juillet','aout','septembre','octobre','novembre','decembre'];
const MONTH_LABELS={janvier:'Janv',fevrier:'Fév',mars:'Mars',avril:'Avr',mai:'Mai',juin:'Juin',juillet:'Juil',aout:'Août',septembre:'Sept',octobre:'Oct',novembre:'Nov',decembre:'Déc'};
...
function parseDateOnly(v){const raw=String(v||'').trim();if(!raw)return null;const s=raw.split(' ')[0].split('T')[0];if(/^\d{4}-\d{2}-\d{2}$/.test(s)){const d=new Date(`${s}T00:00:00`);return Number.isNaN(d.getTime())?null:d;}return null;}
function fmtDate(v){const d=parseDateOnly(v);return d?d.toLocaleDateString('fr-FR',{day:'2-digit',month:'2-digit',year:'2-digit'}):'-';}
function normalizeText(v){return String(v||'').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g,'').trim();}
function saveNotes(){localStorage.setItem(`board-notes::${affaireId}`,(document.getElementById('notes')||{}).value||'');}
function restoreNotes(){const el=document.getElementById('notes');if(el)el.value=localStorage.getItem(`board-notes::${affaireId}`)||'';}
function saveLayout(){const config={};document.querySelectorAll('[data-section]').forEach(inp=>config[inp.getAttribute('data-section')]=inp.checked);localStorage.setItem(`board-layout::${affaireId}`,JSON.stringify(config));applyLayout();}
function restoreLayout(){let config={};try{config=JSON.parse(localStorage.getItem(`board-layout::${affaireId}`)||'{}')||{};}catch(_){config={};}document.querySelectorAll('[data-section]').forEach(inp=>{const key=inp.getAttribute('data-section');inp.checked=(config[key]!==false);inp.addEventListener('change',saveLayout);});applyLayout();}
function applyLayout(){document.querySelectorAll('[data-section]').forEach(inp=>{const key=inp.getAttribute('data-section');const sec=document.getElementById(`section-${key}`);if(sec)sec.classList.toggle('hidden',!inp.checked);});}
function renderMetronome(board){const p=(board&&board.kpis_pilotage)||{};setText('metroReminder',String(Number(p.rappels_ouverts_a_date||p.reminders_open_count||0)));setText('metroFollow',String(Number(p.a_suivre_ouverts||p.followups_open_count||0)));setText('metroDate',p.date_reference||p.reference_date||'-');const companies=(p.rappels_cumules_par_entreprise||p.reminders_by_company||[]);document.getElementById('metroCompanies').innerHTML=companies.length?companies.slice(0,4).map(r=>`<div class='mini-row'><span class='name'>${esc(r.name||r.company||'-')}</span><span>${Number(r.count||0)} rappel(s)</span></div>`).join(''):`<div class='mini-row'><span class='name'>Aucune donnée</span><span>0</span></div>`;}
function renderAnalysis(board){const p=(board&&board.kpis_pilotage)||{};const companyItems=((p.project_company_views||{}).reminder||[]).slice(0,5);const max=Math.max(1,...companyItems.map(x=>Number(x.count||0)));document.getElementById('analysisBars').innerHTML=companyItems.length?companyItems.map(x=>`<div class='bar-row'><div style='font-weight:900'>${esc(x.name||'-')}</div><div class='bar-track'><div class='bar-fill' style='width:${(Number(x.count||0)/max)*100}%'></div></div><div style='text-align:right;font-weight:900'>${Number(x.count||0)}</div></div>`).join(''):`<div class='mini-row'><span class='name'>Aucune donnée</span><span>0</span></div>`;const react=(p.reactivity_by_company||[]).slice(0,6);const maxReact=Math.max(1,...react.map(x=>Math.abs(Number(x.avg_gap_days||0))));document.getElementById('analysisVbars').innerHTML=react.length?react.map(x=>{const val=Number(x.avg_gap_days||0);const h=Math.max(6,(Math.abs(val)/maxReact)*42);return `<div class='vbar'><div style='font-size:7px;font-weight:900'>${val.toFixed(0)}</div><div class='vbar-stick ${normalizeText(x.name)==='cst'?'cst':''}' style='height:${h}px'></div><div class='vbar-label'>${esc(x.name||'-')}</div></div>`;}).join(''):`<div class='mini-row'><span class='name'>Aucune donnée</span><span>0</span></div>`;}
function renderPlanning(pointage){const tasks=((pointage&&pointage.tasks)||[]).filter(t=>!t.is_summary);const perimeters={};for(const t of tasks){const key=String(t.perimeter||'Général').trim()||'Général';const current=perimeters[key]||{label:key,delay:0,count:0};current.delay=Math.max(current.delay,Number(t.delayDays||0));current.count+=1;perimeters[key]=current;}const rows=Object.values(perimeters).sort((a,b)=>b.delay-a.delay||b.count-a.count).slice(0,7);const max=Math.max(1,...rows.map(x=>Math.max(1,x.delay||x.count)));document.getElementById('planningBars').innerHTML=rows.length?rows.map(x=>{const h=Math.max(8,(Math.max(1,x.delay||x.count)/max)*40);const cls=normalizeText(x.label).includes('cst')?'cst':'';return `<div class='vbar'><div style='font-size:7px;font-weight:900'>${x.delay||0} j</div><div class='vbar-stick ${cls}' style='height:${h}px'></div><div class='vbar-label'>${esc(x.label)}</div></div>`;}).join(''):`<div class='mini-row'><span class='name'>Aucune donnée</span><span>0</span></div>`;}
function renderFinance(affaire){const billed=Number(affaire.facturation_totale||((affaire.anteriorite||0)+(affaire.facture_2026||affaire.facturation_cumulee_2026||0)));const cmd=Number(affaire.commande_ht||0);const remain=Number(affaire.reste_a_facturer||0);const data=MONTHS.map(m=>({label:MONTH_LABELS[m],pre:Number((((affaire.mensuel||{})[m]||{}).previsionnel)||0),fac:Number((((affaire.mensuel||{})[m]||{}).facture)||0),prod:Number(((affaire.production_cumulee_mensuelle||{})[m])||0)}));let cp=0,cf=Number(affaire.anteriorite||0);const cum=data.map(d=>{cp+=d.pre;cf+=d.fac;return {l:d.label,pre:cp,fac:cf,prod:d.prod};});const max=Math.max(1,...cum.flatMap(x=>[x.pre,x.fac,x.prod]));const left=24,top=10,width=320,height=72,step=width/Math.max(1,cum.length),barW=Math.max(5,Math.min(11,step*0.35));let out='';for(let i=0;i<=3;i++){const y=top+(height/3)*i;out+=`<line x1='${left}' y1='${y}' x2='${left+width}' y2='${y}' stroke='#edf2f8'/>`;}cum.forEach((d,i)=>{const x=left+i*step+step*0.5;const preH=(d.pre/max)*height;const facH=(d.fac/max)*height;out+=`<rect x='${x-barW-1}' y='${top+height-preH}' width='${barW}' height='${Math.max(0,preH)}' rx='2' fill='#9db4de'/>`;out+=`<rect x='${x+1}' y='${top+height-facH}' width='${barW}' height='${Math.max(0,facH)}' rx='2' fill='#ef8d00'/>`;if(i%2===0)out+=`<text x='${x}' y='${top+height+10}' fill='#66768f' font-size='5' text-anchor='middle'>${d.l}</text>`;});const prodPts=cum.map((d,i)=>`${left+i*step+step*0.5},${top+height-(d.prod/max)*height}`).join(' ');out+=`<polyline points='${prodPts}' fill='none' stroke='#239b5b' stroke-width='1.6' stroke-dasharray='3 2'/>`;document.getElementById('financeChart').innerHTML=out;const maxFinal=Math.max(cmd,billed,remain,1);document.getElementById('financeSummary').innerHTML=[['Prévisionnel annuel',cmd,'pre'],['Facturation cumulée',billed,'fac'],['Reste à facturer',remain,'prod']].map(r=>`<div class='summary-row'><div>${r[0]}</div><div class='summary-track'><div class='summary-fill ${r[2]}' style='height:100%;width:${Math.min(100,(Number(r[1]||0)/maxFinal)*100)}%'></div></div><div style='text-align:right;font-weight:900'>${euro(r[1])}</div></div>`).join('');}
async function init(){restoreLayout();restoreNotes();document.getElementById('notes').addEventListener('input',saveNotes);document.getElementById('printBtn').addEventListener('click',()=>window.print());document.getElementById('closeBtn').addEventListener('click',()=>window.close());document.getElementById('exportPdfBtn').addEventListener('click',()=>window.print());if(!affaireId){setText('boardTitle','Aucune affaire sélectionnée');return;}const finance=await api(`/api/finance/affaire/${encodeURIComponent(affaireId)}`);let board={};let pointage={};try{board=await api(`/api/project-management/board?affaire_id=${encodeURIComponent(affaireId)}`);}catch(_){board={};}try{pointage=await api(`/api/project-management/pointage?affaire_id=${encodeURIComponent(affaireId)}`);}catch(_){pointage={};}const affaire=(finance&&finance.affaire)||{};const billed=Number(affaire.facturation_totale||((affaire.anteriorite||0)+(affaire.facture_2026||affaire.facturation_cumulee_2026||0)));const budget=Number(affaire.commande_ht||0);const billedPct=budget>0?(billed/budget)*100:0;setText('boardTitle',`${(affaire.client||'CLIENT').toUpperCase()} - ${String(affaire.affaire||affaire.display_name||affaireId).toUpperCase()}`);setText('projectName',String(affaire.display_name||affaire.affaire||affaireId).toUpperCase());setText('projectDesc',affaire.project_description||affaire.start_sentence||affaire.affaire||'-');const img=document.getElementById('projectImage');const src=String(affaire.project_image||affaire.image_url||affaire.image||'').trim();if(src){img.src=src;}else{img.style.display='none';}setText('pillBudget',euro(affaire.commande_ht||0));setText('pillRemain',euro(affaire.reste_a_facturer||0));setText('pillBilledPct',`${billedPct.toFixed(1)} %`);setText('pillPlannedPct',`${Number((((board||{}).analytics||{}).kpi_project_progress||{}).calendar_progress_pct||0).toFixed(1)} %`);setText('pillPointedPct',`${(Number(affaire.pointage_progress_ratio||0)*100).toFixed(1)} %`);renderMetronome(board);renderAnalysis(board);renderPlanning(pointage);renderFinance(affaire);}
init().catch(err=>{setText('boardTitle',err.message||'Erreur de chargement');});
</script></body></html>"""
