...
def pointage_finance_summary(affaire_id: str, commande_ht: float) -> Dict[str, float]:
    ...
    progress_amount = clean_number(commande_ht) * planning_progress_ratio if clean_number(commande_ht) > 0 else 0.0
    return {
...
<div class='container'>
  ...
  <div id='pointagePlanningFinanceSection' class='section' style='display:none'>
    ...
    <div class='chart-controls'>
      ...
      <select id='pointageFinanceChartSelect' class='chart-select'>
        <option value='planned-vs-pointed'>Coût planifié à date vs coût pointé à date</option>
    ...
    <div id='pointageFinanceChartTitle' class='small' style='font-weight:800;margin-bottom:8px'>Coût planifié à date vs coût pointé à date</div>
...
function renderPointageFinanceChart(){const sec=document.getElementById('pointagePlanningFinanceSection');const root=document.getElementById('pointageFinanceChart');const title=document.getElementById('pointageFinanceChartTitle');const a=state.selectedAffaire||{};const boond=(state.boondImputation||{}).totals||{};const taskCount=Number(a.pointage_task_count||0);if(!sec||!root)return;if(taskCount<=0){sec.style.display='none';root.innerHTML="<div class='small'>Aucune donnée de pointage planning.</div>";return;}sec.style.display='block';const mode=((document.getElementById('pointageFinanceChartSelect')||{}).value)||'planned-vs-pointed';const rows=mode==='pointed-vs-boond'?[{label:'Coût pointé à date',value:Number(a.pointage_cost_actual_cst||0),color:'#ef8d00',subtitle:'Issu du pointage des tâches'},{label:'Coût de production BOOND',value:Number(boond.total_cost||0),color:'#1b6ef3',subtitle:'Productivité projet BOOND'}]:[{label:'Coût planifié à date',value:Number(a.pointage_cost_planned_cst||0),color:'#5da2ff',subtitle:'Somme des coûts CST planifiés'},{label:'Coût pointé à date',value:Number(a.pointage_cost_actual_cst||0),color:'#ef8d00',subtitle:'Somme des coûts CST pointés'}];if(title)title.textContent=mode==='pointed-vs-boond'?'Coût pointé à date vs coût de production BOOND':'Coût planifié à date vs coût pointé à date';const max=Math.max(1,...rows.map(r=>Number(r.value||0)));root.innerHTML=`<div style='display:grid;grid-template-columns:repeat(${rows.length},minmax(220px,1fr));gap:14px'>${rows.map(r=>{const width=Math.max(4,(Number(r.value||0)/max)*100);return `<div class='pilot-row' style='display:block'><div style='display:flex;justify-content:space-between;gap:12px;align-items:flex-end'><div><div style='font-weight:800'>${esc(r.label)}</div><div class='small'>${esc(r.subtitle)}</div></div><div style='font-weight:900'>${euro(r.value||0)}</div></div><div style='margin-top:10px;height:14px;border-radius:999px;background:#edf1f8;overflow:hidden'><div style='height:100%;width:${width}%;background:${r.color}'></div></div></div>`;}).join('')}</div>`;}
...
function renderPerimeterProgressChart(root,tasks){if(!root)return;const items=groupTasksByPerimeter(tasks).map(group=>{const actionable=(group.tasks||[]).filter(t=>!t.is_summary);if(!actionable.length)return null;const avg=actionable.reduce((sum,t)=>sum+Number(t.progress||0),0)/actionable.length;const blocker=nextBlockingTaskForPerimeter(actionable);return {label:group.label,value:Math.max(0,Math.min(100,avg)),blocker};}).filter(Boolean).sort((a,b)=>b.value-a.value||a.label.localeCompare(b.label,'fr'));if(!items.length){root.innerHTML="<div class='small'>Aucune donnée de périmètre disponible.</div>";return;}root.innerHTML=items.map(x=>{const tooltip=x.blocker&&x.value>0&&x.value<100?`Tâche suivante : ${x.blocker.name} · retard ${x.blocker.delay} j · lot ${x.blocker.owner}`:(x.value>=100?'Périmètre terminé':'Aucune tâche bloquante identifiée');const blockerHtml=x.blocker&&x.value>0&&x.value<100?`<div style='margin-top:8px;padding:8px 10px;border-radius:10px;background:#f6f9ff;border:1px solid #d6e4ff;color:#21406f'><span style='font-weight:800'>Tâche suivante :</span> ${esc(x.blocker.name)} <span style='margin-left:8px;font-weight:700;color:${x.blocker.delay>0?'#d64545':'#5f6f88'}'>${x.blocker.delay>0?`Retard ${x.blocker.delay} j`:'À l\'heure'}</span> <span style='margin-left:8px;color:#5f6f88'>Lot ${esc(x.blocker.owner)}</span></div>`:'';return `<div style='margin:10px 0'><div style='display:flex;justify-content:space-between;gap:12px;margin-bottom:6px'><span style='font-weight:700'>${esc(x.label)}</span><strong>${x.value.toFixed(1)}%</strong></div><div title='${escAttr(tooltip)}' style='height:14px;border-radius:999px;background:#edf1f8;overflow:hidden;cursor:help'><div style='height:100%;width:${x.value}%;background:linear-gradient(90deg,#1b6ef3,#5da2ff)'></div></div>${blockerHtml}</div>`;}).join('');}
