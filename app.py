...
function renderPointage(){const root=document.getElementById('pointageWrap');const d=state.pointage||{};const tasks=d.tasks||[];if(!root)return;if(!tasks.length){root.innerHTML="<div class='small' style='padding:10px'>Aucun planning importé.</div>";return;}const expanded=new Set((d.workState&&d.workState.expanded_levels)||[]);const compact=(d.workState&&d.workState.compact_mode)!==false;const groups=levelGroups(tasks);let html=`<div style='padding:8px 10px;border-bottom:1px solid #dbe4f2;background:#f6f8fc;display:flex;justify-content:space-between;align-items:center'><strong>Mode compact</strong><label style='display:flex;gap:8px;align-items:center'><input id='pointageCompactToggle' type='checkbox' ${compact?'checked':''}> Afficher uniquement Tâche / Ressource / % achevé / Statut / Retard</label></div>`;html+=`<table class='${compact?'compact':''}'><thead><tr><th>Tâche</th><th class='col-zone'>Zone</th><th>Ressource</th><th class='col-start'>Début</th><th class='col-end'>Fin</th><th>% achevé</th><th class='col-real'>Réception réelle</th><th>Retard</th><th class='col-plan'>Coût planifié CST</th><th class='col-actual'>Coût pointé CST</th><th>Statut</th></tr></thead><tbody>`;for(const lvl of Object.keys(groups)){const open=expanded.has(lvl);html+=`<tr class='lvl-row'><td colspan='11'><button type='button' data-lvl='${esc(lvl)}' class='btn' style='height:28px'>${open?'−':'+'}</button> ${esc(lvl)}</td></tr>`;if(open){for(const t of groups[lvl]){const isSummary=Boolean(t.is_summary);const rowCls=isSummary?'lvl-row':(t.isLate?'task-late':(((t.status==='future')&&t.start)?'task-soon':''));const label=t.name;const start=fmtDateFrShort(t.start||'');const end=fmtDateFrShort(t.end||'');const depth=Math.max(0,Number(t.depth||0));const pad=12+(depth*14);if(isSummary){html+=`<tr class='${rowCls}'><td style='padding-left:${pad}px;font-weight:800'>${esc(t.name)}</td><td class='col-zone'>${esc(t.level_label||t.level||'')}</td><td>${esc(t.owner||'')}</td><td class='col-start'>${esc(start)}</td><td class='col-end'>${esc(end)}</td><td>${Number(t.progress||0).toFixed(0)}%</td><td class='col-real'></td><td></td><td class='col-plan'></td><td class='col-actual'></td><td>summary</td></tr>`;continue;}html+=`<tr class='${rowCls}'><td style='padding-left:${pad}px'>${esc(label)}</td><td class='col-zone'>${esc(t.level_label||t.level||'')}</td><td>${esc((t.owner==='Non attribué'?'':t.owner))}</td><td class='col-start'>${esc(start)}</td><td class='col-end'>${esc(end)}</td><td><input data-task='${esc(t.task_id)}' data-field='progress' type='number' min='0' max='100' value='${Number(t.progress||0)}' style='width:70px'></td><td class='col-real'><input data-task='${esc(t.task_id)}' data-field='actualEnd' type='date' value='${esc(t.actualEnd||'')}'></td><td>${Number(t.delayDays||0)} j</td><td class='col-plan' style='color:#b45f06;font-weight:800'>${Number(t.plannedCostCst||0)>0?Number(t.plannedCostCst||0).toFixed(0)+' €':''}</td><td class='col-actual'>${Number(t.actualCostCst||0)>0?Number(t.actualCostCst||0).toFixed(0)+' €':''}</td><td>${t.status==='past'?'<span style="color:#1f7a3f;font-weight:800">Terminé</span>':(t.status==='future'?'<span style="color:#8a94a8;font-weight:800">À venir</span>':'<span style="color:#1b6ef3;font-weight:800">En cours</span>')}</td></tr>`;if(t.is_cet&&t.cetMembers&&t.cetMembers.length){for(const m of t.cetMembers){const cm=((t.cet||{})[m]||{});html+=`<tr><td style='padding-left:${pad+16}px'>↳ ${esc(m)}</td><td class='col-zone'>${esc(t.level||'')}</td><td>CET</td><td class='col-start'></td><td class='col-end'></td><td><input data-task='${esc(t.task_id)}' data-cet='${esc(m)}' data-field='progress' type='number' min='0' max='100' value='${Number(cm.progress||0)}' style='width:70px'></td><td class='col-real'><input data-task='${esc(t.task_id)}' data-cet='${esc(m)}' data-field='actualEnd' type='date' value='${esc(cm.actualEnd||'')}'></td><td colspan='4'></td></tr>`;}}}}}html+='</tbody></table>';root.innerHTML=html;const style=document.getElementById('pointageCompactStyle')||document.createElement('style');style.id='pointageCompactStyle';style.textContent=`#pointageWrap table.compact .col-zone,#pointageWrap table.compact .col-start,#pointageWrap table.compact .col-end,#pointageWrap table.compact .col-real,#pointageWrap table.compact .col-plan,#pointageWrap table.compact .col-actual{display:none;}`;document.head.appendChild(style);const toggle=document.getElementById('pointageCompactToggle');if(toggle){toggle.addEventListener('change',async()=>{const ws={...(d.workState||{}),compact_mode:Boolean(toggle.checked)};await savePointage({},ws);});}Array.from(root.querySelectorAll('button[data-lvl]')).forEach(b=>b.addEventListener('click',async()=>{const lvl=b.getAttribute('data-lvl')||'';const ws={...(d.workState||{}),expanded_levels:[...new Set([...(d.workState?.expanded_levels||[])]) ]};const set=new Set(ws.expanded_levels);if(set.has(lvl))set.delete(lvl);else set.add(lvl);ws.expanded_levels=[...set];await savePointage({},ws);}));Array.from(root.querySelectorAll('input[data-task]')).forEach(inp=>inp.addEventListener('change',async()=>{const task=inp.getAttribute('data-task')||'';const field=inp.getAttribute('data-field')||'';const cet=inp.getAttribute('data-cet')||'';const patch={};if(cet){patch[task]={cet:{[cet]:{[field]:inp.value}}};}else{patch[task]={[field]:inp.value};}await savePointage(patch,d.workState||{});}));}
...
<style>
:root{--accent:#ef8d00;--ok:#239b5b;--warn:#d08a1a;--bad:#cc4747;--ink:#1e2b40;--muted:#66758e;--line:#d8e1ee;--bg:#f4f7fb;--pilot:#fff7ea;--analysis:#fff;--ops:#f2f5f9}
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font-family:Inter,Segoe UI,Arial,sans-serif}
.wrap{max-width:1420px;margin:20px auto;padding:0 16px}.card{border:1px solid var(--line);border-radius:18px;padding:16px;margin-top:12px}.level-1{background:var(--pilot)}.level-2{background:var(--analysis)}.level-3{background:var(--ops)}
.toolbar{display:flex;gap:8px;flex-wrap:wrap;align-items:center}.search,.select,.btn{height:40px;border-radius:10px}.search,.select{border:1px solid var(--line);padding:0 10px;min-width:220px}.btn{border:none;background:var(--accent);color:#fff;font-weight:800;padding:0 12px;text-decoration:none;display:inline-flex;align-items:center;cursor:pointer}
.title{font-size:34px;font-weight:900;margin:8px 0 4px}.subtitle{color:var(--muted)}.eyeb{font-size:12px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:#b66b00}
.grid{display:grid;gap:10px}.kpis{grid-template-columns:repeat(5,minmax(0,1fr))}.health-grid{grid-template-columns:2fr 1fr}
.k{border:1px solid var(--line);border-radius:14px;background:#fff;padding:12px}.k .l{font-size:11px;text-transform:uppercase;color:var(--muted);font-weight:700}.k .v{font-size:30px;font-weight:900;margin-top:6px}.k .s{color:var(--muted);font-size:12px;margin-top:4px}
.badge{display:inline-flex;align-items:center;padding:7px 11px;border-radius:999px;font-size:12px;font-weight:800;margin:3px 6px 3px 0}.badge.ok{background:#eaf9f0;color:var(--ok)}.badge.warn{background:#fff5e7;color:var(--warn)}.badge.bad{background:#ffecec;color:var(--bad)}
.score{display:flex;align-items:center;justify-content:center;height:120px;border:1px solid var(--line);border-radius:16px;background:#fff;font-size:38px;font-weight:900}
.score.ok{color:var(--ok)}.score.warn{color:var(--warn)}.score.bad{color:var(--bad)}
svg{width:100%;height:340px;background:linear-gradient(180deg,#fcfdff,#f5f8fe);border:1px solid var(--line);border-radius:14px}
.legend{font-size:12px;color:var(--muted);display:flex;gap:16px;margin-top:8px}
.small{font-size:12px;color:var(--muted)} table{width:100%;border-collapse:collapse}th,td{padding:9px;border-bottom:1px solid #e8edf5;text-align:left}td.num,th.num{text-align:right}
.costbar{margin-top:5px;height:7px;background:#edf2fa;border-radius:999px;overflow:hidden}.costfill{height:100%;background:#ef8d00}
@media (max-width:1100px){.kpis{grid-template-columns:repeat(2,minmax(0,1fr))}.health-grid{grid-template-columns:1fr}}
</style></head>
<body><div class='wrap'>
  <div class='card level-1'>
    <div class='toolbar'>
      <a class='btn' href='/'>Accueil</a><a id='financeBtn' class='btn' href='/finance'>Finances</a><a id='pmBtn' class='btn' href='/gestion-projet'>Gestion projet</a><button id='exportBtn' class='btn' disabled>Exporter CSV</button>
      <input id='searchInput' class='search' type='search' placeholder='Rechercher une affaire'><select id='affaireSelect' class='select'></select>
    </div>
    <div class='eyeb'>Niveau 1 — Pilotage</div><div id='title' class='title'>Sélectionnez une affaire</div><div id='subtitle' class='subtitle'>Cockpit stratégique projet.</div>
  </div>

  <div class='card level-1'>
    <div class='grid health-grid'>
      <div>
        <h3 style='margin:0 0 8px'>DIAGNOSTIC PROJET</h3>
        <div id='diagnosticBadges'></div>
      </div>
      <div>
        <div class='small'>HEALTH SCORE</div>
        <div id='healthScore' class='score'>0 / 100</div>
      </div>
    </div>
  </div>

  <div class='card level-1'>
    <h3 style='margin:0 0 8px'>KPI de pilotage</h3>
    <div class='grid kpis'>
      <div class='k'><div class='l'>Commande / Budget</div><div id='kCommande' class='v'>0 €</div><div class='s'>Base de pilotage financier</div></div>
      <div class='k'><div class='l'>Facturé</div><div id='kFacture' class='v'>0 €</div><div id='kFactureSub' class='s'>0 % du budget</div></div>
      <div class='k'><div class='l'>Reste à facturer</div><div id='kReste' class='v'>0 €</div><div id='kResteSub' class='s'>0 % du budget</div></div>
      <div class='k'><div class='l'>Avancement financier</div><div id='kAvance' class='v'>0 %</div><div class='s'>Facturation / commande</div></div>
      <div class='k'><div class='l'>Marge projetée</div><div id='kMarge' class='v'>0 €</div><div class='s'>Facturé - coût imputé BOOND</div></div>
    </div>
  </div>

  <div class='card level-2'>
    <h3 style='margin:0 0 8px'>Prévisionnel vs facturation vs production cumulée</h3>
    <svg id='chart' viewBox='0 0 980 340'></svg>
    <div class='legend'><span>— Prévisionnel</span><span>— Facturation</span><span>— Production estimée</span></div>
    <div id='trajectory' class='small' style='margin-top:8px'></div>
  </div>

  <div class='card level-2'>
    <h3 style='margin:0 0 8px'>TRAJECTOIRE PROJET</h3>
    <div id='trajectoryRows' class='small'></div>
  </div>

  <div class='card level-3'>
    <h3 style='margin:0 0 8px'>Imputation BOOND</h3>
    <div id='boondTop' class='small'>-</div>
    <table id='boondTable'></table>
  </div>
</div>
<script>
const MONTHS=['janvier','fevrier','mars','avril','mai','juin','juillet','aout','septembre','octobre','novembre','decembre'];
...
const fmt=v=>Number(v||0).toLocaleString('fr-FR',{maximumFractionDigits:1});

async function api(url){const r=await fetch(url);const d=await r.json();if(!r.ok)throw new Error(d.detail||'Erreur API');return d;}
async function loadAffaires(search=''){const d=await api(`/api/finance/affaires?search=${encodeURIComponent(search||'')}`);state.affaires=d.items||[];const sel=document.getElementById('affaireSelect');sel.innerHTML="<option value=''>Sélectionner une affaire</option>"+state.affaires.map(a=>`<option value='${a.affaire_id}'>${a.display_name||a.affaire_id}</option>`).join('');}
async function loadSelected(id){state.selectedId=id;if(!id){state.selected=null;state.board=null;state.boond=null;render();return;}const sel=document.getElementById('affaireSelect');try{state.selected=await api(`/api/finance/affaire/${encodeURIComponent(id)}`);}catch(_){state.selected=null;state.board=null;state.boond=null;render();return;}if(sel&&!Array.from(sel.options).some(o=>o.value===id)){sel.insertAdjacentHTML('beforeend',`<option value='${id}'>${state.selected.display_name||id}</option>`);}if(sel)sel.value=id;try{state.board=await api(`/api/project-management/board?affaire_id=${encodeURIComponent(id)}`);}catch(_){state.board=null;}try{state.boond=await api(`/api/boond/imputations/by-project?project_name=${encodeURIComponent(state.selected.affaire||state.selected.display_name||'')}&affaire_id=${encodeURIComponent(id)}&affaire=${encodeURIComponent(state.selected.affaire||'')}&numero=${encodeURIComponent(state.selected.numero||'')}&client=${encodeURIComponent(state.selected.client||'')}&display_name=${encodeURIComponent(state.selected.display_name||'')}`);}catch(_){state.boond=null;}localStorage.setItem('selectedAffaireId',id);render();}

function scoreHealth(finPct,prodPct,resteRatio,alerts,rappels){let s=100;if(finPct>prodPct)s-=20;if((prodPct-finPct)>10)s-=20;if(resteRatio>50)s-=10;if(alerts>5)s-=10;if(rappels>5)s-=10;return Math.max(0,s);}
function badge(text,level){return `<span class='badge ${level}'>${text}</span>`;}

function renderChart(prodValue){const root=document.getElementById('chart');const a=state.selected;if(!a){root.innerHTML='';return;}const data=MONTHS.map(m=>({l:MONTH_LABELS[m],pre:Number(((a.mensuel||{})[m]||{}).previsionnel||0),fac:Number(((a.mensuel||{})[m]||{}).facture||0)}));let cp=0,cf=0;const cum=data.map(d=>({l:d.l,pre:(cp+=d.pre),fac:(cf+=d.fac)}));const finalPre=cum.length?cum[cum.length-1].pre:0;const finalFac=cum.length?cum[cum.length-1].fac:0;const finalProd=Number(prodValue||0);const max=Math.max(1,finalPre,finalFac,finalProd);const left=55,top=20,width=900,height=250,step=width/Math.max(1,cum.length-1);
let out='';for(let i=0;i<=4;i++){const y=top+(height/4)*i;out+=`<line x1='${left}' y1='${y}' x2='${left+width}' y2='${y}' stroke='#dde5f2'/>`;}
const pts=(arr,k)=>arr.map((d,i)=>`${left+i*step},${top+height-(Number(d[k]||0)/max)*height}`).join(' ');
out+=`<polyline points='${pts(cum,'pre')}' fill='none' stroke='#9db4de' stroke-width='4'/>`;
out+=`<polyline points='${pts(cum,'fac')}' fill='none' stroke='#ef8d00' stroke-width='4'/>`;
if(finalProd>0){const y=top+height-(finalProd/max)*height;out+=`<line x1='${left}' y1='${y}' x2='${left+width}' y2='${y}' stroke='#239b5b' stroke-width='3' stroke-dasharray='7 7'/>`;}
cum.forEach((d,i)=>{if(i%2===0)out+=`<text x='${left+i*step}' y='${top+height+20}' text-anchor='middle' fill='#60708a' font-size='12'>${d.l}</text>`;const yPre=top+height-(d.pre/max)*height,yFac=top+height-(d.fac/max)*height;out+=`<circle cx='${left+i*step}' cy='${yPre}' r='3' fill='#9db4de'><title>Prévisionnel : ${euro(d.pre)} | Facturation : ${euro(d.fac)} | Production estimée : ${euro(finalProd)}</title></circle>`;out+=`<circle cx='${left+i*step}' cy='${yFac}' r='3' fill='#ef8d00'><title>Prévisionnel : ${euro(d.pre)} | Facturation : ${euro(d.fac)} | Production estimée : ${euro(finalProd)}</title></circle>`;});
root.innerHTML=out;
}

function renderBoond(billed){const top=document.getElementById('boondTop');const table=document.getElementById('boondTable');const t=((state.boond||{}).totals||{});const cost=Number(t.total_cost||0);const margin=billed-cost;const rent=cost>0?(margin/Math.max(cost,1))*100:0;const resources=(state.boond&&state.boond.resources_imputation)||[];const total=Math.max(1,...resources.map(r=>Number(r.cost||0)),cost);
const monthsElapsed=Math.max(1,new Date().getMonth()+1);const burnRate=cost/monthsElapsed;const resteProd=Math.max(0,Number((state.selected&&state.selected.reste_a_facturer)||0));const monthsToEnd=burnRate>0?resteProd/burnRate:0;const endDate=new Date();endDate.setMonth(endDate.getMonth()+Math.ceil(monthsToEnd));
top.innerHTML=`COÛT TOTAL PRODUCTION: <strong>${euro(cost)}</strong> · MARGE IMPUTÉE: <strong>${euro(margin)}</strong> · RENTABILITÉ: <strong>${fmt(rent)}%</strong> · BURN RATE PROJET: <strong>${euro(burnRate)}/mois</strong> · ESTIMATION FIN PROJET: <strong>${isFinite(monthsToEnd)&&monthsToEnd>0?endDate.toLocaleDateString('fr-FR',{month:'long',year:'numeric'}):'N/A'}</strong>`;
if(!resources.length){table.innerHTML="<tbody><tr><td>Aucune ressource BOOND disponible.</td></tr></tbody>";return;}
table.innerHTML=`<thead><tr><th>RESSOURCE</th><th class='num'>JOURS</th><th class='num'>COÛT IMPUTÉ</th><th class='num'>PART DU COÛT</th></tr></thead><tbody>${resources.map(r=>{const c=Number(r.cost||0);const part=(c/total)*100;return `<tr><td>${r.resource_name||r.resource_id||'-'}<div class='costbar'><div class='costfill' style='width:${Math.min(100,part)}%'></div></div></td><td class='num'>${fmt(r.days)}</td><td class='num'>${euro(c)}</td><td class='num'>${fmt(part)}%</td></tr>`;}).join('')}</tbody>`;
}

function render(){const a=state.selected;document.getElementById('financeBtn').href=state.selectedId?`/finance?affaire_id=${encodeURIComponent(state.selectedId)}`:'/finance';document.getElementById('pmBtn').href=state.selectedId?`/gestion-projet?affaire_id=${encodeURIComponent(state.selectedId)}`:'/gestion-projet';document.getElementById('exportBtn').disabled=!state.selectedId;
if(!a){document.getElementById('title').textContent='Sélectionnez une affaire';return;}
const billed=Number(a.facturation_totale||((a.anteriorite||0)+(a.facture_2026||a.facturation_cumulee_2026||0)));const cmd=Number(a.commande_ht||0);const reste=Number(a.reste_a_facturer||0);const finPct=cmd>0?(billed/cmd*100):0;const prodPct=Number((((state.board||{}).analytics||{}).kpi_project_progress||{}).calendar_progress_pct||0);const alerts=Number((((state.board||{}).kpis)||{}).overdue_topics||0);const rappels=Number((((state.board||{}).kpis_pilotage)||{}).reminders_open_count||0);const rent=Number((((state.boond||{}).totals)||{}).profitability_production||0);
const gap=finPct-prodPct;const resteRatio=cmd>0?(reste/cmd*100):0;const messages=[];if(finPct>prodPct)messages.push(badge('⚠ Facturation en avance sur production','warn'));if(prodPct>finPct)messages.push(badge('⚠ Production en avance sur facturation','warn'));if(resteRatio>50)messages.push(badge('⚠ Reste à facturer élevé','bad'));if(alerts>5)messages.push(badge('⚠ Nombre d\'alertes important','bad'));if(rent>150)messages.push(badge('✔ Production très rentable','ok'));if(rent>0&&rent<80)messages.push(badge('⚠ Rentabilité faible','warn'));if(!messages.length)messages.push(badge('✔ Situation maîtrisée','ok'));document.getElementById('diagnosticBadges').innerHTML=messages.join(' ');
const score=scoreHealth(finPct,prodPct,resteRatio,alerts,rappels);const sc=document.getElementById('healthScore');sc.textContent=`${score} / 100`;sc.className='score '+(score>=80?'ok':score>=60?'warn':'bad');
...
const margeProj=billed-Number((((state.boond||{}).totals)||{}).total_cost||0);
[['kCommande',cmd],['kFacture',billed],['kReste',reste],['kMarge',margeProj]].forEach(x=>document.getElementById(x[0]).textContent=euro(x[1]));
document.getElementById('kFactureSub').textContent=`${fmt(finPct)} % du budget`;document.getElementById('kResteSub').textContent=`${fmt(resteRatio)} % du budget`;document.getElementById('kAvance').textContent=`${fmt(finPct)} %`;
const prodValue=(cmd*Math.max(0,prodPct))/100;renderChart(prodValue);
const preTotal=Number(a.total_previsionnel||0),facTotal=Number(a.total_facture||0);const retard=preTotal-facTotal;document.getElementById('trajectory').textContent=`Prévisionnel : ${euro(preTotal)} · Facturation : ${euro(facTotal)} · Production estimée : ${euro(prodValue)}`;
document.getElementById('trajectoryRows').innerHTML=`Prévisionnel total: <strong>${euro(preTotal)}</strong><br>Facturation totale: <strong>${euro(facTotal)}</strong><br>Production estimée: <strong>${euro(prodValue)}</strong><br><span style='color:${retard>0?'#d08a1a':'#239b5b'}'>Retard facturation : <strong>${euro(retard)}</strong></span>`;
renderBoond(billed);
}

async function init(){await loadAffaires('');const params=new URLSearchParams(window.location.search);const pre=params.get('affaire_id')||localStorage.getItem('selectedAffaireId')||'';if(pre){document.getElementById('affaireSelect').value=pre;await loadSelected(pre);}else{render();}
document.getElementById('searchInput').addEventListener('input',async e=>loadAffaires(e.target.value||''));document.getElementById('affaireSelect').addEventListener('change',async e=>{localStorage.setItem('selectedAffaireId',e.target.value||'');await loadSelected(e.target.value||'');});document.getElementById('exportBtn').addEventListener('click',()=>{if(state.selectedId)window.location.href=`/api/finance/affaire/${encodeURIComponent(state.selectedId)}/export-csv`;});}
init();
</script></body></html>"""
