...
<body><div class='wrap'>
  ...
  <div id='sectionReactivity' class='section'>
    <h3>Niveau 2 — Gestion de projet</h3>
    <div class='chart-controls'>
      <label for='level2ChartMetric' class='small'>Afficher :</label>
      <select id='level2ChartMetric' class='chart-select'>
        <option value='reactivity'>Réactivité entreprise</option>
        <option value='delay-trigger'>Déclencheur de retard</option>
        <option value='zone-progress'>Avancement par zone</option>
      </select>
    </div>
    <div id='level2ChartTitle' class='small' style='font-weight:800;margin-bottom:8px'>Réactivité entreprise</div>
    <div id='level2ChartList' class='pilot-list'><div class='small'>Aucune donnée</div></div>
  </div>

  <div id='sectionPointageAnalytics' class='section' style='display:none'>
    <h3>Niveau 4 — Retard moyen par ressource issu du pointage opérationnel</h3>
    <div id='pointageAnalyticsInfo' class='small' style='margin-bottom:10px'>Calculé uniquement sur les tâches pointées et terminées.</div>
    <div id='pointageAnalyticsChart' class='pilot-list'><div class='small'>Aucune donnée</div></div>
  </div>
...
function renderLevel2Empty(message){const root=document.getElementById('level2ChartList');if(root)root.innerHTML=`<div class='chart-empty'>${esc(message||'Aucune donnée')}</div>`;}
function renderReactivityChart(b){const p=(b&&b.kpis_pilotage)||{};const items=p.reactivity_by_company||[];const root=document.getElementById('level2ChartList');if(!root)return;if(!items.length){renderLevel2Empty('Pas assez de tâches clôturées avec échéance.');return;}const vals=items.map(x=>Math.abs(Number(x.avg_gap_days||0)));const max=Math.max(1,...vals);root.innerHTML=`<div style='display:flex;align-items:flex-end;gap:14px;height:240px;padding:10px 0'>${items.map(x=>{const v=Number(x.avg_gap_days||0);const h=Math.max(8,(Math.abs(v)/max)*190);const isTempo=normalizeText(x.name)==='tempo';const color=isTempo?'#ef8d00':(v>0?'#d64545':'#1b6ef3');return `<div style='flex:1;min-width:60px;text-align:center'><div title='${esc(x.name)}: ${v} j' style='margin:0 auto;width:34px;height:${h}px;background:${color};border-radius:8px 8px 0 0'></div><div style='font-size:11px;margin-top:6px;font-weight:700'>${esc(x.name)}</div><div style='font-size:11px;color:#6e7a90'>${v} j</div></div>`;}).join('')}</div>`;}
function buildLevel2DelayTriggerItems(tasks){if(!tasks||!tasks.length)return[];const actionable=tasks.filter(t=>!t.is_summary);const byLevel={};for(const task of actionable){const level=String(task.level_label||task.level||'Général').trim()||'Général';(byLevel[level]=byLevel[level]||[]).push(task);}const items=[];for(const [level,rows] of Object.entries(byLevel)){let responsible='';for(const task of rows){if(task.is_cet&&Array.isArray(task.cetMembers)&&task.cetMembers.length){const delayedMember=task.cetMembers.find(member=>{const memberData=((task.cet||{})[member]||{});const isDone=memberData.status==='past'||!!memberData.actualEnd;return isDone&&Number(memberData.delayDays||0)>0;});if(delayedMember){responsible=delayedMember;break;}}const isDone=task.status==='past'||!!task.actualEnd;if(isDone&&Number(task.delayDays||0)>0){responsible=String(task.owner||'').trim()||'Non renseigné';break;}}if(responsible){items.push({label:level,value:responsible});}}return items.sort((a,b)=>a.label.localeCompare(b.label,'fr'));}
function renderDelayTriggerChart(tasks){const root=document.getElementById('level2ChartList');if(!root)return;const items=buildLevel2DelayTriggerItems(tasks);if(!items.length){renderLevel2Empty('Aucune donnée');return;}root.innerHTML=items.map(item=>`<div class='pilot-row' style='align-items:center'><strong>${esc(item.label)}</strong><span class='tag-pill'>${esc(item.value)}</span></div>`).join('');}
function buildZoneProgressItems(tasks){if(!tasks||!tasks.length)return[];const actionable=tasks.filter(t=>!t.is_summary);const byLevel={};for(const task of actionable){const level=String(task.level_label||task.level||'Général').trim()||'Général';const progress=Math.max(0,Math.min(100,Number(task.progress||0)));const rec=byLevel[level]||(byLevel[level]={sum:0,count:0});rec.sum+=progress;rec.count+=1;}return Object.entries(byLevel).map(([label,rec])=>({label,value:rec.count?(rec.sum/rec.count):0,count:rec.count})).filter(item=>item.count>0).sort((a,b)=>a.label.localeCompare(b.label,'fr'));}
function renderZoneProgressChart(tasks){const root=document.getElementById('level2ChartList');if(!root)return;const items=buildZoneProgressItems(tasks);if(!items.length){renderLevel2Empty('Aucune donnée');return;}root.innerHTML=items.map(item=>{const value=Math.max(0,Math.min(100,Number(item.value||0)));return `<div class='hbar-row'><strong>${esc(item.label)}</strong><div class='hbar-track'><div class='hbar-fill' data-tone='full' style='width:${value}%'></div></div><strong style='text-align:right'>${value.toFixed(0)}%</strong></div>`;}).join('');}
function renderLevel2Chart(){const select=document.getElementById('level2ChartMetric');const title=document.getElementById('level2ChartTitle');const metric=(select&&select.value)||'reactivity';const tasks=((state.pointage||{}).tasks||[]);if(title)title.textContent=metric==='delay-trigger'?'Déclencheur de retard':(metric==='zone-progress'?'Avancement par zone':'Réactivité entreprise');if(metric==='delay-trigger'){renderDelayTriggerChart(tasks);return;}if(metric==='zone-progress'){renderZoneProgressChart(tasks);return;}renderReactivityChart(state.board||{});}
...
async function loadPointage(){if(!state.selectedId)return;try{const d=await pointageApi(`/api/project-management/pointage?affaire_id=${encodeURIComponent(state.selectedId)}`);state.pointage=d;renderPointage();renderPointageAnalytics();renderLevel2Chart();}catch(e){console.warn(e);}}
...
function renderPointageAnalytics(){const sec=document.getElementById('sectionPointageAnalytics');const root=document.getElementById('pointageAnalyticsChart');if(!sec||!root)return;const tasks=((state.pointage||{}).tasks||[]);if(!tasks.length){sec.style.display='none';root.innerHTML="<div class='small'>Aucune donnée</div>";return;}const stats={};const push=(name,delay)=>{const key=String(name||'').trim();if(!key || key==='Non attribué' || key.includes('+'))return;const rec=stats[key]||(stats[key]={sum:0,count:0});rec.sum+=Number(delay||0);rec.count+=1;};for(const t of tasks){if(t.is_summary)continue;if(t.is_cet&&Array.isArray(t.cetMembers)&&t.cetMembers.length){for(const m of t.cetMembers){const cm=((t.cet||{})[m]||{});const isPointed=Number(cm.progress||0)>0 || !!cm.actualEnd;const isDone=cm.status==='past' || !!cm.actualEnd;if(isPointed&&isDone)push(m,cm.delayDays||0);}continue;}const owner=String(t.owner||'').trim();const isPointed=Number(t.progress||0)>0 || !!t.actualEnd;const isDone=t.status==='past' || !!t.actualEnd;if(isPointed&&isDone)push(owner,t.delayDays||0);}const items=Object.entries(stats).map(([name,v])=>({name,avg:v.count?(v.sum/v.count):0,count:v.count})).filter(x=>x.count>0).sort((a,b)=>b.avg-a.avg||a.name.localeCompare(b.name,'fr'));if(!items.length){sec.style.display='none';root.innerHTML="<div class='small'>Aucune donnée de pointage terminée.</div>";return;}sec.style.display='block';const max=Math.max(1,...items.map(x=>Number(x.avg||0)));root.innerHTML=`<div style='display:flex;align-items:flex-end;gap:14px;height:260px;padding:10px 0'>${items.map(x=>{const v=Number(x.avg||0);const h=Math.max(10,(v/max)*190);return `<div style='flex:1;min-width:90px;text-align:center'><div title='${esc(x.name)}: ${v.toFixed(1)} j de retard moyen' style='margin:0 auto;width:42px;height:${h}px;background:${x.name==='CST'?'#ef8d00':'#d64545'};border-radius:10px 10px 0 0'></div><div style='font-size:12px;margin-top:8px;font-weight:800'>${esc(x.name)}</div><div style='font-size:12px;color:#6e7a90'>${v.toFixed(1)} j</div><div style='font-size:11px;color:#8a94a8'>${x.count} tâche(s)</div></div>`;}).join('')}</div>`;}
...
async function init(){
  try{
    ...
    const level2ChartMetric=document.getElementById('level2ChartMetric');if(level2ChartMetric)level2ChartMetric.addEventListener('change',renderLevel2Chart);
