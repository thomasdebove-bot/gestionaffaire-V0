...
<div class='container'>
  <div class='topbar'>
    ...
    <div class='controls'>
      ...
      <div class='project-row'><div id='selectedProjectInfo' class='badge'><span class='dot idle'></span><span>Projet sélectionné depuis l'accueil</span></div></div>
    </div>
  </div>
...
function resolveAffaireId(raw){const key=norm(raw);if(!key)return '';const exact=state.affaires.find(x=>norm(x.affaire_id)===key);if(exact)return exact.affaire_id;const near=state.affaires.find(x=>norm(x.affaire_id).includes(key)||key.includes(norm(x.affaire_id))||norm(x.display_name||'').includes(key));return near?near.affaire_id:'';}
...
async function rebuildCache(){clearError();showNotice('Reconstruction du cache en cours…');await api('/api/finance/rebuild-cache',{method:'POST'});await loadCacheStatus();await loadAffairesList('');if(state.selectedAffaireId&&state.affaires.some(x=>x.affaire_id===state.selectedAffaireId)){await loadSelectedAffaire(state.selectedAffaireId);}else{state.selectedAffaireId='';state.selectedAffaire=null;renderAll();}const info=document.getElementById('selectedProjectInfo');if(info){info.innerHTML=`<span class='dot ${state.selectedAffaireId?'ready':'idle'}'></span><span>${state.selectedAffaireId?`Affaire active : ${esc(state.selectedAffaireId)}`:'Projet sélectionné depuis l'accueil'}</span>`;}showNotice('Cache reconstruit avec succès.');}
async function initFinancePage(){clearError();try{await loadCacheStatus();await loadAffairesList('');const params=new URLSearchParams(window.location.search);const affairFromUrl=params.get('affaire_id');const affairFromStorage=localStorage.getItem('selectedAffaireId')||'';const preselected=affairFromUrl||affairFromStorage;if(preselected){let target=preselected;try{await loadSelectedAffaire(target);}catch(err){const resolved=resolveAffaireId(preselected);if(resolved&&resolved!==preselected){await loadSelectedAffaire(resolved);}else{throw err;}}}else{renderAll();}}catch(err){showError(err.message||'Erreur de chargement');}
const info=document.getElementById('selectedProjectInfo');if(info){info.innerHTML=`<span class='dot ${state.selectedAffaireId?'ready':'idle'}'></span><span>${state.selectedAffaireId?`Affaire active : ${esc(state.selectedAffaireId)}`:'Projet sélectionné depuis l'accueil'}</span>`;}
...
<body><div class='wrap'>
  ...
  <div id='sectionCompany' class='section'>
    <h3>Niveau 2 — Analyse entreprise</h3>
...
function norm(v){return String(v||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'');}
async function loadAffairesList(){try{const d=await api('/api/finance/affaires');state.affaires=d.items||[];}catch(_){state.affaires=[];}}
function resolveAffaireId(raw){const key=norm(raw);if(!key)return '';const exact=state.affaires.find(x=>norm(x.affaire_id)===key);if(exact)return exact.affaire_id;const near=state.affaires.find(x=>norm(x.affaire_id).includes(key)||key.includes(norm(x.affaire_id))||norm(x.display_name||'').includes(key));return near?near.affaire_id:'';}
...
async function loadBoard(id){const requested=id||state.selectedId||'';if(!requested){state.selectedId='';state.selectedLabel='';state.board=null;renderBoard();return;}const selected=resolveAffaire(requested);const aid=(selected&&selected.affaire_id)||requested;state.selectedId=aid;const sel=document.getElementById('affaireSelect');if(sel)sel.value=aid;state.selectedLabel=(selected&&selected.display_name)||state.selectedLabel||'';localStorage.setItem('selectedAffaireId',aid);const info=document.getElementById('selectedProjectInfo');if(info){info.innerHTML=`<span class='dot ready'></span><span>Affaire active : ${esc(aid)}</span>`;}const fin=document.getElementById('financeBtn');if(fin)fin.href=`/finance?affaire_id=${encodeURIComponent(aid)}`;const dash=document.getElementById('dashboardBtn');if(dash)dash.href=`/dashboard?affaire_id=${encodeURIComponent(aid)}`;showLoading(true,'Chargement de la vue projet…');try{let b;try{b=await api(`/api/project-management/board?affaire_id=${encodeURIComponent(aid)}`);}catch(errById){const msg=String((errById&&errById.message)||'');if(state.selectedLabel&&(msg.includes('Affaire introuvable')||msg.includes('affaire_id ou affaire_name requis')||msg.includes('404'))){const alt=String(state.selectedLabel||'').split(' - ').slice(-1)[0]||state.selectedLabel;b=await api(`/api/project-management/board?affaire_name=${encodeURIComponent(alt)}`);}else{throw errById;}}state.board=b;renderBoard();}catch(err){console.error(err);state.board=null;renderBoard();showPageError(err);}finally{showLoading(false);}}
  ...
  try{
    ...
    if(initialId){
      ...
      if(found){
        ...
        if(sel)sel.value=found.affaire_id;
      ...
      await loadPointage();
    }else{renderBoard();const info=document.getElementById('selectedProjectInfo');if(info){info.innerHTML="<span class='dot idle'></span><span>Projet sélectionné depuis l'accueil</span>";}}
...
.top-main{display:flex;flex-direction:column;gap:10px}.menu-row{display:flex;gap:10px;flex-wrap:wrap;align-items:center}
.btn{height:44px;border-radius:12px;border:none;padding:0 14px;background:var(--accent);color:#fff;text-decoration:none;display:inline-flex;align-items:center;font-weight:800;cursor:pointer}
.selected-badge{padding:10px 12px;border-radius:12px;border:1px solid var(--line);background:#fff7ec;color:#8d6419;font-weight:700}
.header{margin-top:14px;padding:22px;background:#fffaf0}.header-top{display:flex;justify-content:space-between;gap:16px;align-items:flex-start}.eyeb{font-size:12px;font-weight:800;letter-spacing:.13em;color:var(--accent);text-transform:uppercase}.title{font-size:40px;font-weight:900;margin:6px 0}.subtitle{color:var(--muted)}.badge{display:inline-flex;align-items:center;padding:8px 14px;border-radius:999px;font-size:12px;font-weight:800}.badge.ok{background:#e9f8ef;color:var(--ok)}.badge.warn{background:#fff6e8;color:var(--warn)}.badge.bad{background:#ffeded;color:var(--bad)}
...
.health{margin-top:16px;padding:16px;border:1px solid #d2deef;border-radius:18px;background:#fff}.health h3{margin:0 0 12px;font-size:18px}.health-grid{display:grid;grid-template-columns:repeat(8,minmax(0,1fr));gap:10px}.health-item{background:#fff;border:1px solid #dbe5f3;border-radius:12px;padding:10px}.health-item .l{font-size:11px;color:var(--muted);text-transform:uppercase;font-weight:700}.health-item .v{margin-top:6px;font-size:24px;font-weight:900}.signal{margin-top:12px;padding:12px;border-radius:12px;background:#fff;border:1px solid #dbe5f3;font-weight:700}
.diag{margin-top:12px;padding:12px;border:1px solid #dbe5f3;border-radius:14px;background:#fff}.diag-title{font-weight:900;margin-bottom:8px}.diag-tags{display:flex;gap:8px;flex-wrap:wrap}.tag{display:inline-flex;align-items:center;padding:7px 10px;border-radius:999px;font-size:12px;font-weight:800}.tag.ok{background:#e9f8ef;color:#177345}.tag.warn{background:#fff4df;color:#a26b08}.tag.bad{background:#ffeded;color:#b23636}
.analysis{margin-top:14px;padding:14px;background:#fff}.kpi-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:12px}.k{border:1px solid var(--line);border-radius:14px;padding:14px;background:#fbfdff}.k .l{color:var(--muted);font-size:12px;text-transform:uppercase;font-weight:700}.k .v{font-size:34px;font-weight:900;margin-top:8px}.k .s{font-size:12px;color:#6f7f98;margin-top:6px}
.charts{margin-top:14px;padding:16px;background:#f3f6fb}.chart-wrap{height:390px;border:1px solid #d5deec;border-radius:16px;padding:12px;background:linear-gradient(180deg,#fbfcff,#f4f7fd)}.axis-note{font-size:12px;color:#60708a;font-weight:700;margin-top:8px;display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap}
.trajectory{margin-top:12px;border:1px solid #d5deec;border-radius:16px;padding:12px;background:#fff}.t-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.t-item{padding:10px;border:1px solid #e0e7f2;border-radius:12px}.t-item b{display:block;font-size:24px;margin-top:4px}.retard.warn{color:#c27511}.retard.ok{color:#1d9a5b}
@media (max-width:1180px){.health-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.kpi-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.t-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media (max-width:740px){.top{grid-template-columns:1fr}.menu-row{flex-direction:column;align-items:stretch}.kpi-grid,.health-grid,.t-grid{grid-template-columns:1fr}.title{font-size:30px}}
</style></head>
<body><div class='wrap'>
  <div class='top'><a class='logo-link' href='/'><div class='brand-mini'><img src='/assets/logo-tempo' alt='Tempo'></div></a><div class='top-main'><div class='menu-row'><a class='btn' href='/'>Accueil</a><a id='financeBtn' class='btn' href='/finance'>Finances</a><a id='pmBtn' class='btn' href='/gestion-projet'>Gestion de projet</a><button id='exportBtn' class='btn' disabled>Exporter CSV</button><div id='selectedProject' class='selected-badge'>Projet sélectionné depuis l'accueil</div></div></div></div>

  <section class='card header'><div class='header-top'><div><div class='eyeb'>Niveau 1 — Pilotage</div><div id='title' class='title'>Sélectionnez une affaire</div><div id='subtitle' class='subtitle'>Vision synthétique de pilotage.</div><div id='healthBadge' class='badge warn'>Sous surveillance</div></div><div class='media'><div id='projectImageBox' class='hero-media'><img id='projectImage' alt='Projet'></div><img class='hero-logo' src='/assets/logo-tempo' alt='Logo Tempo'></div></div>
    <div class='health'><h3>Santé projet</h3><div class='health-grid'><div class='health-item'><div class='l'>Production %</div><div id='hProd' class='v'>0%</div></div><div class='health-item'><div class='l'>Facturation %</div><div id='hFac' class='v'>0%</div></div><div class='health-item'><div class='l'>Écart fac/prod</div><div id='hGap' class='v'>0 pt</div></div><div class='health-item'><div class='l'>Reste à facturer</div><div id='hReste' class='v'>0€</div></div><div class='health-item'><div class='l'>Alertes</div><div id='hAlerts' class='v'>0</div></div><div class='health-item'><div class='l'>Rappels ouverts</div><div id='hRappels' class='v'>0</div></div><div class='health-item'><div class='l'>Sujets ouverts</div><div id='hSujets' class='v'>0</div></div><div class='health-item'><div class='l'>Health score</div><div id='hScore' class='v'>100/100</div></div></div><div id='healthSignal' class='signal'>Sélectionnez une affaire pour afficher le diagnostic.</div></div>
    <div class='diag'><div class='diag-title'>DIAGNOSTIC PROJET</div><div id='diagTags' class='diag-tags'><span class='tag warn'>⚠ En attente de données projet</span></div></div>
  </section>

  <section class='card analysis'><div class='eyeb'>Niveau 2 — Analyse</div><div class='kpi-grid'><div class='k'><div class='l'>Commande / budget</div><div id='kCommande' class='v'>0 €</div><div class='s' id='kCommandeSub'>Base contrat</div></div><div class='k'><div class='l'>Facturé</div><div id='kFacture' class='v'>0 €</div><div class='s' id='kFactureSub'>0 % du budget</div></div><div class='k'><div class='l'>Reste à facturer</div><div id='kReste' class='v'>0 €</div><div class='s' id='kResteSub'>0 % du budget</div></div><div class='k'><div class='l'>Avancement financier</div><div id='kAvance' class='v'>0 %</div><div class='s'>Facturé / commande</div></div><div class='k'><div class='l'>Marge projetée (10%)</div><div id='kMarge' class='v'>0 €</div><div class='s'>Estimation rapide</div></div><div class='k'><div class='l'>Burn rate projet</div><div id='kBurn' class='v'>-</div><div class='s' id='kBurnSub'>Production moyenne mensuelle</div></div><div class='k'><div class='l'>Estimation fin projet</div><div id='kEnd' class='v'>-</div><div class='s' id='kEndSub'>Projection à rythme constant</div></div></div></section>

  <section class='card charts'><div class='eyeb'>Niveau 3 — Trajectoire</div><h3 style='margin:6px 0 10px'>Prévisionnel, facturation et production cumulés</h3><div class='chart-wrap'><svg id='chart' width='100%' height='360' viewBox='0 0 980 360' preserveAspectRatio='xMidYMid meet'></svg></div><div class='axis-note'><span>Axe X : Mois (Janv. → Déc.)</span><span>Axe Y : Montants cumulés (€)</span></div>
    <div class='trajectory'><div class='diag-title'>TRAJECTOIRE PROJET</div><div class='t-grid'><div class='t-item'>Prévisionnel total<b id='tPre'>0 €</b></div><div class='t-item'>Facturation totale<b id='tFac'>0 €</b></div><div class='t-item'>Production estimée<b id='tProd'>0 €</b></div><div class='t-item'>Retard facturation<b id='tDelay' class='retard warn'>0 €</b></div></div></div>
  </section>
</div>
<script>
const MONTHS=['janvier','fevrier','mars','avril','mai','juin','juillet','aout','septembre','octobre','novembre','decembre'];
const MONTH_LABELS={janvier:'Janv.',fevrier:'Févr.',mars:'Mars',avril:'Avr.',mai:'Mai',juin:'Juin',juillet:'Juil.',aout:'Août',septembre:'Sept.',octobre:'Oct.',novembre:'Nov.',decembre:'Déc.'};
const state={affaires:[],selectedId:'',selected:null,board:null,boond:null};
...
async function api(u){const r=await fetch(u);const d=await r.json();if(!r.ok) throw new Error(d.detail||'Erreur API');return d;}
function statusClass(s){if(s>=80)return'ok';if(s>=60)return'warn';return'bad';}
async function loadSelectedAffaire(id){if(!id){state.selectedId='';state.selected=null;state.board=null;state.boond=null;render();return;}state.selectedId=id;const d=await api(`/api/finance/affaire/${encodeURIComponent(id)}`);state.selected=d.affaire;localStorage.setItem('selectedAffaireId',id);const pname=(d.affaire&& (d.affaire.affaire||d.affaire.display_name||''))||'';try{state.board=await api(`/api/project-management/board?affaire_id=${encodeURIComponent(id)}`);}catch(_){try{state.board=pname?await api(`/api/project-management/board?affaire_name=${encodeURIComponent(pname)}`):null;}catch(__){state.board=null;}}try{state.boond=pname?await api(`/api/boond/imputations/by-project?project_name=${encodeURIComponent(pname)}&affaire_id=${encodeURIComponent(id)}&display_name=${encodeURIComponent(d.affaire.display_name||'')}`):null;}catch(_){state.boond=null;}render();}
function computeDiagnostics(ctx){const tags=[];if(ctx.finPct>ctx.prodPct)tags.push({lvl:'warn',txt:'⚠ Facturation en avance sur production'});if(ctx.prodPct>ctx.finPct)tags.push({lvl:'warn',txt:'⚠ Production en avance sur facturation'});if(ctx.reste>ctx.cmd*0.5)tags.push({lvl:'warn',txt:'⚠ Reste à facturer élevé'});if(ctx.alerts>5)tags.push({lvl:'bad',txt:'⚠ Nombre d\'alertes important'});if(ctx.boondRent>150)tags.push({lvl:'ok',txt:'✔ Production très rentable'});if(ctx.boondRent>0&&ctx.boondRent<80)tags.push({lvl:'warn',txt:'⚠ Rentabilité faible'});if(!tags.length)tags.push({lvl:'ok',txt:'✔ Indicateurs globalement stables'});return tags;}
function computeScore(ctx){let s=100;if(ctx.finPct>ctx.prodPct)s-=20;if((ctx.prodPct-ctx.finPct)>10)s-=20;if(ctx.reste>ctx.cmd*0.5)s-=10;if(ctx.alerts>5)s-=10;if(ctx.rappels>5)s-=10;return Math.max(0,s);}
function renderChart(prodPct){const root=document.getElementById('chart');const a=state.selected;if(!a){root.innerHTML='';return {pre:0,fac:0,prod:0};}const data=MONTHS.map(m=>({label:MONTH_LABELS[m],pre:Number(((a.mensuel||{})[m]||{}).previsionnel||0),fac:Number(((a.mensuel||{})[m]||{}).facture||0)}));let cp=0,cf=0;const cum=data.map(d=>({l:d.label,pre:(cp+=d.pre),fac:(cf+=d.fac)}));const finalProd=(Number(a.commande_ht||0)*Math.max(0,prodPct))/100;const totalPre=Math.max(1,cum[cum.length-1]?.pre||1);const prodSeries=cum.map(d=>({v:finalProd>0?(d.pre/totalPre)*finalProd:0}));const max=Math.max(1,...cum.flatMap(x=>[x.pre,x.fac]),finalProd);const left=90,top=20,width=840,height=265,step=width/Math.max(1,cum.length);let out='';
for(let i=0;i<=4;i++){const y=top+(height/4)*i;const val=Math.round(max*(1-i/4));out+=`<line x1='${left}' y1='${y}' x2='${left+width}' y2='${y}' stroke='#dde5f2'/><text x='${left-10}' y='${y+4}' text-anchor='end' fill='#5f6f88' font-size='12'>${val.toLocaleString('fr-FR')} €</text>`;}
out+=`<text x='${left+width/2}' y='${top+height+50}' text-anchor='middle' fill='#5f6f88' font-size='12'>Mois</text>`;
const prePts=[];const prodPts=[];
cum.forEach((d,i)=>{const cx=left+i*step+step*0.5;const py=top+height-(d.pre/max)*height;const fy=top+height-(d.fac/max)*height;const pdy=top+height-((prodSeries[i]||{}).v/max)*height;const bw=Math.min(28,step*0.45);prePts.push(`${cx},${py}`);if(finalProd>0)prodPts.push(`${cx},${pdy}`);out+=`<rect x='${cx-bw/2}' y='${fy}' width='${bw}' height='${Math.max(1,top+height-fy)}' rx='6' fill='#ef8d00'><title>${d.l}
Prévisionnel : ${euro(d.pre)}
Facturation : ${euro(d.fac)}
Production estimée : ${euro((prodSeries[i]||{}).v||0)}</title></rect>`;if(i%2===0)out+=`<text x='${cx}' y='${top+height+22}' text-anchor='middle' fill='#5f6f88' font-size='12'>${d.l}</text>`;});
out+=`<polyline points='${prePts.join(' ')}' fill='none' stroke='#9db4de' stroke-width='4'/>`;if(finalProd>0)out+=`<polyline points='${prodPts.join(' ')}' fill='none' stroke='#239b5b' stroke-width='3' stroke-dasharray='7 7'/>`;
root.innerHTML=out;return {pre:cum[cum.length-1]?.pre||0,fac:cum[cum.length-1]?.fac||0,prod:finalProd};}
function monthLabelAfter(months){const d=new Date();d.setMonth(d.getMonth()+Math.max(0,Math.round(months)));return d.toLocaleDateString('fr-FR',{month:'long',year:'numeric'});} 
function render(){const a=state.selected;document.getElementById('financeBtn').href=state.selectedId?`/finance?affaire_id=${encodeURIComponent(state.selectedId)}`:'/finance';document.getElementById('pmBtn').href=state.selectedId?`/gestion-projet?affaire_id=${encodeURIComponent(state.selectedId)}`:'/gestion-projet';document.getElementById('exportBtn').disabled=!state.selectedId;document.getElementById('selectedProject').textContent=state.selectedId?`Affaire active : ${state.selectedId}`:'Projet sélectionné depuis l'accueil';if(!a){document.getElementById('title').textContent='Sélectionnez une affaire depuis la page Accueil';return;}
const billed=Number(a.facturation_totale||((a.anteriorite||0)+(a.facture_2026||a.facturation_cumulee_2026||0)));const cmd=Number(a.commande_ht||0);const reste=Number(a.reste_a_facturer||0);const finPct=cmd>0?(billed/cmd*100):0;const prodPct=Number((((state.board||{}).analytics||{}).kpi_project_progress||{}).calendar_progress_pct||0);const pilot=((state.board||{}).kpis_pilotage||{});const projectKpis=((state.board||{}).kpis||{});const rappels=Number(pilot.reminders_open_count||pilot.rappels_ouverts_a_date||0);const sujets=Number(projectKpis.open_topics||0);const alerts=Number(projectKpis.overdue_topics||0);const boondCost=Number((((state.boond||{}).totals||{}).total_cost)||0);const boondRent=(boondCost>0)?(billed/boondCost*100):0;const gap=finPct-prodPct;
const score=computeScore({finPct,prodPct,reste,cmd,alerts,rappels});const tags=computeDiagnostics({finPct,prodPct,reste,cmd,alerts,boondRent});
document.getElementById('diagTags').innerHTML=tags.map(t=>`<span class='tag ${t.lvl}'>${esc(t.txt)}</span>`).join('');document.getElementById('healthSignal').textContent=tags.map(t=>t.txt).join(' · ');
const hb=document.getElementById('healthBadge');hb.className='badge '+(alerts>5||score<60?'bad':score<80?'warn':'ok');hb.textContent=score<60?'Risque':score<80?'Attention':'OK';
document.getElementById('hScore').textContent=`${score} / 100`;document.getElementById('hScore').style.color=score<60?'#cc4747':(score<80?'#d08a1a':'#239b5b');
document.getElementById('title').textContent=a.display_name||'-';document.getElementById('subtitle').textContent=`Client ${a.client||'-'} · ${a.affaire||'-'}`;
const imgUrl=String((a.project_image||a.image_url||a.image||'')).trim();const imgBox=document.getElementById('projectImageBox');const img=document.getElementById('projectImage');if(imgUrl){img.src=imgUrl;imgBox.classList.add('show');}else{img.removeAttribute('src');imgBox.classList.remove('show');}
[['kCommande',cmd],['kFacture',billed],['kReste',reste],['kMarge',cmd*0.10],['hReste',reste]].forEach(x=>document.getElementById(x[0]).textContent=euro(x[1]));document.getElementById('kAvance').textContent=pct(finPct);document.getElementById('kFactureSub').textContent=`${pct(finPct)} du budget`;document.getElementById('kResteSub').textContent=`${pct(cmd>0?(reste/cmd*100):0)} du budget`;document.getElementById('hProd').textContent=pct(prodPct);document.getElementById('hFac').textContent=pct(finPct);document.getElementById('hGap').textContent=`${gap.toFixed(1)} pt`;document.getElementById('hAlerts').textContent=String(alerts);document.getElementById('hRappels').textContent=String(rappels);document.getElementById('hSujets').textContent=String(sujets);
const tr=renderChart(prodPct);document.getElementById('tPre').textContent=euro(tr.pre);document.getElementById('tFac').textContent=euro(tr.fac);document.getElementById('tProd').textContent=euro(tr.prod);const delay=tr.pre-tr.fac;const td=document.getElementById('tDelay');td.textContent=euro(delay);td.className='retard '+(delay>0?'warn':'ok');
const monthsElapsed=Math.max(1,Math.round(Number(((((state.board||{}).analytics||{}).kpi_project_progress||{}).elapsed_days_raw||0))/30))||new Date().getMonth()+1;const burn=boondCost>0?boondCost/monthsElapsed:0;document.getElementById('kBurn').textContent=burn>0?euro(burn)+'/mois':'-';document.getElementById('kBurnSub').textContent=burn>0?`Sur ${monthsElapsed} mois écoulé(s)`:'Donnée BOOND indisponible';
const remainingProd=Math.max(0,cmd-boondCost);if(burn>0&&remainingProd>0){const m=remainingProd/burn;document.getElementById('kEnd').textContent=monthLabelAfter(m);document.getElementById('kEndSub').textContent=`~ ${m.toFixed(1)} mois restants`;}else{document.getElementById('kEnd').textContent='-';document.getElementById('kEndSub').textContent='Projection non disponible';}
}
async function init(){await loadAffairesList();const params=new URLSearchParams(window.location.search);const pre=params.get('affaire_id')||localStorage.getItem('selectedAffaireId')||'';if(pre){let target=pre;try{await loadSelectedAffaire(target);}catch(err){const resolved=resolveAffaireId(pre);if(resolved&&resolved!==pre){await loadSelectedAffaire(resolved);}else{console.error(err);render();}}}else{render();}document.getElementById('exportBtn').addEventListener('click',()=>{if(state.selectedId)window.location.href=`/api/finance/affaire/${encodeURIComponent(state.selectedId)}/export-csv`;});}
init();
</script></body></html>"""
