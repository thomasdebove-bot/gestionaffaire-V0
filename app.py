...
const state={cacheStatus:null,affaires:[],selectedAffaireId:"",selectedAffaire:null,monthlyExpanded:false,boondImputation:null,boondForcedProjectId:""};
...
function boondOptionsHtml(b){const opts=(b&&b.project_options)||[];const current=String(state.boondForcedProjectId||(((b||{}).matched_project||{}).project_id||''));return [`<option value=''>-- Choisir un projet BOOND --</option>`].concat(opts.map(o=>`<option value='${esc(o.project_id)}' ${String(o.project_id)===current?'selected':''}>${esc(o.project_label||o.project_reference||o.project_id)} [${esc(String(o.project_id))}] (score ${esc(String(o.score||0))})</option>`)).join('');}
...
function renderBoondImputation(){const root=document.getElementById('boondImputationBox');if(!root)return;const b=state.boondImputation;const a=state.selectedAffaire;if(!a){root.className='empty';root.textContent='Sélectionnez une affaire pour charger le coût cumulé BOOND.';return;}if(!b){root.className='empty';root.textContent='Chargement des données BOOND…';return;}if(!b.matched_project){root.className='empty';root.innerHTML=`<div>Aucun projet BOOND matché pour cette affaire.</div><div style='margin-top:8px' class='small'>Choisissez provisoirement un projet BOOND dans la liste :</div><div style='margin-top:8px;display:flex;gap:8px;align-items:center'><select id='boondProjectSelect' style='padding:6px 8px;min-width:520px'>${boondOptionsHtml(b)}</select><button id='boondProjectOverrideApply' class='btn' type='button'>Forcer</button><button id='boondProjectOverrideClear' class='btn' type='button'>Retirer</button></div>`;const apply=document.getElementById('boondProjectOverrideApply');if(apply)apply.addEventListener('click',async()=>{const sel=document.getElementById('boondProjectSelect');const v=(sel&&sel.value||'').trim();state.boondForcedProjectId=v;if(state.selectedAffaireId){if(v)localStorage.setItem(`boondForcedProjectId:${state.selectedAffaireId}`,v);else localStorage.removeItem(`boondForcedProjectId:${state.selectedAffaireId}`);await loadSelectedAffaire(state.selectedAffaireId);}});const clear=document.getElementById('boondProjectOverrideClear');if(clear)clear.addEventListener('click',async()=>{state.boondForcedProjectId='';if(state.selectedAffaireId){localStorage.removeItem(`boondForcedProjectId:${state.selectedAffaireId}`);await loadSelectedAffaire(state.selectedAffaireId);}});return;}const t=b.totals||{};const cost=(t.total_cost==null)?'-':euro(t.total_cost);const margin=(t.margin_production_ht==null)?'-':euro(t.margin_production_ht);const prof=(t.profitability_production==null)?'-':`${fmt(t.profitability_production)}%`;const resources=(b.resources_imputation||[]);const selectedInfo=`<div style='padding:10px 12px' class='small'>Projet BOOND utilisé: <b>${esc((b.matched_project||{}).project_label||(b.matched_project||{}).project_reference||'-')}</b> (id: <code>${esc((b.matched_project||{}).project_id||'-')}</code>) · mode: ${esc((b.matched_project||{}).matched_on||'auto')}</div>`;const override=`<div style='padding:0 12px 10px;display:flex;gap:8px;align-items:center'><select id='boondProjectSelect' style='padding:6px 8px;min-width:520px'>${boondOptionsHtml(b)}</select><button id='boondProjectOverrideApply' class='btn' type='button'>Forcer</button><button id='boondProjectOverrideClear' class='btn' type='button'>Retirer</button></div>`;const detail=resources.length?`<table style='margin-top:10px'><thead><tr><th>Ressource</th><th class='num'>Jours</th><th class='num'>Coût imputé</th></tr></thead><tbody>${resources.map(r=>`<tr><td>${esc(r.resource_name||r.resource_id||'-')}</td><td class='num'>${fmt(r.days)}</td><td class='num'>${r.cost==null?'-':euro(r.cost)}</td></tr>`).join('')}</tbody></table>`:`<div style='padding:8px 12px' class='small'>Détail ressource indisponible.</div>`;root.className='table-wrap';root.innerHTML=`<table><thead><tr><th>Source</th><th class='num'>Coût cumulé</th><th class='num'>Marge</th><th class='num'>Rentabilité</th></tr></thead><tbody><tr><td>${esc(t.cost_source||'projects_productivity')}</td><td class='num'><strong>${cost}</strong></td><td class='num'>${margin}</td><td class='num'>${prof}</td></tr></tbody></table>${selectedInfo}<div style='padding:10px 12px' class='small'>Coût global issu de la productivité projet BOOND.</div>${override}${detail}`;const apply=document.getElementById('boondProjectOverrideApply');if(apply)apply.addEventListener('click',async()=>{const sel=document.getElementById('boondProjectSelect');const v=(sel&&sel.value||'').trim();state.boondForcedProjectId=v;if(state.selectedAffaireId){if(v)localStorage.setItem(`boondForcedProjectId:${state.selectedAffaireId}`,v);else localStorage.removeItem(`boondForcedProjectId:${state.selectedAffaireId}`);await loadSelectedAffaire(state.selectedAffaireId);}});const clear=document.getElementById('boondProjectOverrideClear');if(clear)clear.addEventListener('click',async()=>{state.boondForcedProjectId='';if(state.selectedAffaireId){localStorage.removeItem(`boondForcedProjectId:${state.selectedAffaireId}`);await loadSelectedAffaire(state.selectedAffaireId);}});} 
