...
<body><div class='wrap'>
  ...
  <div id='sectionOperationalHub' class='section'>
    <div class='hub-header'>
      <div>
        <h3>Niveau 2 — Hub d’analyse opérationnelle</h3>
        <div id='operationalHubInfo' class='small'>Cockpit dynamique par entreprise, zone/niveau et pilotage CST.</div>
      </div>
      <div class='chart-controls'>
        <label for='pointageChartSelect' class='small'>Graphique :</label>
        <select id='pointageChartSelect' class='chart-select'>
          <option value='reactivite'>Réactivité par entreprise</option>
          <option value='retard_declencheur'>Premier déclencheur de retard par zone/niveau</option>
          <option value='cout_cst'>Écart coût planifié vs coût pointé CST</option>
          <option value='avancement'>Avancement par zone/niveau</option>
        </select>
      </div>
    </div>
    <div id='pointageAnalyticsChart' class='pilot-list analytics-shell'><div class='small'>Aucune donnée</div></div>
  </div>

  <div class='section'>
    <h3>Niveau 3 — Pointage opérationnel</h3>
    <div class='chart-controls'>
...
<script>
const state={affaires:[],selectedId:'',selectedLabel:'',board:null,pointage:null,selectedPointageChart:'reactivite'};
...
function companyBarColor(name){return String(name||'').trim().toUpperCase()==='TEMPO'?'#ef8d00':'#d64545';}
function paletteColor(key){const palette=['#d64545','#ef8d00','#239b5b','#1b6ef3','#7c4dff','#0f9d94','#c65d7b','#9a6700','#42526e'];let hash=0;const src=String(key||'N/A');for(let i=0;i<src.length;i+=1){hash=((hash<<5)-hash)+src.charCodeAt(i);hash|=0;}return palette[Math.abs(hash)%palette.length];}
function parseIsoDate(value){if(!value)return null;const d=new Date(value);return Number.isNaN(d.getTime())?null:d;}
function safeRatio(value){const n=Number(value||0);return Number.isFinite(n)?Math.max(0,Math.min(100,n)):0;}
function chartInfo(message){const el=document.getElementById('operationalHubInfo');if(el)el.textContent=message;}
function renderAnalyticsHtml(html){const root=document.getElementById('pointageAnalyticsChart');if(!root)return;root.classList.remove('analytics-switch');root.innerHTML=html;void root.offsetWidth;root.classList.add('analytics-switch');}
function getOperationalSummaryTasks(){return ((state.pointage||{}).tasks||[]).filter(t=>t&&t.is_summary);}
function getOperationalActionTasks(){return ((state.pointage||{}).tasks||[]).filter(t=>t&&!t.is_summary);}
function buildZoneDelayTriggers(){const tasks=(state.pointage&&state.pointage.tasks)||[];if(!tasks.length)return[];const byParent={};for(const task of tasks){const pid=String(task.parent_task_id||'').trim();if(!pid)continue;(byParent[pid]=byParent[pid]||[]).push(task);}return tasks.filter(t=>t&&t.is_summary).map(summary=>{const children=(byParent[String(summary.task_id||'').trim()]||[]).filter(child=>child&&!child.is_summary);const candidates=[];for(const child of children){if(child.is_cet&&Array.isArray(child.cetMembers)&&child.cetMembers.length){for(const member of child.cetMembers){const cm=((child.cet||{})[member]||{});const actual=parseIsoDate(cm.actualEnd);const planned=parseIsoDate(child.end);const pointed=Number(cm.progress||0)>0||!!cm.actualEnd;const done=cm.status==='past'||!!cm.actualEnd;if(!pointed||!done||!actual||!planned)continue;const delay=Math.ceil((actual-planned)/86400000);if(delay>0)candidates.push({zone:summary.level_label||summary.name||'N/A',zoneShort:(summary.level_label||summary.name||'N/A').replace(/^.*?NIVEAU\s*/i,'').trim()||summary.level_label||summary.name||'N/A',lot:member||'N/A',delayDays:delay,actualTime:actual.getTime()});}continue;}const actual=parseIsoDate(child.actualEnd);const planned=parseIsoDate(child.end);const pointed=Number(child.progress||0)>0||!!child.actualEnd;const done=child.status==='past'||!!child.actualEnd;const owner=String(child.owner||'').trim()||'N/A';if(!pointed||!done||!actual||!planned)continue;const delay=Math.ceil((actual-planned)/86400000);if(delay>0)candidates.push({zone:summary.level_label||summary.name||'N/A',zoneShort:(summary.level_label||summary.name||'N/A').replace(/^.*?NIVEAU\s*/i,'').trim()||summary.level_label||summary.name||'N/A',lot:owner,delayDays:delay,actualTime:actual.getTime()});}if(!candidates.length)return {zone:summary.level_label||summary.name||'N/A',zoneShort:(summary.level_label||summary.name||'N/A').replace(/^.*?NIVEAU\s*/i,'').trim()||summary.level_label||summary.name||'N/A',lot:'N/A',delayDays:null,actualTime:null};candidates.sort((a,b)=>(a.actualTime-b.actualTime)||(b.delayDays-a.delayDays)||String(a.lot).localeCompare(String(b.lot),'fr'));return candidates[0];}).sort((a,b)=>String(a.zone).localeCompare(String(b.zone),'fr'));}
function buildCstGapData(){const summaries=getOperationalSummaryTasks();const byParent={};for(const task of (((state.pointage||{}).tasks)||[])){const pid=String(task.parent_task_id||'').trim();if(!pid)continue;(byParent[pid]=byParent[pid]||[]).push(task);}return summaries.map(summary=>{const rows=(byParent[String(summary.task_id||'').trim()]||[]).filter(child=>child&&!child.is_summary&&child.is_cst);const planned=rows.reduce((sum,row)=>sum+Number(row.plannedCostCst||0),0);const actual=rows.reduce((sum,row)=>sum+Number(row.actualCostCst||0),0);return {zone:summary.level_label||summary.name||'N/A',zoneShort:(summary.level_label||summary.name||'N/A').replace(/^.*?NIVEAU\s*/i,'').trim()||summary.level_label||summary.name||'N/A',planned,actual,gap:actual-planned};}).filter(item=>item.planned>0||item.actual>0);}
function buildProgressData(){const summaries=getOperationalSummaryTasks();const byParent={};for(const task of (((state.pointage||{}).tasks)||[])){const pid=String(task.parent_task_id||'').trim();if(!pid)continue;(byParent[pid]=byParent[pid]||[]).push(task);}return summaries.map(summary=>{const children=(byParent[String(summary.task_id||'').trim()]||[]).filter(child=>child&&!child.is_summary);if(!children.length)return {zone:summary.level_label||summary.name||'N/A',zoneShort:(summary.level_label||summary.name||'N/A').replace(/^.*?NIVEAU\s*/i,'').trim()||summary.level_label||summary.name||'N/A',progress:null,weightedHours:0};let weighted=0,totalWeight=0;for(const child of children){const weight=Math.max(Number(child.planned_hours||0),1);weighted+=safeRatio(child.progress)*weight;totalWeight+=weight;}return {zone:summary.level_label||summary.name||'N/A',zoneShort:(summary.level_label||summary.name||'N/A').replace(/^.*?NIVEAU\s*/i,'').trim()||summary.level_label||summary.name||'N/A',progress:totalWeight?weighted/totalWeight:null,weightedHours:totalWeight};}).sort((a,b)=>String(a.zone).localeCompare(String(b.zone),'fr'));}
function renderOperationalHub(){try{const select=document.getElementById('pointageChartSelect');const chart=(select&&select.value)||state.selectedPointageChart||'reactivite';const board=state.board||{};const pointage=state.pointage||{};state.selectedPointageChart=chart;if(chart==='reactivite'){const p=(board&&board.kpis_pilotage)||{};const items=(p.reactivity_by_company||[]).map(item=>({name:item.name||'N/A',value:Number(item.avg_gap_days||0)}));if(!items.length){chartInfo('Réactivité calculée à partir des tâches clôturées avec échéance connue.');renderAnalyticsHtml("<div class='small'>Pas assez de tâches clôturées avec échéance.</div>");return;}const max=Math.max(1,...items.map(x=>Math.abs(x.value)));chartInfo('Comparaison des performances entreprises sur la base du délai moyen constaté après réunion MÉTRONOME.');renderAnalyticsHtml(`<div class='analytics-grid'>${items.map(item=>`<article class='analytics-card'><div class='analytics-title'>${esc(item.name)}</div><div class='bar-chart' style='min-height:210px;padding-top:0'><div class='bar-col' style='min-width:100%'><div class='bar-track-vertical'><div class='bar-vertical' title='${esc(item.name)} · ${item.value.toFixed(1)} jour(s)' style='height:${Math.max(16,(Math.abs(item.value)/max)*210)}px;background:${companyBarColor(item.name)};'></div></div><div class='bar-meta'><strong>${item.value.toFixed(1)} j</strong></div></div></div></article>`).join('')}</div><div class='legend-inline'><span><i class='legend-dot' style='background:#ef8d00'></i>TEMPO</span><span><i class='legend-dot' style='background:#d64545'></i>Autres entreprises</span></div>`);return;}if(!pointage.tasks||!pointage.tasks.length){chartInfo('Importez un planning et du pointage pour activer ce cockpit.');renderAnalyticsHtml("<div class='small'>Aucune donnée de pointage disponible.</div>");return;}if(chart==='retard_declencheur'){const items=buildZoneDelayTriggers();chartInfo('Identification du premier lot qui déclenche un retard réel par zone/niveau, uniquement sur les tâches terminées et pointées.');if(!items.length){renderAnalyticsHtml("<div class='small'>Aucune zone exploitable.</div>");return;}const withDelay=items.filter(item=>item.delayDays!=null);const max=Math.max(1,...withDelay.map(item=>Number(item.delayDays||0)),1);renderAnalyticsHtml(`<div class='bar-chart'>${items.map(item=>{if(item.delayDays==null)return `<div class='bar-col'><div class='bar-track-vertical'><div class='bar-vertical' style='height:20px;background:#d9e1ef'></div></div><div class='bar-label'>${esc(item.zoneShort)}</div><div class='bar-meta'>Lot: <strong>N/A</strong></div><div class='bar-meta'>Aucune dérive</div></div>`;const color=paletteColor(item.lot);return `<div class='bar-col'><div class='bar-track-vertical'><div class='bar-vertical' title='Zone: ${esc(item.zoneShort)} · Lot: ${esc(item.lot)} · Retard: +${item.delayDays} j' style='height:${Math.max(22,(Number(item.delayDays||0)/max)*220)}px;background:${color}'></div></div><div class='bar-label'>${esc(item.zoneShort)}</div><div class='bar-meta'>Lot: <strong>${esc(item.lot)}</strong></div><div class='bar-meta'>Retard: +${item.delayDays} j</div></div>`;}).join('')}</div><div class='legend-inline'>${[...new Set(withDelay.map(item=>item.lot))].slice(0,6).map(lot=>`<span><i class='legend-dot' style='background:${paletteColor(lot)}'></i>${esc(lot)}</span>`).join('')}</div>`);return;}if(chart==='cout_cst'){const items=buildCstGapData();chartInfo('Comparaison des coûts CST planifiés et réellement pointés par zone/niveau.');if(!items.length){renderAnalyticsHtml("<div class='small'>Aucune tâche CST pointée ou planifiée.</div>");return;}const values=[];for(const item of items){values.push(item.planned);values.push(item.actual);}const max=Math.max(1,...values);renderAnalyticsHtml(`<div class='bar-chart'>${items.map(item=>{const plannedH=Math.max(16,(item.planned/max)*220);const actualH=Math.max(16,(item.actual/max)*220);const actualColor=item.gap>0?'#d64545':item.gap<0?'#239b5b':'#ef8d00';return `<div class='bar-col'><div style='display:flex;justify-content:center;gap:10px;align-items:flex-end;height:240px'><div title='Planifié ${item.planned.toFixed(0)} €' class='bar-vertical' style='height:${plannedH}px;background:#9db4de;border-radius:12px 12px 0 0'></div><div title='Réel ${item.actual.toFixed(0)} €' class='bar-vertical' style='height:${actualH}px;background:${actualColor};border-radius:12px 12px 0 0'></div></div><div class='bar-label'>${esc(item.zoneShort)}</div><div class='bar-meta'>Planifié: <strong>${euro(item.planned)}</strong></div><div class='bar-meta'>Réel: <strong>${euro(item.actual)}</strong></div><div class='bar-meta'>Écart: <strong style='color:${actualColor}'>${item.gap>=0?'+':''}${euro(item.gap)}</strong></div></div>`;}).join('')}</div><div class='legend-inline'><span><i class='legend-dot' style='background:#9db4de'></i>Planifié</span><span><i class='legend-dot' style='background:#d64545'></i>Réel au-dessus</span><span><i class='legend-dot' style='background:#239b5b'></i>Réel en-dessous</span><span><i class='legend-dot' style='background:#ef8d00'></i>Réel aligné</span></div>`);return;}if(chart==='avancement'){const items=buildProgressData();chartInfo('Pilotage de l’avancement réel par zone/niveau via moyenne pondérée du % achevé sur les tâches enfants.');if(!items.length){renderAnalyticsHtml("<div class='small'>Aucune zone exploitable.</div>");return;}renderAnalyticsHtml(`<div class='gauge-list'>${items.map(item=>{if(item.progress==null)return `<div class='gauge-row'><div class='gauge-label'>${esc(item.zoneShort)}</div><div class='gauge-track'><div class='gauge-fill' style='width:0%'></div></div><div class='gauge-value'>N/A</div></div>`;const value=safeRatio(item.progress);const tone=value>=75?'good':value>=45?'warn':'bad';return `<div class='gauge-row'><div class='gauge-label'>${esc(item.zoneShort)}</div><div class='gauge-track'><div class='gauge-fill ${tone}' style='width:${value}%'></div></div><div class='gauge-value'>${value.toFixed(0)}%</div></div>`;}).join('')}</div>`);return;}chartInfo('Sélectionnez un graphique du hub opérationnel.');renderAnalyticsHtml("<div class='small'>Graphique inconnu.</div>");}catch(err){console.error('renderOperationalHub',err);chartInfo('Le hub opérationnel est momentanément indisponible.');renderAnalyticsHtml("<div class='small'>Impossible d'afficher le graphique sélectionné pour le moment.</div>");}}
...
function renderBoard(){const b=state.board||{};const p=(b&&b.kpis_pilotage)||{};const k=(b&&b.kpis)||{};const hasMatch=!!((b&&b.match_debug&&((b.match_debug.matched_project_name||'')||(Number(b.match_debug.match_score||0)>0)))&&!b.warning);const secPilot=document.getElementById('sectionPilot');const secHub=document.getElementById('sectionOperationalHub');if(secPilot)secPilot.style.display=hasMatch?'block':'none';if(secHub)secHub.style.display=hasMatch?'block':'none';setText('projectTitle',b.project_name||'-');setText('projectDesc',b.project_description||'-');const img=document.getElementById('projectImage');if(img){const src=String(b.project_image||'').trim();if(src){img.src=src;img.style.display='block';}else{img.removeAttribute('src');img.style.display='none';}}renderMatchDebug(b);setText('kOpen',Number(k.open_topics||0));setText('kOverdue',Number(k.overdue_topics||0));setText('kTopRappels',Number(p.reminders_open_count||0));setText('kDateRef',p.date_reference||p.reference_date||'-');setText('kRappelsDate',Number(p.rappels_ouverts_a_date||p.reminders_open_count||0));setText('kASuivre',Number(p.a_suivre_ouverts||p.followups_open_count||0));renderBars('kByStatus',k.by_status||[]);renderTable(b.rows||[]);renderProjectTimeline(b);renderOperationalHub();const companyRows=(p.rappels_cumules_par_entreprise||p.reminders_by_company||[]);setHtml('pilotByCompany',companyRows.length?companyRows.map(r=>{const nm=r.name||r.company||'';return `<button class='pilot-row-btn pilot-row' data-company='${esc(nm)}'><span class='company-cell'>${companyLogoHtml({name:nm,logo:r.logo||''})}<span class='name'>${esc(nm)}</span></span><span><strong>${Number(r.count||0)}</strong> rappels</span></button>`;}).join(''):"<div class='small'>Aucune donnée</div>");Array.from(document.querySelectorAll('.pilot-row-btn')).forEach(btn=>btn.addEventListener('click',()=>showReminderModal(btn.getAttribute('data-company')||'',b)));}
...
async function loadPointage(){if(!state.selectedId)return;try{const d=await pointageApi(`/api/project-management/pointage?affaire_id=${encodeURIComponent(state.selectedId)}`);state.pointage=d;renderPointage();renderOperationalHub();}catch(e){console.warn(e);}}
...
async function init(){
  ...
  document.getElementById('btnExpandAll').addEventListener('click',async()=>{const pointageTasks=((state.pointage||{}).tasks)||[];const lvls=[...new Set(pointageTasks.filter(x=>x.is_summary).map(x=>x.level_label||x.level||'Général'))];await savePointage({}, {expanded_levels:lvls});});
  ...
  const pointageChartSelect=document.getElementById('pointageChartSelect');if(pointageChartSelect){pointageChartSelect.value=state.selectedPointageChart;pointageChartSelect.addEventListener('change',()=>{state.selectedPointageChart=pointageChartSelect.value||'reactivite';renderOperationalHub();});}
  try{
    await loadAffaires('');
...
.kpis{margin-top:14px;padding:14px}.kpi-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px}.k{border:1px solid var(--line);border-radius:14px;padding:14px;background:#fbfdff}.k .l{color:var(--muted);font-size:12px;text-transform:uppercase;font-weight:700}.k .v{font-size:34px;font-weight:900;margin-top:8px}
