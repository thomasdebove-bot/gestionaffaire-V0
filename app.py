...
<body><div class='wrap'><div class='board-shell'>
  ...
  <section id='section-planning' class='board-section'>
    ...
    <div id='chart-planning-cst' class='chart-box' style='margin-top:14px'><div class='chart-title'>Chronologie des tâches CST</div><div id='planningCst' class='timeline-macro'></div><div id='planningCstPrint' class='print-only'></div></div>
  ...
  <section id='section-finance' class='board-section'>
    ...
    <div id='chart-finance-curve' class='chart-box'><div class='legend'><span class='pre'>Prévisionnel</span><span class='fac'>Facturation</span><span class='prod'>Production</span></div><svg id='financeChart' class='finance-svg' viewBox='0 0 980 320' preserveAspectRatio='xMidYMid meet'></svg><div id='financeSummary'></div><div id='financePrintSummary' class='print-only'></div></div>
...
function computePlanningDatasets(tasks){const actionable=(tasks||[]).filter(t=>!t.is_summary);const byPerimeter={};for(const t of actionable){const key=String(t.perimeter||'Général').trim()||'Général';(byPerimeter[key]=byPerimeter[key]||[]).push(t);}const delayTrigger=Object.entries(byPerimeter).map(([label,list])=>{const late=list.filter(t=>Number(t.delayDays||0)>0).sort((a,b)=>Number(b.delayDays||0)-Number(a.delayDays||0));const it=late[0];return it?{label,subtitle:`Première tâche en retard : ${it.name||'-'} · ${Number(it.delayDays||0)} j`,side:String(it.owner||'Non attribué').trim()||'Non attribué'}:null;}).filter(Boolean).sort((a,b)=>a.label.localeCompare(b.label,'fr'));const perimeterProgress=Object.entries(byPerimeter).map(([label,list])=>{const avg=list.length?list.reduce((s,t)=>s+Number(t.progress||0),0)/list.length:0;const ordered=list.slice().sort((a,b)=>{const ad=parseDateOnly(a.start||'')||parseDateOnly(a.end||'');const bd=parseDateOnly(b.start||'')||parseDateOnly(b.end||'');return (ad?ad.getTime():Number.MAX_SAFE_INTEGER)-(bd?bd.getTime():Number.MAX_SAFE_INTEGER);});const progressed=ordered.filter(t=>Number(t.progress||0)>0||!!t.actualEnd);const lastProgressed=progressed.length?progressed[progressed.length-1]:null;let blocker=null;if(lastProgressed){const pivot=ordered.findIndex(t=>String(t.task_id||'')===String(lastProgressed.task_id||''));if(pivot>=0)blocker=ordered.slice(pivot+1).find(t=>Number(t.progress||0)<100&&!t.actualEnd)||null;}if(!blocker)blocker=ordered.find(t=>Number(t.progress||0)<100&&!t.actualEnd)||null;const subtitle=avg<100&&blocker?`Tâche bloquante : ${blocker.name||'-'} · ${String(blocker.owner||'Non attribué').trim()||'Non attribué'}`:`${list.length} tâche(s)`;return {label,value:avg,subtitle,color:'linear-gradient(90deg,#1b6ef3,#5da2ff)'};}).sort((a,b)=>b.value-a.value).slice(0,8);const stats=(includeUnpointed)=>{const rec={};const push=(name,delay)=>{const k=String(name||'').trim();if(!k||k==='Non attribué'||k.includes('+'))return;const r=rec[k]||(rec[k]={sum:0,count:0});r.sum+=Number(delay||0);r.count+=1;};for(const t of actionable){if(t.is_cet&&Array.isArray(t.cetMembers)&&t.cetMembers.length){for(const m of t.cetMembers){const cm=((t.cet||{})[m]||{});const pointed=Number(cm.progress||0)>0||!!cm.actualEnd;const ok=includeUnpointed?(cm.status==='late'||cm.status==='soon'||cm.status==='past'||pointed):((cm.status==='past'||!!cm.actualEnd)&&pointed);if(ok)push(m,cm.delayDays||0);}continue;}const pointed=Number(t.progress||0)>0||!!t.actualEnd;const ok=includeUnpointed?(t.status==='late'||t.status==='soon'||t.status==='past'||pointed):((t.status==='past'||!!t.actualEnd)&&pointed);if(ok)push(t.owner,t.delayDays||0);}return Object.entries(rec).map(([name,v])=>({name,avg:v.count?v.sum/v.count:0})).sort((a,b)=>b.avg-a.avg).slice(0,6);};const today=new Date();today.setHours(0,0,0,0);const clamp=v=>Math.max(0,Math.min(1,v));const actualRatio=actionable.length?clamp(actionable.reduce((sum,t)=>sum+Math.max(0,Math.min(100,Number(t.progress||0))),0)/(actionable.length*100)):0;const plannedRatio=actionable.length?clamp(actionable.reduce((sum,t)=>{const start=parseDateOnly(t.start||'');const end=parseDateOnly(t.end||'')||start;if(!start&&!end)return sum;const s=start||end;const e=end||start;if(today<=s)return sum;if(today>=e)return sum+1;const duration=Math.max(1,Math.round((e-s)/(1000*60*60*24)));const elapsed=Math.max(0,Math.round((today-s)/(1000*60*60*24)));return sum+clamp(elapsed/duration);},0)/actionable.length):0;const cst=actionable.filter(t=>normalizeText(t.owner||'')==='cst').map(t=>({label:String(t.perimeter||'Sans périmètre').trim()||'Sans périmètre',task:String(t.name||'').trim()||'Tâche CST',start:parseDateOnly(t.start||''),end:parseDateOnly(t.end||'')||parseDateOnly(t.start||''),delay:Number(t.delayDays||0),owner:String(t.owner||'').trim()||'CST'})).filter(x=>x.start||x.end).sort((a,b)=>(a.start||a.end)-(b.start||b.end));return {delayTrigger,perimeterProgress,resources:stats(false),resourcesToday:stats(true),expected:[{label:'Avancement % planning attendu',value:plannedRatio*100,subtitle:'L’avancement théorique à date si aucun retard n’existe.'},{label:'Avancement % planning pointé',value:actualRatio*100,subtitle:'Le planning pointé à date.',color:'linear-gradient(90deg,#ef8d00,#ffbc62)'}],cst};}
function renderCstTimeline(rows){const root=document.getElementById('planningCst');const printRoot=document.getElementById('planningCstPrint');if(!root)return;if(!rows.length){root.innerHTML=`<div class='company-row'><span class='name'>Aucune tâche CST datée</span><span class='count'>0</span></div>`;if(printRoot)printRoot.innerHTML=root.innerHTML;return;}const startMin=new Date(Math.min(...rows.map(r=>(r.start||r.end).getTime())));const endMax=new Date(Math.max(...rows.map(r=>(r.end||r.start).getTime())));const total=Math.max(1,Math.round((endMax-startMin)/(1000*60*60*24)));const today=new Date();today.setHours(0,0,0,0);const todayLeft=Math.max(0,Math.min(100,((today-startMin)/(1000*60*60*24))/total*100));root.innerHTML=rows.map(r=>{const s=r.start||r.end;const e=r.end||r.start;const offset=Math.max(0,Math.round((s-startMin)/(1000*60*60*24)));const span=Math.max(1,Math.round((e-s)/(1000*60*60*24))+1);const left=(offset/total)*100;const width=Math.max(2,(span/total)*100);return `<div class='timeline-macro-row'><div class='timeline-macro-head'><div><div class='timeline-macro-title'>${esc(r.label)}</div><div class='timeline-macro-sub'>${esc(r.task)} · ${esc(r.owner)}${r.delay>0?` · ${r.delay} j`:''}</div></div><div class='timeline-macro-date'>${esc(fmtDate(s.toISOString().slice(0,10)))} → ${esc(fmtDate(e.toISOString().slice(0,10)))}</div></div><div class='timeline-macro-track'><div class='timeline-macro-marker' style='left:${todayLeft}%'></div><div class='timeline-macro-fill' style='left:${left}%;width:${width}%'></div></div><div class='timeline-macro-labels'><span>${esc(fmtDate(startMin.toISOString().slice(0,10)))}</span><span>${esc(fmtDate(endMax.toISOString().slice(0,10)))}</span></div></div>`;}).join('');if(printRoot){printRoot.innerHTML=`<table class='board-table'><thead><tr><th>Périmètre</th><th>Tâche</th><th>Début</th><th>Fin</th><th>Retard</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.label)}</td><td>${esc(r.task)}</td><td>${esc(fmtDate(r.start&&r.start.toISOString().slice(0,10)))}</td><td>${esc(fmtDate(r.end&&r.end.toISOString().slice(0,10)))}</td><td>${esc(String(r.delay||0))} j</td></tr>`).join('')}</tbody></table>`;}}
function renderFinance(affaire){const billed=Number(affaire.facturation_totale||((affaire.anteriorite||0)+(affaire.facture_2026||affaire.facturation_cumulee_2026||0)));const cmd=Number(affaire.commande_ht||0);const remain=Number(affaire.reste_a_facturer||0);const missions=(affaire.missions||[]);const data=MONTHS.map(m=>({label:MONTH_LABELS[m],pre:Number((((affaire.mensuel||{})[m]||{}).previsionnel)||0),fac:Number((((affaire.mensuel||{})[m]||{}).facture)||0),prod:Number(((affaire.production_cumulee_mensuelle||{})[m])||0)}));let cp=0,cf=Number(affaire.anteriorite||0);const cum=data.map(d=>{cp+=d.pre;cf+=d.fac;return {l:d.label,pre:cp,fac:cf,prod:d.prod};});const missionPalettePre=['#cfdcf6','#b9cbed','#a4bae4','#8faada','#7a99d1','#6488c7'];const missionPaletteFac=['#ffd29a','#ffbf73','#ffab4d','#ff9826','#f58a00','#db7b00'];const missionCum=missions.map((mission,idx)=>{let preRun=0,facRun=Number(mission.anteriorite||0);return {key:String(mission.tag||mission.label||idx),label:String(mission.tag||mission.label||`Mission ${idx+1}`),preColor:missionPalettePre[idx%missionPalettePre.length],facColor:missionPaletteFac[idx%missionPaletteFac.length],months:MONTHS.map(month=>{preRun+=Number((((mission.mensuel||{})[month]||{}).previsionnel)||0);facRun+=Number((((mission.mensuel||{})[month]||{}).facture)||0);return {pre:preRun,fac:facRun};})};});const max=Math.max(1,...cum.flatMap(x=>[x.pre,x.fac,x.prod]));const left=54,top=18,width=892,height=236,step=width/Math.max(1,cum.length),barW=Math.max(10,Math.min(24,step*0.3));let out='';for(let i=0;i<=4;i++){const y=top+(height/4)*i;const val=Math.round(max*(1-i/4));out+=`<line x1='${left}' y1='${y}' x2='${left+width}' y2='${y}' stroke='#e6edf7'/><text x='${left-8}' y='${y+4}' text-anchor='end' fill='#5f6f88' font-size='11'>${val.toLocaleString('fr-FR')} €</text>`;}cum.forEach((d,i)=>{const x=left+i*step+step*0.5;let preBase=0,facBase=0;if(missionCum.length){missionCum.forEach(mission=>{const monthVals=mission.months[i]||{pre:0,fac:0};const preSegment=Math.max(0,monthVals.pre-preBase);const facSegment=Math.max(0,monthVals.fac-facBase);if(preSegment>0){const y=top+height-((preBase+preSegment)/max)*height;const h=(preSegment/max)*height;out+=`<rect x='${x-barW-4}' y='${y}' width='${barW}' height='${Math.max(1,h)}' rx='4' fill='${mission.preColor}'><title>${esc(mission.label)} — Prévisionnel cumulé ${d.l}: ${euro(monthVals.pre)} (segment: ${euro(preSegment)})</title></rect>`;}if(facSegment>0){const y=top+height-((facBase+facSegment)/max)*height;const h=(facSegment/max)*height;out+=`<rect x='${x+4}' y='${y}' width='${barW}' height='${Math.max(1,h)}' rx='4' fill='${mission.facColor}'><title>${esc(mission.label)} — Facturation cumulée ${d.l}: ${euro(monthVals.fac)} (segment: ${euro(facSegment)})</title></rect>`;}preBase=monthVals.pre;facBase=monthVals.fac;});}else{const preH=(d.pre/max)*height;const facH=(d.fac/max)*height;out+=`<rect x='${x-barW-4}' y='${top+height-preH}' width='${barW}' height='${Math.max(1,preH)}' rx='4' fill='#9db4de'><title>Prévisionnel cumulé ${d.l}: ${euro(d.pre)}</title></rect>`;out+=`<rect x='${x+4}' y='${top+height-facH}' width='${barW}' height='${Math.max(1,facH)}' rx='4' fill='#ef8d00'><title>Facturation cumulée ${d.l}: ${euro(d.fac)}</title></rect>`;}if(i%2===0)out+=`<text x='${x}' y='${top+height+18}' fill='#5f6f88' font-size='11' text-anchor='middle'>${d.l}</text>`;});const prodPts=cum.map((d,i)=>`${left+i*step+step*0.5},${top+height-(d.prod/max)*height}`).join(' ');out+=`<polyline points='${prodPts}' fill='none' stroke='#239b5b' stroke-width='3' stroke-dasharray='6 5'><title>Production cumulée</title></polyline>`;document.getElementById('financeChart').innerHTML=out;const maxFinal=Math.max(cmd,billed,remain,1);document.getElementById('financeSummary').innerHTML=[['Facturation cumulée',billed,'fac'],['Reste à facturer',remain,'prod']].map(r=>`<div class='summary-row'><div>${r[0]}</div><div class='summary-track'><div class='summary-fill ${r[2]}' style='width:${Math.min(100,(Number(r[1]||0)/maxFinal)*100)}%'></div></div><div style='text-align:right;font-weight:900'>${euro(r[1])}</div></div>`).join('');const printSummary=document.getElementById('financePrintSummary');if(printSummary){printSummary.innerHTML=`<table class='board-table'><thead><tr><th>Indicateur</th><th>Montant</th></tr></thead><tbody><tr><td>Commande HT</td><td class='num'>${euro(cmd)}</td></tr><tr><td>Facturation cumulée</td><td class='num'>${euro(billed)}</td></tr><tr><td>Reste à facturer</td><td class='num'>${euro(remain)}</td></tr></tbody></table>`;}document.getElementById('financeMissions').innerHTML=missions.length?missions.map(m=>`<tr><td>${esc(m.tag||'')}</td><td>${esc(m.label||'')}</td><td>${esc(m.numero||'')}</td><td class='num'>${euro(m.commande_ht)}</td><td class='num'>${euro(m.facturation_totale||((m.anteriorite||0)+(m.facture_2026||m.facturation_cumulee_2026||0)))}</td><td class='num'>${euro(m.reste_a_facturer)}</td><td class='num'>${euro(m.total_previsionnel)}</td><td class='num'>${euro(m.total_facture)}</td></tr>`).join(''):`<tr><td colspan='8'>Aucune mission détaillée.</td></tr>`;}
