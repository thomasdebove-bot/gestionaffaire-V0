...
async function loadBoard(id){const requested=id||state.selectedId||'';if(!requested){state.selectedId='';state.selectedLabel='';state.board=null;syncProjectSelectionUi();renderBoard();return;}const selected=resolveAffaire(requested);const aid=(selected&&selected.affaire_id)||requested;state.selectedId=aid;state.selectedLabel=(selected&&selected.display_name)||state.selectedLabel||'';localStorage.setItem('selectedAffaireId',aid);if(!state.selectedLabel)await ensureSelectedAffaireContext(aid);syncProjectSelectionUi();showLoading(true,'Chargement de la vue projet…');try{let b;try{b=await api(`/api/project-management/board?affaire_id=${encodeURIComponent(aid)}`);}catch(errById){const msg=String((errById&&errById.message)||'');const affairMeta=state.selectedLabel?null:await ensureSelectedAffaireContext(aid);const fallbackName=String(state.selectedLabel||((affairMeta&&affairMeta.display_name)||(affairMeta&&affairMeta.affaire)||'')).trim();if(fallbackName&&(msg.includes('Affaire introuvable')||msg.includes('affaire_id ou affaire_name requis')||msg.includes('404'))){const alt=String(fallbackName).split(' - ').slice(-1)[0]||fallbackName;b=await api(`/api/project-management/board?affaire_name=${encodeURIComponent(alt)}`);}else{throw errById;}}state.board=b;renderBoard();}catch(err){console.error(err);state.board=null;renderBoard();showPageError(err);}finally{showLoading(false);}}
...
function syncProjectSelectionUi(){const search=document.getElementById('searchInput');if(search)search.value=state.selectedLabel||'';const sel=document.getElementById('affaireSelect');if(!sel)return;const currentId=String(state.selectedId||'').trim();const currentLabel=String(state.selectedLabel||currentId||'').trim();if(currentId&&!Array.from(sel.options).some(opt=>String(opt.value||'')===currentId)){const opt=document.createElement('option');opt.value=currentId;opt.textContent=currentLabel||currentId;sel.appendChild(opt);}sel.value=currentId||'';}
async function ensureSelectedAffaireContext(id){const affairId=String(id||state.selectedId||'').trim();if(!affairId)return null;if(state.selectedLabel)return {display_name:state.selectedLabel};try{const detail=await api(`/api/finance/affaire/${encodeURIComponent(affairId)}`);const affaire=(detail&&detail.affaire)||{};state.selectedId=affairId;state.selectedLabel=String(affaire.display_name||affaire.affaire||affairId).trim();localStorage.setItem('selectedAffaireId',affairId);syncProjectSelectionUi();return affaire;}catch(err){console.warn('Impossible de réhydrater le contexte affaire', err);syncProjectSelectionUi();return null;}}
