...
class PointageService:
    ...
    def parse_planning(self, raw: bytes) -> List[Dict[str, Any]]:
        ...
        c_perimeter = self._get_col_by_position(first, 9) or self._find_col(first, ["perimetre", "périmètre", "zone", "niveau", "scope"])
...
function groupTasksByPerimeter(tasks,{includeSummary=false}={}){const groups={};for(const t of (tasks||[])){if(!includeSummary&&t.is_summary)continue;const perimeter=String(t.perimeter||'').trim();if(!perimeter)continue;const key=normalizeText(perimeter)||perimeter;(groups[key]=groups[key]||{label:perimeter,tasks:[]}).tasks.push(t);}return Object.values(groups);}
