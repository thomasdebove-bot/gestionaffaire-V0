...
<body><div class='wrap'><div class='board-shell'>
  <div class='board-header'>
    <div class='board-brand'><img src='/assets/logo-tempo' alt='Tempo'><div class='board-kicker'>BOARD CLIENT DU JJ/MM/AA</div></div>
  ...
  <div class='composer'>
    <div class='composer-title'>Gestion de projet | Composition du board</div>
  ...
  <section id='section-planning' class='board-section'>
    ...
    <div id='chart-planning-resources' class='chart-box' style='margin-top:14px'><div class='chart-title'>Retard moyen par ressource</div><div id='planningResources' class='reactivity-bars'></div></div>
...
function restoreNotes(){let data={};try{data=JSON.parse(localStorage.getItem(projectStorageKey('board-notes'))||'{}')||{};}catch(_){data={};}document.querySelectorAll('[data-notes]').forEach(el=>{const key=el.getAttribute('data-notes');el.value=data[key]||'';const wrap=el.closest('.notes-wrap');if(wrap&&el.value)wrap.classList.add('open');el.addEventListener('input',saveNotes);});}
function saveNotes(){const data={};document.querySelectorAll('[data-notes]').forEach(el=>data[el.getAttribute('data-notes')]=el.value||'');localStorage.setItem(projectStorageKey('board-notes'),JSON.stringify(data));}
...
function renderCompanyRows(rows){return rows.length?rows.map(r=>`<div class='company-row'><span class='company-cell'>${companyLogoHtml(r)}<span class='name'>${esc(r.name||r.company||'-')}</span></span><span class='count'><strong>${Number(r.count||0)}</strong> rappel(s)</span></div>`).join(''):`<div class='company-row'><span class='name'>Aucune donnée</span><span class='count'>0</span></div>`;}
