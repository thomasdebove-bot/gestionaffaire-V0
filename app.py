...
async function loadBoard(id){const aid=id||state.selectedId||'';if(!aid){state.selectedId='';state.selectedLabel='';state.board=null;renderBoard();return;}state.selectedId=aid;const sel=document.getElementById('affaireSelect');if(sel)sel.value=aid;const selected=(state.affaires||[]).find(a=>String(a.affaire_id||'')===String(aid||''));state.selectedLabel=(selected&&selected.display_name)||state.selectedLabel||'';localStorage.setItem('selectedAffaireId',aid);showLoading(true,'Chargement de la vue projet…');try{let b;try{b=await api(`/api/project-management/board?affaire_id=${encodeURIComponent(aid)}`);}catch(errById){const msg=String((errById&&errById.message)||'');if(state.selectedLabel&&(msg.includes('Affaire introuvable')||msg.includes('affaire_id ou affaire_name requis')||msg.includes('404'))){b=await api(`/api/project-management/board?affaire_name=${encodeURIComponent(state.selectedLabel)}`);}else{throw errById;}}state.board=b;renderBoard();}catch(err){console.error(err);state.board=null;renderBoard();showPageError(err);}finally{showLoading(false);}}
...
async function init(){
  try{
    ...
    if(initialId){
      ...
      const found=state.affaires.find(a=>String(a.affaire_id||'')===String(initialId||''));
      if(found){
