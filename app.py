...
function renderDelayTriggerChart(root,tasks){if(!root)return;const childrenByParent=pointageChildrenByParent(tasks);const summaryTasks=(tasks||[]).filter(t=>t.is_summary);const items=[];for(const per of summaryTasks){const perimeter=String(per.perimeter||per.name||'').trim();if(!perimeter)continue;const children=(childrenByParent[String(per.task_id||'')]||[]).filter(t=>!t.is_summary && (t.status==='past' || !!t.actualEnd));if(!children.length)continue;const lateChildren=children.map(t=>({...t,delay:Number(t.delayDays||0)})).filter(t=>t.delay>0).sort((a,b)=>{const ad=parseDateOnly(a.actualEnd||'');const bd=parseDateOnly(b.actualEnd||'');const at=ad?ad.getTime():Number.MAX_SAFE_INTEGER;const bt=bd?bd.getTime():Number.MAX_SAFE_INTEGER;return at-bt || a.delay-b.delay || String(a.name||'').localeCompare(String(b.name||''),'fr');});const trigger=lateChildren[0];if(!trigger)continue;items.push({label:perimeter,lot:String(trigger.owner||'Non attribué').trim()||'Non attribué',delay:trigger.delay,task:trigger.name||''});}if(!items.length){root.innerHTML="<div class='small'>Aucun déclencheur de retard détecté sur les tâches terminées.</div>";return;}root.innerHTML=`<div style='display:flex;flex-direction:column;gap:10px'>${items.map(x=>`<div class='pilot-row' style='align-items:center'><div style='flex:1;min-width:0'><div style='font-weight:800'>${esc(x.label)}</div><div class='small'>Première tâche en retard : ${esc(x.task||'-')} · ${esc(String(x.delay))} j</div></div><div style='min-width:140px;text-align:right;font-weight:900;color:#d64545'>${esc(x.lot)}</div></div>`).join('')}</div>`;}
function renderPerimeterProgressChart(root,tasks){if(!root)return;const childrenByParent=pointageChildrenByParent(tasks);const summaryTasks=(tasks||[]).filter(t=>t.is_summary);const items=[];for(const per of summaryTasks){const perimeter=String(per.perimeter||per.name||'').trim();if(!perimeter)continue;const children=(childrenByParent[String(per.task_id||'')]||[]).filter(t=>!t.is_summary);if(!children.length)continue;const avg=children.reduce((sum,t)=>sum+Number(t.progress||0),0)/children.length;items.push({label:perimeter,value:Math.max(0,Math.min(100,avg))});}items.sort((a,b)=>b.value-a.value||a.label.localeCompare(b.label,'fr'));if(!items.length){root.innerHTML="<div class='small'>Aucune donnée de périmètre disponible.</div>";return;}root.innerHTML=items.map(x=>`<div style='margin:10px 0'><div style='display:flex;justify-content:space-between;gap:12px;margin-bottom:6px'><span style='font-weight:700'>${esc(x.label)}</span><strong>${x.value.toFixed(1)}%</strong></div><div style='height:14px;border-radius:999px;background:#edf1f8;overflow:hidden'><div style='height:100%;width:${x.value}%;background:linear-gradient(90deg,#1b6ef3,#5da2ff)'></div></div></div>`).join('');}
