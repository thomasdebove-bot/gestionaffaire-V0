...
<body><div class='wrap'>
  ...
  <div id='sectionPlanningAnalysis' class='section' style='display:none'>
    ...
    <div class='chart-controls'>
      ...
      <select id='planningChartSelect' class='chart-select'>
        ...
        <option value='cst-timeline'>Chronologie des tâches CST</option>
      </select>
    </div>
  ...
  <div id='perimeterModal' class='pm-modal' style='display:none'>
    <div class='pm-modal-box narrow'>
      <div style='display:flex;justify-content:space-between;align-items:center;gap:12px'>
        <h3 id='perimeterModalTitle' style='margin:0'>Détail du périmètre</h3>
        <button id='perimeterModalClose' class='btn'>Fermer</button>
      </div>
      <div id='perimeterModalBody' style='margin-top:12px'></div>
    </div>
  </div>
...
function taskDisplayDelay(delay){const d=Number(delay||0);if(d>0)return `${d} j de retard`;if(d<0)return `${Math.abs(d)} j d'avance`;return 'À l'heure';}
function delayTriggerForPerimeter(tasks){const completed=(tasks||[]).filter(t=>!t.is_summary).map(t=>({...t,delay:Number(t.delayDays||0)})).filter(t=>t.status==='past' || !!t.actualEnd);const delayed=completed.filter(t=>t.delay>0).sort((a,b)=>{const ad=parseDateOnly(a.actualEnd||a.end||'');const bd=parseDateOnly(b.actualEnd||b.end||'');return ((ad&&ad.getTime())||Number.MAX_SAFE_INTEGER)-((bd&&bd.getTime())||Number.MAX_SAFE_INTEGER)||b.delay-a.delay;});if(delayed.length){const first=delayed[0];return {task:first,name:first.name||'Tâche non renseignée',owner:String(first.owner||'').trim()||'Non attribué',delay:first.delay,lot:String(first.owner||'').trim()||'Non attribué',status:first.statusLabel||taskDisplayDelay(first.delay),kind:'trigger'};}const blocker=nextBlockingTaskForPerimeter(tasks);if(!blocker)return null;return {task:blocker.raw,name:blocker.name,owner:blocker.owner,delay:blocker.delay,lot:blocker.owner,status:taskDisplayDelay(blocker.delay),kind:'next'};}
function renderDelayTriggerChart(root,tasks){if(!root)return;const items=groupTasksByPerimeter(tasks).map(group=>{const detail=delayTriggerForPerimeter(group.tasks||[]);if(!detail)return null;return {label:group.label,task:detail.name,delay:detail.delay,lot:detail.lot,detail};}).filter(Boolean).sort((a,b)=>b.delay-a.delay||a.label.localeCompare(b.label,'fr'));if(!items.length){root.innerHTML="<div class='small'>Aucun déclencheur de retard détecté sur les tâches terminées.</div>";return;}root.innerHTML=`<div style='display:flex;flex-direction:column;gap:10px'>${items.map(x=>`<button type='button' class='perimeter-action' data-perimeter='${escAttr(x.label)}'><div style='display:flex;justify-content:space-between;gap:12px;align-items:center'><div style='flex:1;min-width:0'><div class='perimeter-kicker'>Périmètre</div><div style='font-weight:800;font-size:18px'>${esc(x.label)}</div><div class='small' style='margin-top:4px'>Tâche déclencheuse : ${esc(x.task||'-')} · ${esc(String(x.delay))} j</div></div><div style='min-width:140px;text-align:right;font-weight:900;color:#d64545'>${esc(x.lot)}</div></div></button>`).join('')}</div>`;Array.from(root.querySelectorAll('[data-perimeter]')).forEach(btn=>btn.addEventListener('click',()=>showPerimeterModal(btn.getAttribute('data-perimeter')||'',tasks)));}
function renderPerimeterProgressChart(root,tasks){if(!root)return;const items=groupTasksByPerimeter(tasks).map(group=>{const actionable=(group.tasks||[]).filter(t=>!t.is_summary);if(!actionable.length)return null;const avg=actionable.reduce((sum,t)=>sum+Number(t.progress||0),0)/actionable.length;const blocker=nextBlockingTaskForPerimeter(actionable);return {label:group.label,value:Math.max(0,Math.min(100,avg)),blocker};}).filter(Boolean).sort((a,b)=>b.value-a.value||a.label.localeCompare(b.label,'fr'));if(!items.length){root.innerHTML="<div class='small'>Aucune donnée de périmètre disponible.</div>";return;}root.innerHTML=items.map(x=>{const tooltip=x.blocker&&x.value>0&&x.value<100?`Tâche suivante : ${x.blocker.name} · retard ${x.blocker.delay} j · lot ${x.blocker.owner}`:(x.value>=100?'Périmètre terminé':'Aucune tâche bloquante identifiée');return `<button type='button' class='perimeter-action' data-perimeter='${escAttr(x.label)}' style='margin:10px 0'><div style='display:flex;justify-content:space-between;gap:12px;margin-bottom:6px'><span style='font-weight:700'>${esc(x.label)}</span><strong>${x.value.toFixed(1)}%</strong></div><div title='${escAttr(tooltip)}' style='height:14px;border-radius:999px;background:#edf1f8;overflow:hidden;cursor:pointer'><div style='height:100%;width:${x.value}%;background:linear-gradient(90deg,#1b6ef3,#5da2ff)'></div></div></button>`;}).join('');Array.from(root.querySelectorAll('[data-perimeter]')).forEach(btn=>btn.addEventListener('click',()=>showPerimeterModal(btn.getAttribute('data-perimeter')||'',tasks)));}
function renderCstTimelineChart(root,tasks){if(!root)return;const items=(tasks||[]).filter(t=>!t.is_summary&&t.is_cst).map(t=>{const start=parseDateOnly(t.start||'')||parseDateOnly(t.end||'')||parseDateOnly(t.actualEnd||'');const end=parseDateOnly(t.end||'')||parseDateOnly(t.start||'')||parseDateOnly(t.actualEnd||'');if(!start&&!end)return null;return {task:t,start:start||end,end:end||start,owner:String(t.owner||'').trim()||'Non attribué',perimeter:String(t.perimeter||'').trim()||'Général',progress:Math.max(0,Math.min(100,Number(t.progress||0))),delay:Number(t.delayDays||0)};}).filter(Boolean).sort((a,b)=>a.start-b.start||a.end-b.end||String(a.task.name||'').localeCompare(String(b.task.name||''),'fr'));if(!items.length){root.innerHTML="<div class='small'>Aucune tâche CST planifiée avec dates.</div>";return;}const min=items.reduce((acc,it)=>Math.min(acc,it.start.getTime()),items[0].start.getTime());const max=items.reduce((acc,it)=>Math.max(acc,it.end.getTime()),items[0].end.getTime());const span=Math.max(1,max-min);const today=new Date();today.setHours(0,0,0,0);const todayPct=((today.getTime()-min)/span)*100;const ticks=[];const cursor=new Date(items[0].start.getFullYear(),items[0].start.getMonth(),1);const last=new Date(items[items.length-1].end.getFullYear(),items[items.length-1].end.getMonth(),1);while(cursor<=last){ticks.push(new Date(cursor));cursor.setMonth(cursor.getMonth()+1);}root.innerHTML=`<div class='cst-timeline-wrap'><div class='cst-timeline-head'><div class='small' style='margin-bottom:10px'>Lecture macro des enchaînements CST. Le trait orange matérialise la date du jour.</div><div class='cst-axis'>${ticks.map(t=>{const pct=((t.getTime()-min)/span)*100;return `<div class='cst-axis-tick' style='left:${pct}%'>${esc(t.toLocaleDateString('fr-FR',{month:'short',year:'2-digit'}))}</div>`;}).join('')}${todayPct>=0&&todayPct<=100?`<div class='cst-today-line' style='left:${todayPct}%'></div>`:''}</div></div>${items.map(it=>{const left=((it.start.getTime()-min)/span)*100;const width=Math.max(2,((it.end.getTime()-it.start.getTime())/span)*100);const label=`${fmtDateFrShort(it.task.start||'')} → ${fmtDateFrShort(it.task.end||'')}`;return `<div class='cst-timeline-row'><div class='cst-task-meta'><div class='cst-task-name'>${esc(it.task.name||'Tâche sans nom')}</div><div class='cst-task-sub'>${esc(it.perimeter)} · ${esc(it.owner)} · ${it.progress.toFixed(0)}% · ${esc(taskDisplayDelay(it.delay))}</div></div><div class='cst-track'>${todayPct>=0&&todayPct<=100?`<div class='cst-today-line' style='left:${todayPct}%'></div>`:''}<div class='cst-bar' style='left:${left}%;width:${width}%'>${esc(label)}</div></div></div>`;}).join('')}</div>`;}
...
function showPerimeterModal(perimeter,tasks){const body=document.getElementById('perimeterModalBody');const title=document.getElementById('perimeterModalTitle');const modal=document.getElementById('perimeterModal');if(!body||!modal)return;const groups=groupTasksByPerimeter(tasks||[]);const group=groups.find(g=>normalizeText(g.label)===normalizeText(perimeter));if(title)title.textContent=`Détail du périmètre - ${perimeter||'Périmètre'}`;if(!group){body.innerHTML="<div class='small'>Aucune donnée disponible pour ce périmètre.</div>";modal.style.display='flex';return;}const detail=delayTriggerForPerimeter(group.tasks||[]);if(!detail){body.innerHTML="<div class='small'>Aucun déclencheur de retard identifié.</div>";modal.style.display='flex';return;}const triggerDate=fmtDateFr(detail.task?.actualEnd||detail.task?.end||'');body.innerHTML=`<div class='perimeter-kicker'>${detail.kind==='trigger'?'Tâche déclencheuse de retard':'Prochaine tâche bloquante'}</div><div style='font-size:22px;font-weight:900;margin-top:6px'>${esc(detail.name)}</div><div class='small' style='margin-top:6px'>Date de référence : ${esc(triggerDate)}</div><div class='perimeter-grid'><div class='detail-card'><div class='label'>Responsable</div><div class='value'>${esc(detail.owner)}</div></div><div class='detail-card'><div class='label'>Depuis combien de temps</div><div class='value'>${esc(taskDisplayDelay(detail.delay))}</div></div><div class='detail-card'><div class='label'>Périmètre</div><div class='value'>${esc(perimeter)}</div></div></div>`;modal.style.display='flex';}
function hidePerimeterModal(){const modal=document.getElementById('perimeterModal');if(modal)modal.style.display='none';}
...
async function init(){
  try{
    ...
    const reminderClose=document.getElementById('reminderModalClose');if(reminderClose)reminderClose.addEventListener('click',hideReminderModal);const perimeterClose=document.getElementById('perimeterModalClose');if(perimeterClose)perimeterClose.addEventListener('click',hidePerimeterModal);
