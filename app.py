...
function renderPerimeterProgressChart(root,tasks){if(!root)return;const items=groupTasksByPerimeter(tasks).map(group=>{const actionable=(group.tasks||[]).filter(t=>!t.is_summary);if(!actionable.length)return null;const avg=actionable.reduce((sum,t)=>sum+Number(t.progress||0),0)/actionable.length;return {label:group.label,value:Math.max(0,Math.min(100,avg))};}).filter(Boolean).sort((a,b)=>b.value-a.value||a.label.localeCompare(b.label,'fr'));if(!items.length){root.innerHTML="<div class='small'>Aucune donnée de périmètre disponible.</div>";return;}root.innerHTML=items.map(x=>`<div style='margin:10px 0'><div style='display:flex;justify-content:space-between;gap:12px;margin-bottom:6px'><span style='font-weight:700'>${esc(x.label)}</span><strong>${x.value.toFixed(1)}%</strong></div><div style='height:14px;border-radius:999px;background:#edf1f8;overflow:hidden'><div style='height:100%;width:${x.value}%;background:linear-gradient(90deg,#1b6ef3,#5da2ff)'></div></div></div>`).join('');}
...
function renderPointageAnalytics(root,tasks){if(!root)return;const stats={};const push=(name,delay)=>{const key=String(name||'').trim();if(!key || key==='Non attribué' || key.includes('+'))return;const rec=stats[key]||(stats[key]={sum:0,count:0});rec.sum+=Number(delay||0);rec.count+=1;};for(const t of tasks){if(t.is_summary)continue;if(t.is_cet&&Array.isArray(t.cetMembers)&&t.cetMembers.length){for(const m of t.cetMembers){const cm=((t.cet||{})[m]||{});const isPointed=Number(cm.progress||0)>0 || !!cm.actualEnd;const isDone=cm.status==='past' || !!cm.actualEnd;if(isPointed&&isDone)push(m,cm.delayDays||0);}continue;}const owner=String(t.owner||'').trim();const isPointed=Number(t.progress||0)>0 || !!t.actualEnd;const isDone=t.status==='past' || !!t.actualEnd;if(isPointed&&isDone)push(owner,t.delayDays||0);}const items=Object.entries(stats).map(([name,v])=>({name,avg:v.count?(v.sum/v.count):0,count:v.count})).filter(x=>x.count>0).sort((a,b)=>b.avg-a.avg||a.name.localeCompare(b.name,'fr'));if(!items.length){root.innerHTML="<div class='small'>Aucune donnée de pointage terminée.</div>";return;}const max=Math.max(1,...items.map(x=>Number(x.avg||0)));root.innerHTML=`<div style='display:flex;align-items:flex-end;gap:14px;height:260px;padding:10px 0'>${items.map(x=>{const v=Number(x.avg||0);const h=Math.max(10,(v/max)*190);return `<div style='flex:1;min-width:90px;text-align:center'><div title='${esc(x.name)}: ${v.toFixed(1)} j de retard moyen' style='margin:0 auto;width:42px;height:${h}px;background:${x.name==='CST'?'#ef8d00':'#d64545'};border-radius:10px 10px 0 0'></div><div style='font-size:12px;margin-top:8px;font-weight:800'>${esc(x.name)}</div><div style='font-size:12px;color:#6e7a90'>${v.toFixed(1)} j</div><div style='font-size:11px;color:#8a94a8'>${x.count} tâche(s)</div></div>`;}).join('')}</div>`;}
