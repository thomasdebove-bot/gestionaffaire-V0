...
</div></div><div id='printBoardSheet' class='print-board-sheet'></div>
<script>
...
function renderPointageFinanceChart(){const sec=document.getElementById('pointagePlanningFinanceSection');const root=document.getElementById('pointageFinanceChart');const title=document.getElementById('pointageFinanceChartTitle');const a=state.selectedAffaire||{};const boond=(state.boondImputation||{}).totals||{};const taskCount=Number(a.pointage_task_count||0);if(!sec||!root)return;if(taskCount<=0){sec.style.display='none';root.innerHTML="<div class='small'>Aucune donnée de pointage planning.</div>";return;}sec.style.display='block';const mode=((document.getElementById('pointageFinanceChartSelect')||{}).value)||'planned-vs-pointed';const rows=mode==='pointed-vs-boond'?[{label:'Vente pointée à date',value:Number(a.pointage_progress_amount||0),color:'#ef8d00',subtitle:'Commande x avancement réellement pointé'},{label:'Coût de production BOOND',value:Number(boond.total_cost||0),color:'#1b6ef3',subtitle:'Productivité projet BOOND'}]:[{label:'Vente planifiée à date',value:Number(a.pointage_planned_to_date_amount||0),color:'#5da2ff',subtitle:`Où le planning devrait en être au ${new Date().toLocaleDateString('fr-FR')}`},{label:'Vente pointée à date',value:Number(a.pointage_progress_amount||0),color:'#ef8d00',subtitle:'Commande x avancement réellement pointé'}];if(title)title.textContent=mode==='pointed-vs-boond'?'Vente pointée à date vs coût de production BOOND':'Vente planifiée à date vs vente pointée à date';const max=Math.max(1,...rows.map(r=>Number(r.value||0)));root.innerHTML=`<div style='display:grid;grid-template-columns:repeat(${rows.length},minmax(260px,1fr));gap:18px'>${rows.map(r=>{const width=Math.max(4,(Number(r.value||0)/max)*100);return `<div class='pilot-row' style='display:block;padding:18px'><div style='display:flex;justify-content:space-between;gap:18px;align-items:flex-start'><div style='min-width:0'><div style='font-weight:800;font-size:16px'>${esc(r.label)}</div><div class='small' style='margin-top:4px'>${esc(r.subtitle)}</div></div><div style='font-weight:900;white-space:nowrap'>${euro(r.value||0)}</div></div><div style='margin-top:14px;height:14px;border-radius:999px;background:#edf1f8;overflow:hidden'><div style='height:100%;width:${width}%;background:${r.color}'></div></div></div>`;}).join('')}</div>`;}
...
<body><div class='wrap'><div class='board-shell'>
  ...
  <section id='section-finance' class='board-section'>
    ...
    <div id='chart-finance-curve' class='chart-box'><div class='legend'><span class='pre'>Prévisionnel</span><span class='fac'>Facturation</span><span class='prod'>Production</span></div><svg id='financeChart' class='finance-svg' viewBox='0 0 980 320' preserveAspectRatio='none'></svg><div id='financeSummary'></div><div id='financePrintSummary' class='print-only'></div></div>
...
function getBoardNotes(){try{return JSON.parse(localStorage.getItem(projectStorageKey('board-notes'))||'{}')||{};}catch(_){return {};}}
function compactRows(items, mapper, emptyCols=1){return (items||[]).length?(items||[]).map(mapper).join(''):`<tr><td colspan="${emptyCols}">Aucune donnée</td></tr>`;}
function renderPrintBoardSheet({affaire,board,planning,analysisReactivity,planningResources,planningResourcesToday}){const root=document.getElementById('printBoardSheet');if(!root)return;const p=(board&&board.kpis_pilotage)||{};const notes=getBoardNotes();const billed=Number(affaire.facturation_totale||((affaire.anteriorite||0)+(affaire.facture_2026||affaire.facturation_cumulee_2026||0)));const rowsCompany=(p.rappels_cumules_par_entreprise||p.reminders_by_company||[]);const analysisCompany=((p.project_company_views||{}).reminder||[]).slice(0,8);const missions=(affaire.missions||[]);const img=String(board.project_image||affaire.project_image||affaire.image_url||affaire.image||'').trim();const projectTitle=String(board.project_name||affaire.affaire||affaire.display_name||affaireId).toUpperCase();const projectDesc=String(board.project_description||affaire.project_description||affaire.start_sentence||affaire.description||'Description indisponible');const boardDate=(notes.board_date||new Date().toISOString().slice(0,10)).split('-').reverse().join('/');const notesText=[notes.general,notes.metronome,notes.analysis,notes.planning,notes.finance].filter(Boolean).join('\n\n');const resourceNames=Array.from(new Set([...planningResources.map(x=>x.name),...planningResourcesToday.map(x=>x.name)]));root.innerHTML=`<div class="print-board-page"><div class="print-board-grid"><section class="print-block print-span-4"><div class="print-project">${img?`<img src="${esc(img)}" alt="Projet">`:'<div></div>'}<div><div class="print-title">Board client · ${esc(boardDate)}</div><div class="print-project-title">${esc(projectTitle)}</div><div class="print-project-desc">${esc(projectDesc)}</div></div></div></section><section class="print-block print-span-4"><div class="print-title">Synthèse commande / planning</div><div class="print-kpi-grid"><div class="print-kpi"><div class="l">Commande HT</div><div class="v">${esc(euro(affaire.commande_ht||0))}</div></div><div class="print-kpi"><div class="l">Reste à facturer</div><div class="v">${esc(euro(affaire.reste_a_facturer||0))}</div></div><div class="print-kpi"><div class="l">% facturé</div><div class="v">${(Number(affaire.commande_ht||0)>0?(billed/Number(affaire.commande_ht||0))*100:0).toFixed(1)} %</div></div><div class="print-kpi"><div class="l">Planning attendu</div><div class="v">${Number((planning.expected[0]||{}).value||0).toFixed(1)} %</div></div><div class="print-kpi"><div class="l">Planning pointé</div><div class="v">${(Number(affaire.pointage_progress_ratio||0)*100).toFixed(1)} %</div></div></div></section><section class="print-block"><div class="print-title">Métro</div><div class="print-mini-list"><div class="print-mini-row"><span>Rappels ouverts à date</span><strong>${Number(p.rappels_ouverts_a_date||p.reminders_open_count||0)}</strong></div><div class="print-mini-row"><span>À suivre ouverts</span><strong>${Number(p.a_suivre_ouverts||p.followups_open_count||0)}</strong></div><div class="print-mini-row"><span>Date de référence</span><strong>${esc(String(p.date_reference||p.reference_date||'-'))}</strong></div></div><table class="print-table" style="margin-top:2mm"><thead><tr><th>Entreprise</th><th>Rappels</th></tr></thead><tbody>${compactRows(rowsCompany,r=>`<tr><td>${esc(r.name||r.company||'-')}</td><td class="num">${Number(r.count||0)}</td></tr>`,2)}</tbody></table></section><section class="print-block"><div class="print-title">Analyse</div><table class="print-table"><thead><tr><th>Entreprise</th><th>Ouverts</th></tr></thead><tbody>${compactRows(analysisCompany,r=>`<tr><td>${esc(r.name||r.label||'-')}</td><td class="num">${Number(r.count||r.value||0)}</td></tr>`,2)}</tbody></table><div class="print-title" style="margin-top:2mm">Réactivité</div><table class="print-table"><thead><tr><th>Entreprise</th><th>Retard moyen</th></tr></thead><tbody>${compactRows(analysisReactivity,r=>`<tr><td>${esc(r.name||'-')}</td><td class="num">${Number(r.avg_gap_days??r.avg??0).toFixed(1)} j</td></tr>`,2)}</tbody></table></section><section class="print-block"><div class="print-title">Planning — déclencheurs</div><table class="print-table"><thead><tr><th>Périmètre</th><th>Détail</th><th>Resp.</th></tr></thead><tbody>${compactRows(planning.delayTrigger,r=>`<tr><td>${esc(r.label||'-')}</td><td>${esc(r.subtitle||'')}</td><td>${esc(r.side||'-')}</td></tr>`,3)}</tbody></table><div class="print-title" style="margin-top:2mm">Périmètres</div><table class="print-table"><thead><tr><th>Périmètre</th><th>Av.</th><th>Blocage</th></tr></thead><tbody>${compactRows(planning.perimeterProgress,r=>`<tr><td>${esc(r.label||'-')}</td><td class="num">${Number(r.value||0).toFixed(1)}%</td><td>${esc(r.subtitle||'')}</td></tr>`,3)}</tbody></table></section><section class="print-block"><div class="print-title">Planning — ressources</div><table class="print-table"><thead><tr><th>Ressource</th><th>Pointé</th><th>À date</th></tr></thead><tbody>${resourceNames.length?resourceNames.map(name=>{const a=planningResources.find(x=>x.name===name);const b=planningResourcesToday.find(x=>x.name===name);return `<tr><td>${esc(name)}</td><td class="num">${a?`${Number(a.avg||0).toFixed(1)} j`:'-'}</td><td class="num">${b?`${Number(b.avg||0).toFixed(1)} j`:'-'}</td></tr>`;}).join(''):'<tr><td colspan="3">Aucune donnée</td></tr>'}</tbody></table><div class="print-title" style="margin-top:2mm">Avancement</div><table class="print-table"><thead><tr><th>Indicateur</th><th>Valeur</th><th>Détail</th></tr></thead><tbody>${compactRows(planning.expected,r=>`<tr><td>${esc(r.label||'-')}</td><td class="num">${Number(r.value||0).toFixed(1)}%</td><td>${esc(r.subtitle||'')}</td></tr>`,3)}</tbody></table></section><section class="print-block print-span-2"><div class="print-title">Chronologie CST</div><table class="print-table"><thead><tr><th>Périmètre</th><th>Tâche</th><th>Début</th><th>Fin</th><th>Retard</th></tr></thead><tbody>${compactRows(planning.cst,r=>`<tr><td>${esc(r.label||'-')}</td><td>${esc(r.task||'-')}</td><td>${esc(fmtDate(r.start&&r.start.toISOString().slice(0,10)))}</td><td>${esc(fmtDate(r.end&&r.end.toISOString().slice(0,10)))}</td><td class="num">${Number(r.delay||0)} j</td></tr>`,5)}</tbody></table></section><section class="print-block"><div class="print-title">Finance</div><table class="print-table"><thead><tr><th>Indicateur</th><th>Montant</th></tr></thead><tbody><tr><td>Commande HT</td><td class="num">${esc(euro(affaire.commande_ht||0))}</td></tr><tr><td>Facturation cumulée</td><td class="num">${esc(euro(billed))}</td></tr><tr><td>Reste à facturer</td><td class="num">${esc(euro(affaire.reste_a_facturer||0))}</td></tr></tbody></table></section><section class="print-block print-span-4"><div class="print-title">Missions</div><table class="print-table"><thead><tr><th>Tag</th><th>Mission</th><th>N°</th><th>Commande</th><th>Facturation</th><th>Reste</th><th>Prévisionnel</th><th>Facturé</th></tr></thead><tbody>${missions.length?missions.map(m=>`<tr><td>${esc(m.tag||'')}</td><td>${esc(m.label||'')}</td><td>${esc(m.numero||'')}</td><td class="num">${esc(euro(m.commande_ht))}</td><td class="num">${esc(euro(m.facturation_totale||((m.anteriorite||0)+(m.facture_2026||m.facturation_cumulee_2026||0))))}</td><td class="num">${esc(euro(m.reste_a_facturer))}</td><td class="num">${esc(euro(m.total_previsionnel))}</td><td class="num">${esc(euro(m.total_facture))}</td></tr>`).join(''):'<tr><td colspan="8">Aucune mission détaillée.</td></tr>'}</tbody></table></section><section class="print-block print-span-4"><div class="print-title">Notes</div><div class="print-notes">${esc(notesText||'Aucune note.')}</div></section></div></div>`;}
async function init(){restoreLayout();restoreNotes();document.querySelectorAll('[data-notes-target]').forEach(btn=>btn.addEventListener('click',()=>toggleNotes(btn.getAttribute('data-notes-target'))));document.getElementById('exportBtn').addEventListener('click',exportBoard);document.getElementById('importBtn').addEventListener('click',()=>document.getElementById('importInput').click());document.getElementById('importInput').addEventListener('change',async e=>{if(e.target.files&&e.target.files[0]){await importBoard(e.target.files[0]);e.target.value='';}});document.getElementById('printBtn').addEventListener('click',()=>{if(window.__boardPrintData){renderPrintBoardSheet(window.__boardPrintData);}window.print();});document.getElementById('closeBtn').addEventListener('click',()=>window.close());const reminderModal=document.getElementById('companyReminderModal');if(reminderModal)reminderModal.addEventListener('click',e=>{if(e.target===reminderModal)hideCompanyReminderModal();});if(!affaireId){setText('projectTitle','Aucune affaire sélectionnée');return;}const finance=await api(`/api/finance/affaire/${encodeURIComponent(affaireId)}`);let board={};let pointage={};try{board=await api(`/api/project-management/board?affaire_id=${encodeURIComponent(affaireId)}`);}catch(_){board={};}try{pointage=await api(`/api/project-management/pointage?affaire_id=${encodeURIComponent(affaireId)}`);}catch(_){pointage={};}const affaire=(finance&&finance.affaire)||{};const billed=Number(affaire.facturation_totale||((affaire.anteriorite||0)+(affaire.facture_2026||affaire.facturation_cumulee_2026||0)));const budget=Number(affaire.commande_ht||0);const billedPct=budget>0?(billed/budget)*100:0;const projectTitle=String(board.project_name||affaire.affaire||affaire.display_name||affaireId).toUpperCase();const projectDesc=String(board.project_description||affaire.project_description||affaire.start_sentence||affaire.description||'').trim();const projectImage=String(board.project_image||affaire.project_image||affaire.image_url||affaire.image||'').trim();setText('projectTitle',projectTitle);setText('projectDesc',projectDesc||'Description indisponible');const img=document.getElementById('projectImage');if(projectImage){img.src=projectImage;img.style.display='block';}else{img.removeAttribute('src');img.style.display='none';}setText('pillBudget',euro(affaire.commande_ht||0));setText('pillRemain',euro(affaire.reste_a_facturer||0));setText('pillBilledPct',`${billedPct.toFixed(1)} %`);const planning=computePlanningDatasets((pointage&&pointage.tasks)||[]);setText('pillPlannedPct',`${Number((planning.expected[0]||{}).value||0).toFixed(1)} %`);setText('pillPointedPct',`${(Number(affaire.pointage_progress_ratio||0)*100).toFixed(1)} %`);const p=(board&&board.kpis_pilotage)||{};setText('metroReminder',String(Number(p.rappels_ouverts_a_date||p.reminders_open_count||0)));setText('metroFollow',String(Number(p.a_suivre_ouverts||p.followups_open_count||0)));setText('metroDate',p.date_reference||p.reference_date||'-');document.getElementById('metroCompanies').innerHTML=renderCompanyRows((p.rappels_cumules_par_entreprise||p.reminders_by_company||[]));Array.from(document.querySelectorAll('.company-row-btn')).forEach(btn=>btn.addEventListener('click',()=>showCompanyReminderModal(btn.getAttribute('data-company')||'',board)));const closeModal=document.getElementById('companyReminderClose');if(closeModal)closeModal.onclick=hideCompanyReminderModal;const analysisReactivity=(p.reactivity_by_company||[]).slice(0,8);const planningResources=planning.resources.map(x=>({name:x.name,avg:x.avg}));const planningResourcesToday=planning.resourcesToday.map(x=>({name:x.name,avg:x.avg}));renderBarList('analysisBars',((p.project_company_views||{}).reminder||[]).slice(0,8));renderVerticalBars('analysisReactivity',analysisReactivity,{tempoName:'tempo'});renderPrintMetricList('analysisReactivityPrint',analysisReactivity,{suffix:' j'});renderPlanningCards('planningDelay',planning.delayTrigger.map(x=>({label:x.label,subtitle:x.subtitle,side:x.side})));renderProgressRows('planningProgress',planning.perimeterProgress);renderVerticalBars('planningResources',planningResources,{tempoName:'cst'});renderPrintMetricList('planningResourcesPrint',planningResources,{suffix:' j'});renderVerticalBars('planningResourcesToday',planningResourcesToday,{tempoName:'cst'});renderPrintMetricList('planningResourcesTodayPrint',planningResourcesToday,{suffix:' j'});renderProgressRows('planningExpected',planning.expected);renderCstTimeline(planning.cst);renderFinance(affaire);window.__boardPrintData={affaire,board,planning,analysisReactivity,planningResources,planningResourcesToday};}
