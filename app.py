...
<style>
:root{--line:#d9e1ef;--ink:#18273f;--muted:#617089;--accent:#ef8d00;--panel:#fff;--bg:#eef3fa;--shadow:0 12px 34px rgba(15,30,54,.08)}
*{box-sizing:border-box}body{margin:0;font-family:Inter,Segoe UI,Arial,sans-serif;background:var(--bg);color:var(--ink)}
.wrap{max-width:1480px;margin:20px auto;padding:0 16px}.sheet{background:var(--panel);border:1px solid var(--line);border-radius:24px;box-shadow:var(--shadow);padding:22px}.top{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap}.eyeb{font-size:12px;font-weight:900;letter-spacing:.14em;color:var(--accent);text-transform:uppercase}.title{font-size:38px;font-weight:900;margin:8px 0 4px}.subtitle{color:var(--muted);font-size:18px}.actions{display:flex;gap:10px;flex-wrap:wrap}.btn{height:44px;border:none;border-radius:12px;padding:0 16px;background:var(--accent);color:#fff;font-weight:800;cursor:pointer}.btn.secondary{background:#122033}.meta{margin-top:18px;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.meta-card,.kpi,.notes-card,.table-card{border:1px solid var(--line);border-radius:18px;background:#fff;padding:16px}.meta-card .label,.kpi .label{font-size:12px;font-weight:800;color:var(--muted);text-transform:uppercase}.meta-card .value{margin-top:8px;font-size:18px;font-weight:800}.kpi-grid{margin-top:16px;display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:12px}.kpi .value{margin-top:8px;font-size:30px;font-weight:900}.board-grid{margin-top:16px;display:grid;grid-template-columns:1.2fr .8fr;gap:14px}.note-box{width:100%;min-height:260px;border:1px dashed #b8c4d8;border-radius:16px;padding:14px;font:inherit;resize:vertical;background:#fbfdff}.note-help{margin-top:8px;color:var(--muted);font-size:12px}.mini-list{display:grid;gap:10px}.mini-row{display:flex;justify-content:space-between;gap:10px;padding:12px;border:1px solid var(--line);border-radius:12px;background:#fbfdff}.mini-row .name{font-weight:800}.table-card{margin-top:16px}.loading{padding:24px;text-align:center;color:var(--muted)}
@media (max-width:1100px){.meta,.kpi-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.board-grid{grid-template-columns:1fr}}@media print{body{background:#fff}.wrap{max-width:none;margin:0;padding:0}.sheet{border:none;box-shadow:none;border-radius:0;padding:0}.actions{display:none}.note-box{border:1px solid #c8d1e2;background:#fff;min-height:180px;resize:none}}
</style></head>
<body><div class='wrap'><div class='sheet'>
  <div class='top'>
    <div><div class='eyeb'>BOARD CLIENT</div><div id='title' class='title'>Chargement…</div><div id='subtitle' class='subtitle'>Vue imprimable des KPI affaire.</div></div>
    <div class='actions'><button class='btn secondary' onclick='window.close()'>Fermer</button><button class='btn' onclick='window.print()'>Imprimer</button></div>
  </div>
  <div class='meta'><div class='meta-card'><div class='label'>Affaire</div><div id='metaAffaire' class='value'>-</div></div><div class='meta-card'><div class='label'>Client</div><div id='metaClient' class='value'>-</div></div><div class='meta-card'><div class='label'>Date de référence</div><div id='metaDate' class='value'>-</div></div><div class='meta-card'><div class='label'>Statut BOARD</div><div id='metaStatus' class='value'>Préparation</div></div></div>
  <div class='kpi-grid'><div class='kpi'><div class='label'>Sujets ouverts</div><div id='kOpen' class='value'>0</div></div><div class='kpi'><div class='label'>Sujets en retard</div><div id='kLate' class='value'>0</div></div><div class='kpi'><div class='label'>Rappels ouverts</div><div id='kReminder' class='value'>0</div></div><div class='kpi'><div class='label'>À suivre ouverts</div><div id='kFollow' class='value'>0</div></div><div class='kpi'><div class='label'>Commande HT</div><div id='kBudget' class='value'>0 €</div></div><div class='kpi'><div class='label'>Reste à facturer</div><div id='kRemain' class='value'>0 €</div></div></div>
  <div class='board-grid'><div class='notes-card'><div class='label' style='font-size:12px;font-weight:800;color:var(--muted);text-transform:uppercase'>Messages clés / notes client</div><textarea id='notes' class='note-box' placeholder='Saisir ici les points importants à communiquer au client…'></textarea><div class='note-help'>Les notes restent modifiables dans cette fenêtre et s’impriment avec le BOARD.</div></div><div class='notes-card'><div class='label' style='font-size:12px;font-weight:800;color:var(--muted);text-transform:uppercase'>KPI à partager</div><div id='miniList' class='mini-list'><div class='loading'>Chargement des indicateurs…</div></div></div></div>
  <div class='table-card'><div class='label' style='font-size:12px;font-weight:800;color:var(--muted);text-transform:uppercase;margin-bottom:10px'>Top rappels par entreprise</div><div id='companyList' class='mini-list'><div class='loading'>Chargement…</div></div></div>
</div></div>
<script>
const euro=v=>new Intl.NumberFormat('fr-FR',{style:'currency',currency:'EUR',maximumFractionDigits:0}).format(Number(v||0));
const esc=v=>String(v||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
...
async function init(){if(!affaireId){setText('title','Aucune affaire sélectionnée');setText('subtitle','Ouvrez cette page depuis la gestion de projet pour générer le BOARD.');return;}restoreNotes();document.getElementById('notes').addEventListener('input',saveNotes);const finance=await api(`/api/finance/affaire/${encodeURIComponent(affaireId)}`);let board=null;try{board=await api(`/api/project-management/board?affaire_id=${encodeURIComponent(affaireId)}`);}catch(_){board={};}const affaire=(finance&&finance.affaire)||{};const k=(board&&board.kpis)||{};const p=(board&&board.kpis_pilotage)||{};setText('title',affaire.display_name||affaire.affaire||affaireId);setText('subtitle',`BOARD client prêt à être partagé${affaire.client?` · ${affaire.client}`:''}`);setText('metaAffaire',affaire.affaire||affaire.display_name||affaireId);setText('metaClient',affaire.client||'-');setText('metaDate',p.date_reference||p.reference_date||new Date().toLocaleDateString('fr-FR'));setText('metaStatus',Number(k.overdue_topics||0)>0?'Sous surveillance':'Pilotage OK');setText('kOpen',String(Number(k.open_topics||0)));setText('kLate',String(Number(k.overdue_topics||0)));setText('kReminder',String(Number(p.rappels_ouverts_a_date||p.reminders_open_count||0)));setText('kFollow',String(Number(p.a_suivre_ouverts||p.followups_open_count||0)));setText('kBudget',euro(affaire.commande_ht||0));setText('kRemain',euro(affaire.reste_a_facturer||0));document.getElementById('miniList').innerHTML=[['Facturé',euro(affaire.facturation_totale||((affaire.anteriorite||0)+(affaire.facture_2026||affaire.facturation_cumulee_2026||0)))],['Avancement planning',`${(Number(affaire.pointage_progress_ratio||0)*100).toFixed(1)} %`],['Tâches prises en compte',`${Number(affaire.pointage_task_count||0)} tâche(s)`],['Projet METRONOME',board&&board.project_name?board.project_name:'Non résolu']].map(([a,b])=>`<div class='mini-row'><span class='name'>${esc(a)}</span><span>${esc(b)}</span></div>`).join('');const companies=(p.rappels_cumules_par_entreprise||p.reminders_by_company||[]);document.getElementById('companyList').innerHTML=companies.length?companies.slice(0,8).map(r=>`<div class='mini-row'><span class='name'>${esc(r.name||r.company||'-')}</span><span>${Number(r.count||0)} rappel(s)</span></div>`).join(''):"<div class='loading'>Aucun rappel ouvert par entreprise.</div>";}
init().catch(err=>{setText('title','Erreur de chargement');setText('subtitle',err.message||'Impossible de construire le BOARD.');});
</script></body></html>"""
