...
function resolveBoardAffaire(input){const raw=String(input||'').trim();if(!raw)return null;const low=raw.toLowerCase();const slug=raw.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'');return (state.affaires||[]).find(a=>String(a.affaire_id||'')===raw)||(state.affaires||[]).find(a=>String(a.display_name||'').toLowerCase()===low)||(state.affaires||[]).find(a=>String(a.display_name||'').toLowerCase().startsWith(low))||(state.affaires||[]).find(a=>String((a.display_name||'').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,''))===slug)||null;}
async function loadAffaires(search=''){const d=await api(`/api/finance/affaires?search=${encodeURIComponent(search)}`);state.affaires=d.items||[];const sel=document.getElementById('affaireSelect');sel.innerHTML=`<option value=''>Sélectionnez une affaire</option>`+state.affaires.map(x=>`<option value='${esc(x.affaire_id)}'>${esc(x.display_name)}</option>`).join('');const found=resolveBoardAffaire(state.selectedId);if(found){state.selectedId=found.affaire_id||state.selectedId;sel.value=found.affaire_id;}else if(state.affaires.length){state.selectedId=state.affaires[0].affaire_id||'';sel.value=state.selectedId;}else{state.selectedId='';}}
...
async function loadProject(id){const found=resolveBoardAffaire(id);const selectedId=(found&&found.affaire_id)||String(id||'').trim();state.selectedId=selectedId;if(!selectedId){state.affaire=null;state.board=null;state.pointage=null;renderAll();return;}const sel=document.getElementById('affaireSelect');if(sel)sel.value=selectedId;const search=document.getElementById('searchInput');if(search&&found)search.value=found.display_name||'';localStorage.setItem('selectedAffaireId',selectedId);window.history.replaceState({},'',`/reunion-board?affaire_id=${encodeURIComponent(selectedId)}`);const [aff,board,pointage]=await Promise.all([api(`/api/finance/affaire/${encodeURIComponent(selectedId)}`),api(`/api/project-management/board?affaire_id=${encodeURIComponent(selectedId)}`).catch(()=>({})),api(`/api/project-management/pointage?affaire_id=${encodeURIComponent(selectedId)}`).catch(()=>({}))]);state.affaire=aff.affaire||null;state.board=board||null;state.pointage=pointage||null;if(search&&state.affaire)search.value=state.affaire.display_name||found?.display_name||'';renderAll();}
async function init(){document.getElementById('printBtn').addEventListener('click',()=>window.print());document.getElementById('saveNotesBtn').addEventListener('click',saveNotes);document.getElementById('clearNotesBtn').addEventListener('click',()=>{document.getElementById('notesInput').value='';saveNotes();});document.getElementById('notesInput').addEventListener('input',()=>{if(state.selectedId)localStorage.setItem(noteKey(),document.getElementById('notesInput').value||'');});document.getElementById('meetingTitleInput').addEventListener('input',()=>{if(state.selectedId)localStorage.setItem(`${noteKey()}::title`,document.getElementById('meetingTitleInput').value||'');});document.getElementById('meetingDateInput').value=new Date().toISOString().slice(0,10);const refreshMeetingDate=()=>{document.getElementById('datePill').textContent=`Date de réunion : ${fmtDate(document.getElementById('meetingDateInput').value||'')}`;};document.getElementById('meetingDateInput').addEventListener('change',refreshMeetingDate);refreshMeetingDate();await loadAffaires('');const params=new URLSearchParams(window.location.search);const pre=params.get('affaire_id')||localStorage.getItem('selectedAffaireId')||state.affaires[0]?.affaire_id||'';state.selectedId=pre;const found=resolveBoardAffaire(pre);const targetId=(found&&found.affaire_id)||pre||state.affaires[0]?.affaire_id||'';if(targetId){await loadProject(targetId);}else{renderAll();}document.getElementById('searchInput').addEventListener('input',async e=>{await loadAffaires(e.target.value||'');});document.getElementById('affaireSelect').addEventListener('change',async e=>{await loadProject(e.target.value||'');});}
init();
</script></body></html>"""
