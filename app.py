...
function updateUi(){document.getElementById('projectBadge').textContent=state.selectedLabel?`Affaire : ${state.selectedLabel}`:'Aucune affaire sélectionnée';document.getElementById('state').textContent=state.selectedLabel?`Vous naviguez sur l'affaire ${state.selectedLabel}.`:'Sélectionnez une affaire pour activer les modules.';setModuleLink('financeLink','finance');setModuleLink('dashboardLink','dashboard');setModuleLink('pmLink','gestion-projet');setModuleLink('meetingBoardLink','reunion-board');if(state.selectedId){localStorage.setItem('selectedAffaireId',state.selectedId);const pending=localStorage.getItem('pendingModuleAfterSelection')||'';if(pending){localStorage.removeItem('pendingModuleAfterSelection');window.location.href=`/${pending}?affaire_id=${encodeURIComponent(state.selectedId)}`;return;}}else{localStorage.removeItem('selectedAffaireId');}}
