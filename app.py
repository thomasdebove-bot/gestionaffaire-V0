...
<body><div class='wrap'>
  ...
  <div id='sectionNiveau2Charts' class='section'>
    <h3>Niveau 2 — Graphiques planning</h3>
    <div class='chart-controls'>
      <label for='level2ChartSelect' class='small'>Afficher :</label>
      <select id='level2ChartSelect' class='chart-select'>
    ...
    <div id='level2ChartTitle' class='small' style='font-weight:800;margin-bottom:8px'>Réactivité par entreprise suivant réunion MÉTRONOME</div>
...
function renderLevel2Chart(){const root=document.getElementById('level2ChartList');const title=document.getElementById('level2ChartTitle');const mode=((document.getElementById('level2ChartSelect')||{}).value)||'reactivity';const board=state.board||{};const pointage=state.pointage||{};if(mode==='reactivity'){if(title)title.textContent='Réactivité par entreprise suivant réunion MÉTRONOME';renderReactivityChart(root,(((board||{}).kpis_pilotage)||{}).reactivity_by_company||[]);return;}if(mode==='delay-trigger'){if(title)title.textContent='Déclencheur de retard';renderDelayTriggerChart(root,(pointage&&pointage.tasks)||[]);return;}if(title)title.textContent='Avancement par périmètre';renderPerimeterProgressChart(root,(pointage&&pointage.tasks)||[]);}
function renderReactivity(b){renderLevel2Chart();}
...
async function init(){
  try{
    ...
    const level2ChartSelect=document.getElementById('level2ChartSelect');if(level2ChartSelect)level2ChartSelect.addEventListener('change',()=>renderLevel2Chart());
