...
<body><div class='wrap'><div class='board-shell'>
  ...
  <section id='section-planning' class='board-section'>
    ...
    <div id='chart-planning-cst' class='chart-box' style='margin-top:14px'><div style='display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap'><div class='chart-title' style='margin:0'>Chronologie des périmètres</div><div class='section-tools'><button id='planningTimelineHelpBtn' class='notes-btn screen-only' type='button' style='height:30px'>?</button><label class='mini-toggle screen-only'><input id='planningTimelineActualToggle' type='checkbox'>Afficher le pointé</label></div></div><div id='planningCst' class='timeline-macro' style='margin-top:10px'></div><div id='planningCstPrint' class='print-only'></div></div>
  ...
  <section id='section-finance' class='board-section'>
    ...
    <div id='planningTimelineModal' class='pm-modal hidden'><div class='pm-modal-box'><div style='display:flex;justify-content:space-between;align-items:center;gap:12px'><h3 style='margin:0'>Chronologie des périmètres — aide de lecture</h3><button id='planningTimelineModalClose' class='action-btn' style='height:38px;background:#1b6ef3;color:#fff;border-color:#1b6ef3'>Fermer</button></div><div id='planningTimelineModalBody' style='margin-top:14px'></div></div></div>
...
function showPlanningTimelineModal(){const modal=document.getElementById('planningTimelineModal');const body=document.getElementById('planningTimelineModalBody');if(body){body.innerHTML=`<div class='finance-prod-note'><strong>Lecture rapide :</strong> chaque ligne est un périmètre. Barre orange = période planning. Barre bleue (option) = période pointée. Trait vertical = date du jour.</div><div class='finance-prod-steps'><div class='finance-prod-step'><div class='label'>Fenêtre globale</div><div class='small'>Axe horizontal borné par la première date de début et la dernière date de fin des périmètres.</div></div><div class='finance-prod-step'><div class='label'>Pourquoi la ligne peut sembler à droite</div><div class='small'>Si la date du jour est proche de la fin de la fenêtre globale, le marqueur est naturellement proche du bord droit.</div></div><div class='finance-prod-step'><div class='label'>Hauteur limitée</div><div class='small'>Le bloc affiche 5 lignes maximum, puis un défilement interne pour conserver une page lisible.</div></div></div>`;}if(modal)modal.classList.remove('hidden');}
function hidePlanningTimelineModal(){const modal=document.getElementById('planningTimelineModal');if(modal)modal.classList.add('hidden');}
function showPlanningDelayModal({rootId,item,suffix=' j'}={}){const modal=document.getElementById('planningDelayModal');const title=document.getElementById('planningDelayModalTitle');const body=document.getElementById('planningDelayModalBody');if(title)title.textContent=rootId==='planningResourcesToday'?'Retard moyen par ressource à date':'Retard moyen par ressource';if(body){const val=Number(item&& (item.avg_gap_days??item.avg) ||0);const count=Number(item&&item.count||0);body.innerHTML=`<div class='detail-main'><div class='detail-badge'>${val.toFixed(1)}${suffix}</div><div style='margin-top:12px;font-size:20px;font-weight:900'>${esc((item&&item.name)||'-')}</div><div class='small' style='margin-top:10px'>Moyenne calculée sur ${count} tâche(s) ${rootId==='planningResourcesToday'?'(vue à date)':'(vue pointée)'}. Cliquez les autres barres pour comparer les retards.</div></div>`;}if(modal)modal.classList.remove('hidden');}
