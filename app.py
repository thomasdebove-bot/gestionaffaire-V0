...
function renderVerticalBars(rootId,items,{tempoName='tempo',suffix=' j'}={}){const root=document.getElementById(rootId);if(!root)return;if(!items.length){root.innerHTML=`<div class='company-row'><span class='name'>Aucune donnée</span><span class='count'>0</span></div>`;return;}const max=Math.max(1,...items.map(x=>Math.abs(Number(x.avg_gap_days??x.avg??0))));root.innerHTML=items.map(x=>{const val=Number(x.avg_gap_days??x.avg??0);const h=Math.max(8,(Math.abs(val)/max)*168);const cls=normalizeText(x.name||'')===tempoName?'tempo':'';return `<div class='react-col'><div class='react-stick ${cls}' style='height:${h}px'></div><div class='react-name'>${esc(x.name||'-')}</div><div class='react-val'>${val.toFixed(1)}${suffix}</div></div>`;}).join('');}
